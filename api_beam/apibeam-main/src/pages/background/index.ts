...
// This bundle ships with its local relay, so default to it rather than routing
// browser-session prompts through the upstream hosted service. Users can still
// configure an HTTPS relay in the extension Settings page.
...
const getApiBaseUrl = async (): Promise<string> => {
  return new Promise((resolve) => {
    chrome.storage.local.get(["apiBaseUrl"], (result) => {
      ...
      const baseUrl = (stored || DEFAULT_API_BASE_URL).trim();
...
chrome.runtime.onMessage.addListener(
  async (msg: AgentMessage, _sender, sendResponse) => {
    ...
    if (msg.type === "question_answer") {
      ...
      const newUrl = `${(msg.content as string || DEFAULT_API_BASE_URL).trim().replace(/\/+$/, "")}/`;
