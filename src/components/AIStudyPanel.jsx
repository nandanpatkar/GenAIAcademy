...
import {
  ...
  CheckSquare, Library, Network, AlignLeft,
  Sparkles, Bookmark, AlertCircle,
  BookmarkCheck, Trash2, FolderOpen, Save, Clock,
  X, RotateCcw,
} from "lucide-react";
...
export function buildNotebookLMUrl(module) {
  const sources = [];
...
export function countSources(module) {
...
export function AIStudyPanel({ module, pathColor }) {
...
/** The "Deep Learning Suite" block: NotebookLM launch banner + the AI Study Panel. */
export function DeepLearningSuite({ module, pathColor }) {
  const { total, } = countSources(module);
  const hasNoSources = total === 0;
  const notebookUrl = buildNotebookLMUrl(module);

  const openNotebookLM = () => {
    window.open(hasNoSources ? "https://notebooklm.google.com/" : notebookUrl, "_blank");
  };
