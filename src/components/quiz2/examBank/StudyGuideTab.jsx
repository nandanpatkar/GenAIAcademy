...
export default function StudyGuideTab({ exam }) {
  ...
  useEffect(() => {
    ...
    safeFetchJson(`/api/exam2?resource=studyguide&exam=${encodeURIComponent(exam.slug)}`)
      .then((data) => {
        if (cancelled) return;
  ...
  const selectTopic = async (path) => {
    ...
    setArticleError("");
    try {
      const data = await safeFetchJson(`/api/exam2?resource=studyguide-content&exam=${encodeURIComponent(exam.slug)}&path=${encodeURIComponent(path)}`);
