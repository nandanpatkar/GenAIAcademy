...
export default function ResourcePanel({ module, pathColor, onClose, onEditModule, isEditMode }) {
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const newFile = { name: file.name, size: (file.size / 1024 / 1024).toFixed(2) + " MB", type: ext, url: data.url };
            ...
            {isEditMode && (
              <label
                style={{
                  ...
                  fontSize: 11, cursor: "pointer", marginTop: 8, transition: "all .15s",
                }}
                onMouseEnter={(e) => { e.currentTarget.style.borderColor = pathColor; e.currentTarget.style.color = pathColor; }}
                onMouseLeave={(e) => { e.currentTarget.style.borderColor = "var(--border2)"; e.currentTarget.style.color = "var(--text3)"; }}
              >
                {uploading ? "Uploading..." : "+ Attach file (PDF, DOC, PNG...)"}
