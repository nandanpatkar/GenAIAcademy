/*
 * Developer Module 1 — MSO Foundations.
 * Content transcribed from "MODULE 1_DEV.txt".
 * Block vocabulary is documented in src/data/courses.js.
 */
...
export const module1 = {
  ...
  shortTitle: 'MSO Foundations',
  summary:
    'Tokens, context, sampling, model tiers, prompting modes, and how a developer reaches the API.',
  duration: '~50 min',
  lede: 'Before you write a line of code against Claude, it helps to know what the words mean. This module introduces the model fundamentals and the technical foundations that the rest of the Developer course assumes you already have.',
  objectives: [
  ...
  sections: [
      ...
      blocks: [
        {
          type: 'p',
          variant: 'lede',
          text: 'Before you write a line of code against Claude, it helps to know what the words mean.',
        },
        {
          type: 'p',
          text: 'This module introduces the model fundamentals and the technical foundations that the rest of the Developer course assumes you already have.',
        },
        {
          type: 'table',
          ...
          rows: [
            ...
            ['How LLMs Behave', '1 screen'],
            ['Models & Reasoning', '1 screen'],
            ['Prompting Modes', '1 screen'],
            ['Technical Substrate', '1 screen'],
            ['Module Wrap-up', '4 screens'],
          ],
        },
        disclaimer,
      ],
          ...
          items: [
            {
              eyebrow: 'Tier',
              title: 'Haiku',
              body: 'Built for speed and cost efficiency on tasks that fit its capability envelope.',
            },
            {
              eyebrow: 'Tier',
              title: 'Sonnet',
              body: 'The balanced default for most production workloads, and the recommended starting point.',
            },
            {
              eyebrow: 'Tier',
              title: 'Opus',
              body: 'Handles demanding work above the Sonnet envelope.',
            },
            {
              eyebrow: 'Tier',
              title: 'Fable',
              body: 'The most capable tier, built for the most demanding reasoning, coding, and agentic work where maximum intelligence is the priority.',
            },
          ],
        },
        {
    ...
    {
      id: 'dev-m1-substrate',
      eyebrow: 'Teaching · Technical Substrate',
      duration: '12 min',
    ...
    {
      id: 'dev-m1-exercise',
      eyebrow: 'Exercise · Predict the Behavior',
      duration: '6 min',
      title: 'Exercise: predict the behavior',
      blocks: [
        {
          type: 'p',
          text: "Try it now. Each scenario below presents a configuration drawn from one of this module's four foundations, sampling, prompting mode, request shape, and the context budget. For each one, select the answer that predicts the correct behavior and identifies the reason why. Partial credit is available when you answer three of four correctly.",
        },
        { type: 'h', level: 3, text: 'Scenario 1 · Sampling' },
        {
          type: 'p',
          text: 'Consider a classification task run at temperature 0 versus the same task run at a high temperature. Predict how the outputs differ across repeated runs.',
        },
        {
          type: 'callout',
          variant: 'key',
          title: 'Model answer',
          body: [
            'At a low temperature, the model concentrates probability on the most likely tokens, so repeated runs return the same label far more consistently, though never with guaranteed determinism, even at temperature 0. At a high temperature, the distribution spreads out, so wording and even the chosen label can vary. For a classifier you want the low-temperature, repeatable behavior.',
            'Temperature shapes the probability distribution the model samples from, it does not fix or disable the output. On the newest models, sampling parameters are omitted entirely. Non-default values return an error, and repeatability is managed through prompt design.',
          ],
        },
        { type: 'h', level: 3, text: 'Scenario 2 · Prompting mode' },
        {
          type: 'p',
          text: 'Consider a task that keeps returning output in the wrong structure under a zero-shot prompt. Predict what changes if you switch to multi-shot.',
        },
        {
          type: 'callout',
          variant: 'key',
          title: 'Model answer',
          body: [
            'Adding two or three correct input-output examples shows the model the exact structure to match, which usually fixes a structure problem that more instruction text did not. The cost is extra tokens on every call, so add the fewest examples that make the output reliable.',
            'Examples in a prompt are not training and do not lower cost. They sit in the prompt, demonstrate the exact output shape, and add tokens on every call.',
          ],
        },
        { type: 'h', level: 3, text: 'Scenario 3 · Request shape' },
        {
          type: 'p',
          text: 'Consider a pipeline that must process 50,000 documents overnight with no user waiting. Predict which request shape fits and why.',
        },
        {
          type: 'callout',
          variant: 'key',
          title: 'Model answer',
          body: [
            'The batch pattern fits: submit the requests in a batch and poll for completion, accepting longer latency for a lower per-token cost. A synchronous loop would hit rate limits and tie up the application, and streaming buys nothing because no user is watching.',
            'Match the request shape to whether a user is waiting. Streaming only helps when someone is watching the output arrive.',
          ],
        },
        { type: 'h', level: 3, text: 'Scenario 4 · The context budget' },
        {
          type: 'p',
          text: 'Consider a long multi-turn agent session whose context window keeps filling. Predict the symptoms and name the budget at fault.',
        },
        {
          type: 'callout',
          variant: 'key',
      ...
      blocks: [
        {
          ...
          items: [
            {
              title: 'Tokens are the unit of input, output, and cost.',
              text: 'Think and budget in tokens rather than words, since that is what the API meters and the context window measures.',
            },
            {
              title:
                'The context window is a fixed token budget that holds the whole request at once.',
              text: "An oversized input errors before generation, while hitting the ceiling mid-generation returns truncated output with a `model_context_window_exceeded` stop reason, so managing history is the application's job.",
            },
            {
              title: 'Sampling makes generation non-deterministic.',
              text: 'The same prompt can return different wording on each run, so testing on exact text is unreliable. This is what evals are built for.',
            },
            {
              title: 'Model choice and reasoning mode are separate, composable levers.',
              text: 'Pick the smallest model and the simplest reasoning and prompting that meet your eval and add capability only where the eval says you need it.',
            },
            {
              title: 'A developer reaches Claude over a REST API, usually through an SDK.',
              text: 'Choose between synchronous, streaming, async/await, or batch based on whether a user is waiting and whether the workload is real-time or bulk offline.',
            },
          ],
        },
        {
          type: 'p',
          text: '**What comes next:** Module 2 puts these foundations to work across prompting craft, tool schemas, streaming, context engineering, and agent construction.',
        },
      ],
    },

    {
      ...
      blocks: [
        {
          type: 'p',
          text: 'Claude 101 (Skilljar), Building with the Claude API (Skilljar), AI Fluency: Framework & Foundations (Skilljar), platform.claude.com/docs. Verify product specifics at publish time.',
        },
        {
          type: 'callout',
          variant: 'key',
          title: 'You can now speak the shared vocabulary of the Developer course.',
          body: [
  ...
  /* The module's own 4-question checkpoint quiz. */
  reviewQuestions: [
    {
      id: 'dev-m1-rq-1',
      question:
      ...
      options: [
        { id: 'A', text: 'That is true, the model is deterministic.' },
        {
          id: 'B',
          text: 'Not necessarily, the model samples each next token from a probability distribution, so wording can vary even when both answers are correct.',
        },
        { id: 'C', text: 'That is only true if streaming is off.' },
        { id: 'D', text: 'That is only true on the largest model.' },
      ],
      correctOptionId: 'B',
      rationale:
        'Sampling makes generation non-deterministic, which is why tests assert on properties and meaning rather than exact text.',
    },
    {
      id: 'dev-m1-rq-2',
      question: 'Which statement best separates model choice from reasoning mode?',
      options: [
        { id: 'A', text: 'They are the same setting.' },
        { id: 'B', text: 'Extended thinking is a different model.' },
        {
          id: 'C',
          text: 'Model choice picks which member of the family runs; extended thinking is a per-call setting that any supporting model can run with on or off.',
        },
        { id: 'D', text: 'Reasoning mode is fixed per account.' },
      ],
      correctOptionId: 'C',
      rationale: 'The two levers are independent but can work together.',
    },
    {
      id: 'dev-m1-rq-3',
      question:
      ...
      options: [
        { id: 'A', text: 'Improves accuracy substantially.' },
        { id: 'B', text: 'Adds token cost on every call for little or no gain.' },
        { id: 'C', text: 'Changes the model being used.' },
        { id: 'D', text: 'Disables sampling.' },
      ],
      correctOptionId: 'B',
      rationale:
        'Add examples when the output shape is wrong, not when zero-shot already works.',
    },
    {
      id: 'dev-m1-rq-4',
      question:
      ...
      options: [
        { id: 'A', text: 'Synchronous calls in a loop.' },
        { id: 'B', text: 'Streaming.' },
        { id: 'C', text: 'Batch submission with polling.' },
        { id: 'D', text: 'A larger context window.' },
      ],
      correctOptionId: 'C',
      rationale:
        'Batch trades latency for lower per-token cost and avoids the rate limits a synchronous loop hits.',
    },
  ],
};
