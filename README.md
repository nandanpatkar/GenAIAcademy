...
build:reference → build:manual-path → build:interview-index → build:codelab
  → validate:codelab → build:aws-simulator → vite build
```
...
5. **`validate:codelab`** — sanity-checks the generated manifest JSON.
6. **`build:aws-simulator`** — Angular production build of `system-design-simulator/frontend`, copied into `public/aws-simulator/`.
7. **`vite build`** — the actual SPA build (Rollup, manual chunk splitting for `react-vendor`/`supabase`/`motion`, `modulePreload: false`).
