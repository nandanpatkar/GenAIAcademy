# Self-Signed Certificate: ALB Solution for AgentCore Gateway

This lab demonstrates how to connect [Amazon Bedrock AgentCore Gateway](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/gateway.html) to an API that uses a **self-signed TLS certificate**.
...
**Simplest fix:** If you can, replace the self-signed certificate with a public ACM certificate for a domain you own. This avoids adding any new infrastructure.

**If that's not possible** (e.g., you don't own the domain), deploy an **internal ALB with a public certificate** in front of your API:
...
3. Point the new ALB at your API
4. Use `routingDomain` in the AgentCore Gateway target to route through the new ALB's publicly resolvable DNS

The new ALB's domain does **not** need to match your API's domain — it terminates TLS with the public cert, then forwards to your API over HTTPS. Your existing setup remains untouched for internal consumers.
...
## Step 2: Deploy a sample backend with a self-signed cert

For this lab, we deploy a sample API that uses a self-signed TLS certificate. This simulates any internal endpoint with a self-signed certificate — the same solution applies regardless of what your API runs on.

**SelfSignedBackend** deploys:
   - An **EC2 instance** running a REST API (FastAPI) on HTTPS port 443, using a self-signed certificate generated via `openssl` in user data
...
The **private hosted zone** resolves `api.internal.<baseDomain>` to the EC2's private IP within the VPC, matching the certificate's CN.

The **NLB** provides a publicly resolvable DNS name (`internal-xxx.elb.amazonaws.com`) that VPC Lattice can use as a `routingDomain`, since the private hosted zone domain is not publicly resolvable. The NLB does TCP passthrough — it does not terminate TLS, so the EC2's self-signed certificate is presented directly to the client.

> **Note:** If you have a certificate from AWS Private CA instead of a self-signed certificate, the same solution applies, see the [private certificate authority lab](./02-private-certificate-authority.ipynb).
...
```python
# Create API key credential provider (needed for both the optional fail test and the working target)
...
Our API uses a **self-signed certificate**. If we try to create an AgentCore Gateway target pointing to this endpoint, the target creation will **succeed** and the target will become `READY`. However, when you **invoke** the tool, the TLS handshake will fail because AgentCore Gateway validates certificates against **public root CAs only**.

You will see the following error on invocation:
...
To prove it, uncomment and run the cells below. They create a target, wait for it to become ready, invoke it to see the TLS error, and clean it up. This takes a few minutes. **Skip these cells if you want to go straight to the solution.**

```python
# # The EC2 instance serves HTTPS:443 with the self-signed cert.
# # The NLB provides a publicly resolvable DNS (routingDomain) so VPC Lattice
# # can route to it. But the self-signed cert will cause a TLS error.
...
| Option | When to use |
|--------|-------------|
| **Replace self-signed cert with public cert** on existing endpoint | Simplest fix — if you own the domain and can get a public ACM certificate for it |
| **Add a new ALB with public cert** in front (this lab) | When you can't modify the existing endpoint's cert or don't own the domain |

We'll proceed with Option 2: deploying a new internal ALB with a public certificate in front of your API.
...
4. The ALB **forwards the request over HTTPS** to the backend using the self-signed certificate
5. All traffic stays inside the VPC

> **Note:** The ALB does not validate the backend's self-signed certificate when forwarding over HTTPS. This is standard ALB behavior — backend certificate validation is not performed.
...
# The EC2 instance serves HTTPS using the self-signed certificate
...
5. The ALB **forwards the request over HTTPS** to the backend using the self-signed certificate
6. All traffic stays inside the VPC

Your API keeps its self-signed certificate as-is. Internal consumers continue accessing it directly. AgentCore Gateway uses the new ALB with the public certificate.
...
3. Destroy the CDK stacks (backend)
...
```python
# # Step 4: Delete the retained NLB security group
