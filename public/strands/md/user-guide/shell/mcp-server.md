...
To grant access, pass a [TOML config file](lc:user-guide/shell/configuration#toml-configuration) before the flag. The server applies the same binds, credentials, allowlist, and limits you’d set in code.
...
Reach for the MCP server when your agent already speaks MCP and you want isolation without writing binding code: the agent gets a shell plus file tools, all mediated by the Kernel, configured from one TOML file. When you’re writing the agent host yourself in Python or Node.js, embed the [Python](lc:user-guide/shell/quickstart#python) or [Node.js](lc:user-guide/shell/quickstart#nodejs) API directly instead, which gives you the typed `Output`, file operations, and per-session control without a subprocess.

## Next steps

-   [Configuration](lc:user-guide/shell/configuration): the full TOML schema for binds, credentials, the allowlist, and limits.
-   [Security Model](lc:user-guide/shell/security): what the sandbox guarantees per session and why one shell per session is the contract.
