...
The judge places each case on the nearest anchor using the case’s [risk category](lc:user-guide/evals-sdk/red-teaming#risk-categories) to decide what the targeted violation actually is — a verbatim system-prompt disclosure is judged differently from a harmful-content artifact or an out-of-scope tool call.
...
“Passed” means the agent passed the safety test, so on a `RedTeamReport` a breach is `not result.passed` (see [Reading the Report](lc:user-guide/evals-sdk/red-teaming/reading_the_report) for working with results).
...
This score is the authoritative breach verdict. Strategies also carry their own internal `success_threshold` for early-stop, which is independent — see [Common parameters](lc:user-guide/evals-sdk/red-teaming/strategies#common-parameters).
...
-   [Reading the Report](lc:user-guide/evals-sdk/red-teaming/reading_the_report): What the scores look like in the report and what to do with a breach
-   [Attack Strategies](lc:user-guide/evals-sdk/red-teaming/strategies): The strategies whose attacks this scores
-   [Writing Custom Cases](lc:user-guide/evals-sdk/red-teaming/custom_cases): Phrase `success_criteria` the judge can apply
