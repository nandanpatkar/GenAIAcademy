...
async def main():
    report = await experiment.run_evaluations_async(math_agent, max_workers=1)

    # The experiment already returns a single combined report across all evaluators —
    # each row in `report.cases` carries an `evaluator` key naming the producer.
    report.display(include_recommendations=True)
...
-   [Failure Detection](lc:user-guide/evals-sdk/detectors/failure_detection): Standalone failure detection
-   [Root Cause Analysis](lc:user-guide/evals-sdk/detectors/root_cause_analysis): Standalone root cause analysis
...
- [Root Cause Analysis](lc:user-guide/evals-sdk/detectors/root_cause_analysis) (3 shared tags)
...
- [Operating Agents in Production](lc:user-guide/deploy/operating-agents-in-production) (2 shared tags)
