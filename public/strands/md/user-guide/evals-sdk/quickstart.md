...
>
> The `@eval_task()` decorator eliminates boilerplate. Your function can return an `Agent` (auto-invoked with `case.input`), a `str`, or a `dict`. For trace-based evaluators, use `@eval_task(TracedHandler())` to automatically collect spans. See the [Task Decorator guide](lc:user-guide/evals-sdk/how-to/eval_task) for details.
...
Manual task function (without decorator)
...
Installing `strands-agents-evals` also installs a `strands-evals` console script — a thin wrapper over the same Python API for CI gates and one-off checks. It’s the fastest way to run an experiment without writing a runner:

```bash
# Schema-check, then run an experiment file against an agent factory
strands-evals validate basic_evaluation.json
strands-evals run basic_evaluation.json --agent my_agent_path:build_agent --display

# One-off check with no experiment file
strands-evals run \
  --input "What is the capital of France?" \
  --expected-output "Paris" \
  --agent my_agent_path:build_agent
```

`--agent MODULE:ATTR` points at the same factory you’d pass to `experiment.run_evaluations` in Python. See the [CLI reference](lc:user-guide/evals-sdk/cli) for `report`, `diagnose`, `generate`, CI exit codes, and the full flag set.
...
async def run_async_evaluation():
    experiment = Experiment[str, str](cases=test_cases, evaluators=[evaluator])
    report = await experiment.run_evaluations_async(get_response_async)
...
`EvaluationReport` exposes parallel lists indexed by case position. Each entry in `cases` is a dict carrying the case input/expected output and an `evaluator` key naming the evaluator that produced that row.

```python
# Access individual case results
for i, case in enumerate(report.cases):
    print(f"Case: {case.get('name')}")
    print(f"Evaluator: {case.get('evaluator')}")
    print(f"Score: {report.scores[i]}")
    print(f"Passed: {report.test_passes[i]}")
...
-   [Command-Line Interface](lc:user-guide/evals-sdk/cli) - Run experiments and diagnose sessions from the shell or CI
-   [Task Decorator](lc:user-guide/evals-sdk/how-to/eval_task) - Simplify task functions with `@eval_task`
-   [Output Evaluator](lc:user-guide/evals-sdk/evaluators/output_evaluator) - LLM-based output evaluation with custom rubrics
-   [Trajectory Evaluator](lc:user-guide/evals-sdk/evaluators/trajectory_evaluator) - Tool usage and sequence evaluation
-   [Helpfulness Evaluator](lc:user-guide/evals-sdk/evaluators/helpfulness_evaluator) - Seven-level helpfulness assessment
-   [Correctness Evaluator](lc:user-guide/evals-sdk/evaluators/correctness_evaluator) - Factual correctness with optional reference comparison
-   [Deterministic Evaluators](lc:user-guide/evals-sdk/evaluators/deterministic_evaluators) - Fast, code-based checks (Equals, Contains, ToolCalled)
-   [Custom Evaluators](lc:user-guide/evals-sdk/evaluators/custom_evaluator) - Build domain-specific evaluation logic
-   [Tool Simulation](lc:user-guide/evals-sdk/simulators/tool_simulation) - Test tool-using agents without real infrastructure
-   [Result Caching](lc:user-guide/evals-sdk/how-to/result_caching) - Cache task results to iterate on evaluators without re-running agents
-   [Remote Trace Providers](lc:user-guide/evals-sdk/how-to/trace_providers) - Evaluate traces from production backends
-   [Experiment Generator](lc:user-guide/evals-sdk/experiment_generator) - Automatically generate comprehensive test suites
-   [Serialization](lc:user-guide/evals-sdk/how-to/serialization) - Save, load, and version your evaluation experiments
-   [Detectors](lc:user-guide/evals-sdk/detectors) - Automatic failure detection and root cause analysis for agent traces

## Related pages

- [Get started](lc:user-guide/quickstart/overview) (1 shared tag)
- [Python Quickstart](lc:user-guide/quickstart/python) (1 shared tag)
