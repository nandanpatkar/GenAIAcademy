Steering provides modular prompting for complex agent tasks through context-aware guidance that appears when relevant, rather than front-loading all instructions in monolithic prompts. Steering handlers observe agent activity and intervene at key moments — before a tool call or after a model response — with targeted feedback. They are built on the [interventions](lc:user-guide/concepts/agents/interventions) framework and use the same typed action model.

> [!NOTE] Python SDK
>
> In Python, steering handlers use the [plugins](lc:user-guide/concepts/plugins/steering) interface rather than interventions. See [Steering (Plugins)](lc:user-guide/concepts/plugins/steering) for the Python API reference including `LLMSteeringHandler` and context providers.
...
-   [Interventions](lc:user-guide/concepts/agents/interventions) — The typed action model, lifecycle methods, evaluation order, and error handling
-   [Hooks](lc:user-guide/concepts/agents/hooks) — Low-level event callbacks that context providers subscribe to
-   [Steering (Python)](lc:user-guide/concepts/plugins/steering) — Python-specific steering using the plugins interface
