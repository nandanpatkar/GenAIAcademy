...
>
> TypeScript does not support callback handlers. For real-time event handling in TypeScript, use the [async iterator pattern](lc:user-guide/concepts/streaming/async-iterators) with `agent.stream()` or see [Hooks](lc:user-guide/concepts/agents/hooks) for lifecycle event handling.
...
> **Note:** For asynchronous applications, consider [async iterators](lc:user-guide/concepts/streaming/async-iterators) instead.
