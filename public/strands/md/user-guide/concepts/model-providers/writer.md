...
| `max_tokens` | `Optional[int]` | Maximum number of tokens to generate | See the Context Window for [each available model](#available-models) | \[reference\](https://strandsagents.com/docs/[https://dev.writer.com/api-reference/completion-api/chat-completion/#body-max-](https://dev.writer.com/api-reference/completion-api/chat-completion#body-max-) tokens) |
...
> **Note**: The `web_search` and `email_sender` tools in this example are custom tools that you would need to define. See [Python Tools](lc:user-guide/concepts/tools/custom-tools) for guidance on creating custom tools, or use existing tools from the [strands\_tools package](lc:user-guide/concepts/tools/community-tools-package).
...
> **Note**: Structured output disables streaming and returns the complete response at once, unlike regular chat completions, which stream by default. See [Callback Handlers](lc:user-guide/concepts/streaming/callback-handlers) for more details.
...
-   [Python API Reference](lc:api/python/strands.models.model)
...
- [OpenAI](lc:user-guide/concepts/model-providers/openai) (2 shared tags)
...
- [harness-sdk/strands-py/src/strands/models/writer.py](https://github.com/strands-agents/harness-sdk/blob/main/strands-py/src/strands/models/writer.py)
