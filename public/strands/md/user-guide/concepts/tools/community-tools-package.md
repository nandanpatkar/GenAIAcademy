...
>
> The Community Tools Package (`strands-agents-tools`) is currently Python-only. TypeScript users should use [vended tools](lc:user-guide/concepts/tools/vended-tools) included in the TypeScript SDK or create custom tools using the `tool()` function.
