...
Bidirectional streaming uses a different event model than [standard streaming](lc:user-guide/concepts/streaming):
...
See [Connection Lifecycle](lc:user-guide/concepts/bidirectional-streaming/agent#connection-restart) for more details on timeout handling.
...
Emitted when the model requests tool execution. See [Tools Overview](lc:user-guide/concepts/tools) for details.
...
For details on hook events and usage patterns, see the [Hooks](lc:user-guide/concepts/bidirectional-streaming/hooks) documentation.
