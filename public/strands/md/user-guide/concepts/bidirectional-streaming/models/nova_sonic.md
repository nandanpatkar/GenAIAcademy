...
Nova Sonic accepts text input at any point during an active voice session, without interrupting or waiting on the audio stream. Send a [`BidiTextInputEvent`](lc:user-guide/concepts/bidirectional-streaming/events#biditextinputevent) the same way you would outside of a live conversation:
...
| --- | --- | --- | --- |
| `audio` | `AudioConfig` instance. | `{"voice": "tiffany", "output_rate": 24000}` | [reference](lc:api/python/strands.experimental.bidi.types.model#AudioConfig); see [supported voices](https://docs.aws.amazon.com/nova/latest/nova2-userguide/sonic-language-support.html) for valid `voice` values |
...
-   [Python API Reference](lc:api/python/strands.experimental.bidi.models.nova_sonic#BidiNovaSonicModel)
