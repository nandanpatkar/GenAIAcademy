...
| --- | --- | --- | --- |
| `audio` | `AudioConfig` instance. | `{"voice": "Kore"}` | [reference](lc:api/python/strands.experimental.bidi.types.model#AudioConfig) |
...
Currently, `BidiGeminiLiveModel` does not produce a message history and so has limited compatability with the Strands [session manager](lc:user-guide/concepts/bidirectional-streaming/session-management). However, the provider does utilize Gemini’s [Session Resumption](https://ai.google.dev/gemini-api/docs/live-session) as part of the [connection restart](lc:user-guide/concepts/bidirectional-streaming/agent#connection-restart) workflow. This allows Gemini Live connections to persist up to 24 hours. After this time limit, a new `BidiGeminiLiveModel` instance must be created to continue conversations.
...
-   [Python API Reference](lc:api/python/strands.experimental.bidi.models.gemini_live#BidiGeminiLiveModel)
