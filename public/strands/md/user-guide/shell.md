...
The [Configuration](lc:user-guide/shell/configuration) guide covers each of these in depth, including the TOML format that lets you declare the whole policy in a file.
...
-   [Quickstart](lc:user-guide/shell/quickstart): install the shell and run your first sandboxed command through MCP, Python, or Node.js.
-   [Configuration](lc:user-guide/shell/configuration): bind directories, inject credentials, set the network allowlist, and load it all from TOML.
-   [Commands](lc:user-guide/shell/commands): the full command inventory, supported flags, and known gaps versus GNU coreutils.
-   [MCP Server](lc:user-guide/shell/mcp-server): expose the shell to any MCP-compatible agent framework over stdio.
-   [Security Model](lc:user-guide/shell/security): the Kernel boundary, the SSRF guard, credential handling, and how to layer OS isolation for adversarial workloads.
