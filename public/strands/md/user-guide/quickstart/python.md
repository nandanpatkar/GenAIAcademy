...
More details can be found in the [Agent Loop](lc:user-guide/concepts/agents/agent-loop) documentation.
...
After running an agent, you can understand what happened during execution through traces and metrics. Every agent invocation returns an [`AgentResult`](lc:api/python/strands.agent.agent_result#AgentResult) object with comprehensive observability data.

Traces provide detailed insight into the agent’s reasoning process. You can access in-memory traces and metrics directly from the [`AgentResult`](lc:api/python/strands.agent.agent_result#AgentResult), or export them using [OpenTelemetry](lc:user-guide/observability-evaluation/traces) to observability platforms.
...
This observability data helps you debug agent behavior, optimize performance, and understand the agent’s reasoning process. For detailed information, see [Observability](lc:user-guide/observability-evaluation/observability), [Traces](lc:user-guide/observability-evaluation/traces), and [Metrics](lc:user-guide/observability-evaluation/metrics).
...
Learn more in the [Callback Handlers](lc:user-guide/concepts/streaming/callback-handlers) documentation.
...
See the [Logs documentation](lc:user-guide/observability-evaluation/logs) for more information.

## Model Providers

### Identifying a configured model

Strands defaults to the Bedrock model provider using Claude Sonnet 4. The model your agent is using can be retrieved by accessing [`model.config`](lc:api/python/strands.models.model#Model.get_config):
...
More details in the [Amazon Bedrock Model Provider](lc:user-guide/concepts/model-providers/amazon-bedrock) documentation.

### Additional Model Providers

Strands Agents supports several other model providers beyond Amazon Bedrock:

-   **[Anthropic](lc:user-guide/concepts/model-providers/anthropic)** - Direct API access to Claude models
-   **[LiteLLM](lc:user-guide/concepts/model-providers/litellm)** - Unified interface for OpenAI, Mistral, and other providers
-   **[Llama API](lc:user-guide/concepts/model-providers/llamaapi)** - Access to Meta’s Llama models
-   **[Mistral](lc:user-guide/concepts/model-providers/mistral)** - Access to Mistral models
-   **[Ollama](lc:user-guide/concepts/model-providers/ollama)** - Run models locally for privacy or offline use
-   **[OpenAI](lc:user-guide/concepts/model-providers/openai)** - Access to OpenAI or OpenAI-compatible models
-   **[Writer](lc:user-guide/concepts/model-providers/writer)** - Access to Palmyra models
-   **[Cohere community](lc:community/model-providers/cohere)** - Use Cohere models through an OpenAI compatible interface
-   **[CLOVA Studio community](lc:community/model-providers/clova-studio)** - Korean-optimized AI models from Naver Cloud Platform
-   **[FireworksAI community](lc:community/model-providers/fireworksai)** - Use FireworksAI models through an OpenAI compatible interface
-   **[Custom Providers](lc:user-guide/concepts/model-providers/custom_model_provider)** - Build your own provider for specialized needs
...
For asynchronous applications (like web servers or APIs), Strands provides an async iterator approach using [`stream_async()`](lc:api/python/strands.agent.agent#Agent.stream_async). This is particularly useful with async frameworks like FastAPI or Django Channels.
...
See the [Async Iterators](lc:user-guide/concepts/streaming/async-iterators) documentation for full details.

> Note, Strands also offers an [`invoke_async()`](lc:api/python/strands.agent.agent#Agent.invoke_async) method for non-iterative async invocations.

### Callback Handlers (Callbacks)

We can create a custom callback function (named a [callback handler](lc:user-guide/concepts/streaming/callback-handlers)) that is invoked at various points throughout an agent’s lifecycle.
...
See the [Callback Handlers](lc:user-guide/concepts/streaming/callback-handlers) documentation for full details.

## Next Steps

Ready to learn more? Check out these resources:

-   [Examples](lc:examples) - Examples for many use cases, multi-agent systems, autonomous agents, and more
-   [Community Supported Tools](lc:user-guide/concepts/tools/community-tools-package) - The `strands-agents-tools` package provides many powerful example tools for your agents to use during development
-   [Agent Loop](lc:user-guide/concepts/agents/agent-loop) - Learn how Strands agents work under the hood
-   [State & Sessions](lc:user-guide/concepts/agents/state) - Understand how agents maintain context and state across a conversation or workflow
-   [Multi-agent](lc:user-guide/concepts/multi-agent/agents-as-tools) - Orchestrate multiple agents together as one system, with each agent completing specialized tasks
-   [Observability & Evaluation](lc:user-guide/observability-evaluation/observability) - Understand how agents make decisions and improve them with data
-   [Operating Agents in Production](lc:user-guide/deploy/operating-agents-in-production) - Taking agents from development to production, operating them responsibly at scale

## Related pages

- [Get started](lc:user-guide/quickstart/overview) (1 shared tag)
- [Strands Evaluation Quickstart](lc:user-guide/evals-sdk/quickstart) (1 shared tag)
- [Strands Shell Quickstart](lc:user-guide/shell/quickstart) (1 shared tag)
- [TypeScript Quickstart](https://strandsagents.com/docs/user-guide/quickstart/typescript/) (1 shared tag)
- [Red Teaming Quickstart](lc:user-guide/evals-sdk/red-teaming/quickstart) (1 shared tag)
- [Voice & Realtime Quickstart](lc:user-guide/concepts/bidirectional-streaming/quickstart) (1 shared tag)
