...
1. [**01_hooks_basics.ipynb**](https://github.com/strands-agents/samples/tree/main/python/01-learn/16-hooks-lifecycle/01_hooks_basics.ipynb) - Register callbacks for `BeforeModelCallEvent` and `AfterModelCallEvent` with `agent.add_hook()` to measure model latency, use `AgentInitializedEvent` for one-time setup, and trace the full event lifecycle across model and tool calls.
2. [**02_hooks_intercept.ipynb**](https://github.com/strands-agents/samples/tree/main/python/01-learn/16-hooks-lifecycle/02_hooks_intercept.ipynb) - Use `BeforeToolCallEvent.cancel_tool` to block a tool, `AfterToolCallEvent.retry` to re-run a failed call, `AfterInvocationEvent.resume` to auto-continue the agent, and `BeforeInvocationEvent.messages` to redact input before the model sees it.
3. [**03_hooks_packaging.ipynb**](https://github.com/strands-agents/samples/tree/main/python/01-learn/16-hooks-lifecycle/03_hooks_packaging.ipynb) - Pass per-invocation context between hooks via `invocation_state`, package hooks into a reusable `HookProvider` class, and combine everything into a single observability and control plugin.
...
| File                                                       | Description                                                              |
|------------------------------------------------------------|--------------------------------------------------------------------------|
| [01_hooks_basics.ipynb](https://github.com/strands-agents/samples/tree/main/python/01-learn/16-hooks-lifecycle/01_hooks_basics.ipynb)           | Your first hook and the full event lifecycle.                            |
| [02_hooks_intercept.ipynb](https://github.com/strands-agents/samples/tree/main/python/01-learn/16-hooks-lifecycle/02_hooks_intercept.ipynb)     | Writable fields: cancel, retry, resume, and redaction.                   |
| [03_hooks_packaging.ipynb](https://github.com/strands-agents/samples/tree/main/python/01-learn/16-hooks-lifecycle/03_hooks_packaging.ipynb)     | Cross-hook data sharing and reusable `HookProvider` packaging.           |
...
1. Start with [01_hooks_basics.ipynb](https://github.com/strands-agents/samples/tree/main/python/01-learn/16-hooks-lifecycle/01_hooks_basics.ipynb) and work through the notebooks in order.
...
- See [`13-human-in-the-loop`](https://github.com/strands-agents/samples/tree/main/python/01-learn/13-human-in-the-loop) for how interrupts compose with hooks (`BeforeToolCallEvent` is `_Interruptible`).
- See [`08-observability`](https://github.com/strands-agents/samples/tree/main/python/01-learn/08-observability) for production-grade tracing built on the same hook surface.
