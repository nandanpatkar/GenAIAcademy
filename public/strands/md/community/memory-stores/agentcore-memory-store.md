The [AgentCore Memory Store](https://github.com/aws/bedrock-agentcore-sdk-typescript/tree/main/src/memory/integrations/strands) implements the Strands [`MemoryStore`](lc:user-guide/concepts/memory/overview#custom-stores) interface on top of Amazon Bedrock AgentCore Memory. Search maps to AgentCore’s `retrieveMemoryRecords`, and conversation turns are ingested for server-side memory extraction into long-term records. Attach it to an agent through a [`MemoryManager`](lc:user-guide/concepts/memory/overview) like any other store. See the [Getting Started](lc:user-guide/concepts/memory/overview#getting-started) section for more information.
