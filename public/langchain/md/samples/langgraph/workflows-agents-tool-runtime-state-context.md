...
>
> [Workflows + agents](lc:oss/python/langgraph/workflows-agents)

```python workflows-agents-tool-runtime-state-context.py
...
if __name__ == "__main__":
    expected = "User user_123 in organization org_456"
    contents = [str(getattr(message, "content", "")) for message in result["messages"]]
    assert any(expected in content for content in contents), contents
    print("✓ workflows-agents tool runtime state/context sample completed")
```
