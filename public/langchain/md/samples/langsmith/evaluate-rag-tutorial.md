...
```python evaluate-rag-tutorial.py
...
if not os.environ.get("OPENAI_API_KEY") or not os.environ.get("LANGSMITH_API_KEY"):
    print(
        "[evaluate-rag-tutorial] Skipping (OPENAI_API_KEY and LANGSMITH_API_KEY required)."
    )
    sys.exit(1)
...
if __name__ == "__main__":
    ...
    assert callable(rag_bot)
    assert callable(correctness)
    assert callable(relevance)
    assert callable(groundedness)
    assert callable(retrieval_relevance)
    assert experiment_results is not None
    print("✓ evaluate-rag-tutorial")
```
