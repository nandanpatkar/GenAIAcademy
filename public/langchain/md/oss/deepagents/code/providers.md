Deep Agents Code supports any [chat model provider compatible with LangChain](lc:oss/python/integrations/chat/index), unlocking use for virtually any LLM that supports tool calling. Any service that exposes an OpenAI-compatible or Anthropic-compatible API also works out of the box—see [Compatible APIs](lc:oss/deepagents/code/config-file#compatible-apis).
...
2. **Set credentials**
    ...
    Add an API key for your provider with the [`/auth`](lc:oss/deepagents/code/credentials#use-%2Fauth-recommended) credential manager:
    ...
    For non-interactive runs, CI/CD, or anywhere a TUI isn't available, store the same key from the shell with [`dcode auth set`](lc:oss/deepagents/code/credentials#manage-credentials-from-the-shell-dcode-auth) or set the provider's environment variable instead. See [Provider credentials](lc:oss/deepagents/code/credentials) for the full key resolution order, the [`DEEPAGENTS_CODE_` prefix](lc:oss/deepagents/code/configuration#deepagents_code_-prefix) for scoping a key to Deep Agents Code, and the [Provider reference](#provider-reference) for each provider's environment variable.

    To configure model parameters, see [Model parameters](#model-parameters).

## Provider reference

Using a provider not listed here? See [Arbitrary providers](lc:oss/deepagents/code/config-file#arbitrary-providers): any LangChain-compatible provider can be used in Deep Agents Code with additional setup.

| Provider | Package | Credential env var | Model profiles |
| --- | --- | --- | --- |
| OpenAI | [`langchain-openai`](lc:oss/python/integrations/chat/openai) | `OPENAI_API_KEY` | ✅ |
| OpenAI (Codex) | [`langchain-openai`](lc:oss/python/integrations/chat/openai) | None — [sign in with ChatGPT](#sign-in-with-chatgpt) | ✅ |
| Azure OpenAI | [`langchain-openai`](lc:oss/python/integrations/chat/azure_chat_openai) | `AZURE_OPENAI_API_KEY` | ✅ |
| Anthropic | [`langchain-anthropic`](lc:oss/python/integrations/chat/anthropic) | `ANTHROPIC_API_KEY` | ✅ |
| Google Gemini API | [`langchain-google-genai`](lc:oss/python/integrations/chat/google_generative_ai) | `GOOGLE_API_KEY` | ✅ |
| Google Vertex AI | [`langchain-google-genai`](lc:oss/python/integrations/chat/google_generative_ai#credentials) | `GOOGLE_CLOUD_PROJECT` | ✅ |
| Baseten | [`langchain-baseten`](https://github.com/basetenlabs/langchain-baseten) | `BASETEN_API_KEY` | ✅ |
| AWS Bedrock | [`langchain-aws`](lc:oss/python/integrations/chat/bedrock) | `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` | ✅ |
| AWS Bedrock Converse | [`langchain-aws`](lc:oss/python/integrations/chat/bedrock) | `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY` | ✅ |
| Hugging Face | [`langchain-huggingface`](lc:oss/python/integrations/chat/huggingface) | `HUGGINGFACEHUB_API_TOKEN` | ✅ |
| Ollama | [`langchain-ollama`](lc:oss/python/integrations/chat/ollama) | `OLLAMA_API_KEY` (cloud only; optional) | ❌ |
| Groq | [`langchain-groq`](lc:oss/python/integrations/chat/groq) | `GROQ_API_KEY` | ✅ |
| Cohere | [`langchain-cohere`](lc:oss/python/integrations/chat/cohere) | `COHERE_API_KEY` | ❌ |
| Fireworks | [`langchain-fireworks`](lc:oss/python/integrations/chat/fireworks) | `FIREWORKS_API_KEY` | ✅ |
| Together | [`langchain-together`](lc:oss/python/integrations/chat/together) | `TOGETHER_API_KEY` | ❌ |
| Meta | [`langchain-meta`](https://github.com/langchain-ai/langchain-meta) | `MODEL_API_KEY` | ✅ |
| Mistral AI | [`langchain-mistralai`](lc:oss/python/integrations/chat/mistralai) | `MISTRAL_API_KEY` | ✅ |
| DeepSeek | [`langchain-deepseek`](lc:oss/python/integrations/chat/deepseek) | `DEEPSEEK_API_KEY` | ✅ |
| IBM (watsonx.ai) | [`langchain-ibm`](lc:oss/python/integrations/chat/ibm_watsonx) | `WATSONX_APIKEY` | ❌ |
| Nvidia | [`langchain-nvidia-ai-endpoints`](lc:oss/python/integrations/chat/nvidia_ai_endpoints) | `NVIDIA_API_KEY` | ✅ |
| xAI | [`langchain-xai`](lc:oss/python/integrations/chat/xai) | `XAI_API_KEY` | ✅ |
| Perplexity | [`langchain-perplexity`](lc:oss/python/integrations/chat/perplexity) | `PERPLEXITY_API_KEY` (or `PPLX_API_KEY`) | ✅ |
| OpenRouter | [`langchain-openrouter`](lc:oss/python/integrations/chat/openrouter) | `OPENROUTER_API_KEY` | ✅ |
| LiteLLM | [`langchain-litellm`](lc:oss/python/integrations/chat/litellm) | Per-provider (see [docs](https://docs.litellm.ai/)) | ❌ |


> [!TIP]
>
> You can scope any credential to Deep Agents Code by adding a `DEEPAGENTS_CODE_` prefix. For example, `DEEPAGENTS_CODE_OPENAI_API_KEY` takes priority over `OPENAI_API_KEY` within Deep Agents Code without affecting other tools. See [`DEEPAGENTS_CODE_` prefix](lc:oss/deepagents/code/configuration#deepagents_code_-prefix) for details.


> [!TIP]
>
> [Model profiles](lc:oss/python/langchain/models#model-profiles) provide model metadata used by the interactive `/model` switcher. If a model is missing from the switcher, pass the model name directly or add it via `config.toml`.
...
The `openai_codex` provider lets you use OpenAI's Codex models with your paid **ChatGPT** subscription instead of an `OPENAI_API_KEY`. You sign in with your ChatGPT account, and it shows up as its own provider in both `/auth` and the `/model` switcher, separate from the API-key-based `openai` provider.
    ...
    #### Step: Authorize in your browser
...
| --- | --- | --- |
| OpenRouter | [`langchain-openrouter`](lc:oss/python/integrations/chat/openrouter) | `openrouter:<model>` (built-in, see [Provider reference](#provider-reference)) |
| LiteLLM | [`langchain-litellm`](lc:oss/python/integrations/chat/litellm) | `litellm:<model>` (built-in, see [Provider reference](#provider-reference)) |
    ...
    [
     {
      "label": "In session",
      "lang": "txt",
      "code": "/install openrouter"
     },
     {
      "label": "Shell",
      "lang": "bash",
      "code": "dcode --install openrouter"
     }
    ]
    ```
...
3. Select the model:
        ...
        [
         ...
         },
         {
          "label": "Shell",
          "lang": "bash",
          "code": "dcode --model ollama:qwen3:4b"
         }
        ]
        ```
...
3. Select a model:
        ...
        [
         ...
         },
         {
          "label": "Shell",
          "lang": "bash",
          "code": "GROQ_API_KEY=\"your-api-key\" dcode --model groq:openai/gpt-oss-120b"
         }
        ]
        ```
...
* **Via config file:** Set `[models].default` in `~/.deepagents/config.toml` (see [Configuration](lc:oss/deepagents/code/configuration)).
* **From the shell:**
...
> [!NOTE]
>
> Don't put credentials (`api_key`) in `params`—use [`api_key_env`](lc:oss/deepagents/code/config-file#provider-configuration) to point at an environment variable instead.


To override fields on the model's runtime *profile* (`max_input_tokens`, `tool_calling`, capability flags)—distinct from constructor params—see [Profile overrides](lc:oss/deepagents/code/config-file#profile-overrides-advanced).

## Advanced configuration

For detailed configuration of provider params, profile overrides, custom base URLs, compatible APIs, arbitrary providers, and lifecycle hooks, see [Config file](lc:oss/deepagents/code/config-file) and [Hooks](lc:oss/deepagents/code/hooks).
