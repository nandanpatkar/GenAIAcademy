...
Auto is an experimental beta. To use it, set the opt-in flag and then choose Auto for your session.
    ...
    #### Step: Set the experimental opt-in
...
- The active model is not an independent security authority. [MCP read-only annotations](lc:oss/deepagents/code/mcp-tools#read-only-tool-annotations-in-auto-mode) are trusted as a deliberate beta tradeoff.
...
- [Remote sandboxes](lc:oss/deepagents/code/remote-sandboxes)
