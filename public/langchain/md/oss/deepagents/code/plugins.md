...
Use the same skill format as standalone Deep Agents Code skills. The installed plugin name becomes the skill namespace. For more information, see [Memory and skills](lc:oss/deepagents/code/memory-and-skills#skills).
...
For supported MCP transports and fields, see [MCP tools](lc:oss/deepagents/code/mcp-tools).

### Add hooks

Place a hook document at `hooks/hooks.json`, declare a relative `hooks` path, or define hooks inline in the plugin manifest. Hook commands receive the path variables above. See [Hooks](lc:oss/deepagents/code/hooks) for the configuration and event reference.
...
- [Command reference](lc:oss/deepagents/code/cli-reference)
