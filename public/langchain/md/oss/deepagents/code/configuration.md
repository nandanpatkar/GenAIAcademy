Deep Agents Code stores configuration under `~/.deepagents/` and in project-level dotfiles. For the full directory tree, session storage, and skill paths, see [Data locations](lc:oss/deepagents/code/configuration#data-locations).
...
**Provider API keys** use a separate order. See [Key resolution order](lc:oss/deepagents/code/credentials#key-resolution-order).
...
**Provider endpoints** (`base_url`) resolve with their matching API key. See [Endpoints, keys, and gateways](lc:oss/deepagents/code/config-file#endpoints-keys-and-gateways).
...
All four commands accept `--json` for machine-readable output. For the full list of management subcommands, see [CLI reference](lc:oss/deepagents/code/cli-reference).
...
For provider keys specifically, see [Provider credentials](lc:oss/deepagents/code/credentials).
...
By default, when Deep Agents Code loads skills it validates that a resolved skill file path stays inside one of the standard [skill directories](lc:oss/deepagents/code/configuration#skills). This prevents symlinks inside skill directories from reading arbitrary files outside those roots.
...
For user-defined themes, built-in overrides, and terminal-specific mappings, see the `[themes.*]` and `[ui.terminal_themes]` sections in [Config file](lc:oss/deepagents/code/config-file) or configure them directly in `config.toml`:
...
**`DEEPAGENTS_CODE_VERSION`** · `string`
...
**`DEEPAGENTS_CODE_PRERELEASE`** · `string`
...
**`DEEPAGENTS_CODE_PYTHON`** · `string`
Python version to use for the install.


**`DEEPAGENTS_CODE_SKIP_OPTIONAL`** · `string`
Set to `1` to skip optional tool checks.


**`DEEPAGENTS_CODE_VERBOSE`** · `string`
...
To route every user's model traffic through a managed gateway (provisioning a gateway key and base URL fleet-wide), see [Managed gateways](lc:oss/deepagents/code/config-file#managed-gateways).
...
Time budget in seconds for the [Auto mode](lc:oss/deepagents/code/approval-modes) classifier to review each batch of gated actions. Valid range: `1`–`300`. Out-of-range or non-integer values fall back to the default (`20`). Consider [selecting a faster classifier model](lc:oss/deepagents/code/config-file#default-and-recent-model) before raising this value. Overrides `[models].auto_classifier_timeout` in `config.toml`. See [Auto classifier timeout](lc:oss/deepagents/code/config-file#auto-classifier-timeout).
...
**`DEEPAGENTS_CODE_DISABLED_PROJECT_MCP_SERVERS`** · `string`
...
**`DEEPAGENTS_CODE_DANGEROUSLY_ENABLE_PROJECT_MCP_SERVERS`** · `string`
...
Override the LangSmith project name for Deep Agents Code's own agent traces. Shell commands still run with the user's original `LANGSMITH_PROJECT`, so app, test, or script traces can appear in a separate project. See [Trace with LangSmith](lc:oss/deepagents/code/quickstart#trace-with-langsmith).


**`DEEPAGENTS_CODE_LANGSMITH_REDACT`** · `string`
Toggle client-side secret redaction for Deep Agents Code's LangSmith agent-trace inputs and outputs. Accepts `1`, `true`, `yes`, or `on` to enable redaction and `0`, `false`, `no`, or `off` to disable it, case-insensitively. When redaction is enabled, tracing is disabled for that run if redaction cannot be configured. See [Configure LangSmith trace redaction](lc:oss/deepagents/code/config-file#redact-langsmith-trace-secrets).


**`DEEPAGENTS_CODE_LANGSMITH_REPLICA_PROJECTS`** · `string`
A second LangSmith project to *also* write agent traces to. When set and tracing is active, each agent run is dual-written to the primary project (from `DEEPAGENTS_CODE_LANGSMITH_PROJECT`, or `deepagents-code` by default) and this project. Off by default. See [Trace with LangSmith](lc:oss/deepagents/code/quickstart#trace-with-langsmith).
...
**`DEEPAGENTS_CODE_ONBOARDING`** · `string`
...
LangGraph graph step budget, which is the maximum number of node invocations the `dcode` agent graph may execute per turn. Valid range: `25`–`100000`. Out-of-range or non-integer values log a warning and fall back to the default (`2000`). Overridden by `--recursion-limit` at the CLI. See [Agent runtime limits](lc:oss/deepagents/code/config-file#agent-runtime-limits).
...
**`DEEPAGENTS_CODE_USER_ID`** · `string`
...
| **ChatGPT OAuth token** | `~/.deepagents/.state/chatgpt-auth.json` | R/W | Backs the [`openai_codex`](lc:oss/deepagents/code/providers) provider; created when you sign in with ChatGPT and refreshed automatically. Readable only by your user account. |
...
Each subagent is an `AGENTS.md` file with YAML frontmatter (`name`, `description`, optional `model`) and a markdown body for the system prompt. See [Use subagents in Deep Agents Code](lc:oss/deepagents/code/subagents) for the full format reference.
...
- [CLI reference](lc:oss/deepagents/code/cli-reference)
- [Hooks](lc:oss/deepagents/code/hooks)
- [Data locations](#data-locations)
- [MCP tools](lc:oss/deepagents/code/mcp-tools)
