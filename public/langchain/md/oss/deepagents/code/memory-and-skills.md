...
In practice, skills and memory sit on a spectrum. For more on when to use each, see [Skills, memory, and tools](lc:oss/python/deepagents/skills#skills-memory-and-tools).
...
> [!TIP]
>
> Building a custom agent with the SDK? See [Memory](lc:oss/python/deepagents/memory) for programmatic memory backends.
...
> [!TIP]
>
> To generate a repository wiki and wire pointers into `AGENTS.md` and `CLAUDE.md` for coding agents, see [OpenWiki](lc:oss/openwiki/overview).
...
Deep agent skills follow the [Agent Skills specification](https://agentskills.io/). For more on how skills work and how to write effective ones, see [Skills](lc:oss/python/deepagents/skills).
...
### Add a skill
    ...
    #### Step: Add optional resources

        Optionally add additional scripts or other resources to the `test-skill` folder. For more information, see [Usage](lc:oss/python/deepagents/skills#add-supporting-resources).
...
When duplicate skill names exist, later-precedence directories override earlier ones (see [App data](lc:oss/deepagents/code/configuration#skills)).
