Deep Agents Code uses the [sandbox as tool](lc:oss/python/deepagents/sandboxes#sandbox-as-tool-pattern) pattern: the `dcode` process (LLM loop, memory, tool dispatch) runs on your machine, but agent tool calls (`read_file`, `write_file`, `execute`, etc.) target the remote sandbox, not your local filesystem. To get files into the sandbox, use a [setup script](#setup-scripts) or the provider's file transfer APIs (see [Working with files](lc:oss/python/deepagents/sandboxes#working-with-files)).

For a deeper look at sandbox architecture, integration patterns, and security best practices, see [Sandboxes](lc:oss/python/deepagents/sandboxes).

    #### Step: Install provider dependency
                    ...
                    [
                     {
                      "label": "In session",
                      "lang": "txt",
                      "code": "/install agentcore"
                     },
                     {
                      "label": "Shell",
                      "lang": "bash",
                      "code": "dcode --install agentcore"
                     }
                    ]
                    ```
                    ...
                    [
                     {
                      "label": "In session",
                      "lang": "txt",
                      "code": "/install daytona"
                     },
                     {
                      "label": "Shell",
                      "lang": "bash",
                      "code": "dcode --install daytona"
                     }
                    ]
                    ```
                    ...
                    [
                     {
                      "label": "In session",
                      "lang": "txt",
                      "code": "/install runloop"
                     },
                     {
                      "label": "Shell",
                      "lang": "bash",
                      "code": "dcode --install runloop"
                     }
                    ]
                    ```
                    ...
                    [
                     {
                      "label": "In session",
                      "lang": "txt",
                      "code": "/install langchain-e2b --package"
                     },
                     {
                      "label": "Shell",
                      "lang": "bash",
                      "code": "dcode --install langchain-e2b --package"
                     }
                    ]
                    ```
    ...
    #### Step: Run Deep Agents Code with a sandbox
...
For example, the `langchain-e2b` package publishes an `e2b` provider (see [sandbox integrations](lc:oss/python/integrations/sandboxes/index)). Install it as a package, set your credentials, then select it:
...
For an in-house or local provider you don't want to package, declare it under `[sandboxes.providers]` in `~/.deepagents/config.toml`. This parallels [arbitrary model providers](lc:oss/deepagents/code/config-file#arbitrary-providers) and uses the same `class_path` trust model.
...
> [!WARNING]
>
> Setting `class_path` causes Deep Agents Code to import and run arbitrary Python from the named module—module-level code executes on import. This is the same trust model as the model [`class_path`](lc:oss/deepagents/code/config-file#arbitrary-providers): you control your own machine and your own config file.
...
> [!WARNING]
>
> Sandboxes isolate code execution, but agents remain vulnerable to prompt injection with untrusted inputs. Use human-in-the-loop approval, short-lived secrets, and trusted setup scripts only. See [Security considerations](lc:oss/python/deepagents/sandboxes#security-considerations) for details.
