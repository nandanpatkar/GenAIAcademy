Define custom synchronous [subagents](lc:oss/python/deepagents/subagents) as markdown files so Deep Agents Code can delegate specialized tasks to them.
...
Project subagents override user subagents with the same name (see [precedence rules](lc:oss/deepagents/code/configuration#subagents)).

The frontmatter requires `name` and `description` (same as the [`SubAgent` dictionary spec](lc:oss/python/deepagents/subagents#subagent-dictionary-based)). The markdown body becomes the subagent's `system_prompt`. In addition to the base spec, `AGENTS.md` files support an optional `model` frontmatter field that overrides the main agent's model for this subagent. Use the `provider:model-name` format (e.g., `anthropic:claude-opus-4-8`, `openai:gpt-5.5`). Omit it to inherit the main agent's model.
...
`dcode` ships with the code interpreter enabled, so [dynamic subagents](lc:oss/python/deepagents/dynamic-subagents) work out of the box.
...
You can also use dynamic subagents in the coding agent of your choice over [ACP](lc:oss/python/deepagents/acp) (for example, Zed).
...
This overrides the built-in general-purpose subagent, routing all delegated tasks to a cheaper model. See [Override the general-purpose subagent](lc:oss/python/deepagents/subagents#override-the-general-purpose-subagent) for more.
