...
For `.env` loading order and the `DEEPAGENTS_CODE_` prefix, see [Configuration](lc:oss/deepagents/code/configuration#environment-variables).
...
The `/auth` prompt also has an optional **base URL** field. Leave it blank to use the provider's default endpoint, or set a custom one to use with this key. The base URL is saved alongside the key. See [Endpoints, keys, and gateways](lc:oss/deepagents/code/config-file#endpoints-keys-and-gateways) for how endpoints resolve, including with gateways.
...
Selecting the `openai_codex` provider in `/auth` starts a browser sign-in instead of prompting for an API key, letting you use OpenAI models with a ChatGPT subscription. To re-authenticate or sign out, select `openai_codex` again. See [Sign in with ChatGPT (Codex models)](lc:oss/deepagents/code/providers) for the full flow.

`/auth` manages LLM provider credentials, the Tavily web-search key, and LangSmith tracing. Enter a Tavily key to [activate web search](#enable-web-search-with-tavily) on the next launch. Enter a LangSmith key to enable tracing. Keys are also read from the environment. You can [set them in `~/.deepagents/.env` or your shell](lc:oss/deepagents/code/configuration#environment-variables).
...
To keep keys in a file instead, define them in a [`.env` file](lc:oss/deepagents/code/configuration#environment-variables).

## Key resolution order

When a provider's key is set in more than one place, Deep Agents Code uses the first of these that is set:

1. **`DEEPAGENTS_CODE_`-prefixed env var** — for example `DEEPAGENTS_CODE_OPENAI_API_KEY` as an inline shell export. The [`DEEPAGENTS_CODE_` prefix](lc:oss/deepagents/code/configuration#deepagents_code_-prefix) is the explicit "use this key in Deep Agents Code" override.
...
Each provider's API key and its endpoint (`base_url`) resolve as a pair from the same source. See [Endpoints, keys, and gateways](lc:oss/deepagents/code/config-file#endpoints-keys-and-gateways).
...
The built-in `web_search` tool uses [Tavily](https://tavily.com). Deep Agents Code shows a "Web search disabled" notification on startup until you provide a key. You can store the key in the [`/auth`](#use-%2Fauth-recommended) credential manager, where Tavily appears as a non-model service, or set the `TAVILY_API_KEY` environment variable.
    ...
    #### Tab: Set an environment variable
            ...
            #### Step: Add it to your environment
                ...
                Shell exports take precedence over `.env` values (see [Loading order and precedence](lc:oss/deepagents/code/configuration#loading-order-and-precedence)). To scope a key to Deep Agents Code only without affecting other tools that read `TAVILY_API_KEY`, use the [`DEEPAGENTS_CODE_` prefix](lc:oss/deepagents/code/configuration#deepagents_code_-prefix): `DEEPAGENTS_CODE_TAVILY_API_KEY=tvly-...`.
            

            #### Step: Reload or restart
