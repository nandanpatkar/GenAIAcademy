...
MCP servers can attach standard `ToolAnnotations` when advertising a tool. Deep Agents Code lets a tool bypass classifier review in [Auto approval mode](lc:oss/deepagents/code/approval-modes) only when all of the following are true:
...
To connect Deep Agents Code to LangSmith, use the [LangSmith Remote MCP](lc:langsmith/langsmith-remote-mcp):
...
- [LangSmith Remote MCP](lc:langsmith/langsmith-remote-mcp): connect Deep Agents Code to LangSmith tools over OAuth
- [LangChain MCP guide](lc:oss/python/langchain/mcp): protocol details, building custom servers, and using `langchain-mcp-adapters` programmatically
