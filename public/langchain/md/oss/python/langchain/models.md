...
Models are the reasoning engine of [agents](lc:oss/python/langchain/agents). They drive the agent's decision-making process, determining which tools to call, how to interpret results, and when to provide a final answer.
...
For provider-specific integration information and capabilities, see the provider's [chat model page](lc:oss/python/integrations/chat/index).


> [!TIP]
>
> [LangSmith](lc:langsmith/observability) traces each model call so you can compare providers, inspect tool routing, and debug failures. Follow the [tracing quickstart](lc:langsmith/trace-with-langchain) to get set up.
>
...
1. **With agents** - Models can be dynamically specified when creating an [agent](lc:oss/python/langchain/agents#model).
...
Browse the [full list of supported providers](lc:oss/python/integrations/providers/overview), or see [Providers and models](lc:oss/python/concepts/providers-and-models) for a conceptual overview of how providers, packages, and model names work together in LangChain.
...
> [!NOTE]
>
> In addition to chat models, LangChain provides support for other adjacent technologies, such as embedding models and vector stores. See the [integrations page](lc:oss/python/integrations/providers/overview) for details.
...
> [!TIP]
>
> For long-running agent graphs on unreliable networks, consider higher `max_retries` (for example 10–15) and a [checkpointer](lc:oss/python/langgraph/persistence) so that progress is preserved across failures.
...
>
>     To find all the parameters supported by a given chat model, head to the [chat model integrations](lc:oss/python/integrations/chat/index) page.
...
See the [messages](lc:oss/python/langchain/messages) guide for more detail on roles, types, and content.
...
[
 ...
 },
 {
  ...
  "lang": "python",
  "code": "from langchain.messages import HumanMessage, AIMessage, SystemMessage\n\nconversation = [\n    SystemMessage(\"You are a helpful assistant that translates English to French.\"),\n    HumanMessage(\"Translate: I love programming.\"),\n    AIMessage(\"J'adore la programmation.\"),\n    HumanMessage(\"Translate: I love building applications.\")\n]\n\nresponse = model.invoke(conversation)\nprint(response)  # AIMessage(\"J'adore cr\u00e9er des applications.\")"
 }
]
```
...
Calling `stream()`[BaseChatModel.stream] returns an iterator that yields output chunks as they are produced. You can use a loop to process each chunk in real-time:
    ...
    [
     {
      "label": "Basic text streaming",
      "lang": "python",
      "code": "for chunk in model.stream(\"Why do parrots have colorful feathers?\"):\n    print(chunk.text, end=\"|\", flush=True)"
     },
     {
      "label": "Stream tool calls, reasoning, and other content",
      "lang": "python",
      "code": "for chunk in model.stream(\"What color is the sky?\"):\n    for block in chunk.content_blocks:\n        if block[\"type\"] == \"reasoning\" and (reasoning := block.get(\"reasoning\")):\n            print(f\"Reasoning: {reasoning}\")\n        elif block[\"type\"] == \"tool_call_chunk\":\n            print(f\"Tool call chunk: {block}\")\n        elif block[\"type\"] == \"text\":\n            print(block[\"text\"])\n        else:\n            ..."
     }
    ]
    ```
...
> See the `astream_events()`[BaseChatModel.astream_events] reference for event types and other details.
        ...
        In [LangGraph agents](lc:oss/python/langchain/agents), for example, you can call `model.invoke()` within nodes, but LangChain will automatically delegate to streaming if running in a streaming mode.
...
Some model providers offer built-in tools that can be enabled via model or invocation parameters (e.g. [`ChatOpenAI`](lc:oss/python/integrations/chat/openai), [`ChatAnthropic`](lc:oss/python/integrations/chat/anthropic)). Check the respective [provider reference](lc:oss/python/integrations/providers/overview) for details.


> [!TIP]
>
> See the [tools guide](lc:oss/python/langchain/tools) for details and other options for creating tools.
...
When binding user-defined tools, the model's response includes a **request** to execute a tool. When using a model separately from an [agent](lc:oss/python/langchain/agents), it is up to you to execute the requested tool and return the result back to the model for use in subsequent reasoning. When using an [agent](lc:oss/python/langchain/agents), the agent loop will handle the tool execution loop for you.

Below, we show some common ways you can use tool calling.

    ### Tool execution loop

        When a model returns tool calls, you need to execute the tools and pass the results back to the model. This creates a conversation loop where the model can use tool results to generate its final response. LangChain includes [agent](lc:oss/python/langchain/agents) abstractions that handle this orchestration for you.
    ...
    ### Forcing tool calls
        ...
        By default, the model has the freedom to choose which bound tool to use based on the user's input. However, you might want to force choosing a tool, ensuring the model uses either a particular tool or **any** tool from a given list:
            ...
            [
             {
              "label": "Force use of any tool",
              "lang": "python",
              "code": "model_with_tools = model.bind_tools([tool_1], tool_choice=\"any\")"
             },
             {
              "label": "Force use of specific tools",
              "lang": "python",
              "code": "model_with_tools = model.bind_tools([tool_1], tool_choice=\"tool_1\")"
             }
            ]
            ```
...
> [!TIP]
>
> Most models supporting tool calling enable parallel tool calls by default. Some (including [OpenAI](lc:oss/python/integrations/chat/openai) and [Anthropic](lc:oss/python/integrations/chat/anthropic)) allow you to disable this feature. To do this, set `parallel_tool_calls=False`:
>         ```python
...
> [!TIP]
>
> To learn about structured output, see [Structured output](lc:oss/python/langchain/structured-output).
...
>
>     See your [provider's integration page](lc:oss/python/integrations/providers/overview) for supported methods and configuration options.
...
1. [Summarization middleware](lc:oss/python/langchain/middleware/built-in#summarization) can trigger summarization based on a model's context window size.
2. [Structured output](lc:oss/python/langchain/structured-output) strategies in `create_agent` can be inferred automatically (e.g., by checking support for native structured output features).
3. Model inputs can be gated based on supported [modalities](#multimodal) and maximum input tokens.
4. [Deep Agents Code](lc:oss/deepagents/code/overview) filters the [interactive model switcher](lc:oss/deepagents/code/providers#which-models-appear-in-the-switcher) to models whose profiles report `tool_calling` support and text I/O, and displays context window sizes and capability flags in the selector detail view.
...
### Updating or overwriting profile data
    ...
    **Option 2 (fix data upstream)**
    ...
    The primary source for the data is the [models.dev](https://models.dev/) project. This data is merged with additional fields and overrides in LangChain [integration packages](lc:oss/python/integrations/providers/overview) and are shipped with those packages.
    ...
    2. (If needed) update additional fields and overrides in `langchain_<package>/data/profile_augmentations.toml` through a pull request to the LangChain [integration package](lc:oss/python/integrations/providers/overview)`.
...
Certain models can process and return non-textual data such as images, audio, and video. You can pass non-textual data to a model by providing [content blocks](lc:oss/python/langchain/messages#message-content).
...
>
>     1. Data in the cross-provider standard format (see [our messages guide](lc:oss/python/langchain/messages))
...
See the [multimodal section](lc:oss/python/langchain/messages#multimodal) of the messages guide for details.
...
See the [integrations page](lc:oss/python/integrations/providers/overview) for details on specific providers.
...
**If supported by the underlying model,** you can surface this reasoning process to better understand how the model arrived at its final answer.
    ...
    [
     {
      "label": "Stream reasoning output",
      "lang": "python",
      "code": "for chunk in model.stream(\"Why do parrots have colorful feathers?\"):\n    reasoning_steps = [r for r in chunk.content_blocks if r[\"type\"] == \"reasoning\"]\n    print(reasoning_steps if reasoning_steps else chunk.text)"
     },
     {
      "label": "Complete reasoning output",
      "lang": "python",
      "code": "response = model.invoke(\"Why do parrots have colorful feathers?\")\nreasoning_steps = [b for b in response.content_blocks if b[\"type\"] == \"reasoning\"]\nprint(\" \".join(step[\"reasoning\"] for step in reasoning_steps))"
     }
    ]
    ```
...
Some providers also accept a native alias for `reasoning_effort` (for example, `ChatAnthropic` accepts `effort` and `ChatGoogleGenerativeAI` accepts `thinking_level`). See the [chat model integrations](lc:oss/python/integrations/chat/index) page for provider-specific detail.

For details, see the [integrations page](lc:oss/python/integrations/providers/overview) or [reference](https://reference.langchain.com/python/integrations/) for your respective chat model.

### Local models

LangChain supports running models locally on your own hardware. This is useful for scenarios where either data privacy is critical, you want to invoke a custom model, or when you want to avoid the costs incurred when using a cloud-based model.

[Ollama](lc:oss/python/integrations/chat/ollama) is one of the easiest ways to run chat and embedding models locally.
...
- **Implicit provider caching:** providers automatically pass on cost savings if a request hits a cache, with no configuration required. Examples: [OpenAI](lc:oss/python/integrations/chat/openai) and [Gemini](lc:oss/python/integrations/chat/google_generative_ai).
- **Provider-level explicit controls:** providers let you manually indicate cache points for greater control or to guarantee cost savings. These mirror the underlying provider/API behavior. Examples:
    ...
    - Anthropic content-block [`cache_control`](lc:oss/python/integrations/chat/anthropic#prompt-caching)
    - [Gemini](https://reference.langchain.com/python/integrations/langchain_google_genai/).
    - AWS Bedrock [`cachePoint`](lc:oss/python/integrations/chat/bedrock#prompt-caching) blocks
- **LangChain middleware:** for agents, middleware lets LangChain optimize caching of stable system prompt and tool content. Examples:
    - Anthropic's [`AnthropicPromptCachingMiddleware`](lc:oss/python/integrations/middleware/anthropic#prompt-caching)
    - AWS Bedrock's [`BedrockPromptCachingMiddleware`](lc:oss/python/integrations/middleware/aws#prompt-caching)


> [!WARNING]
>
> Prompt caching is often only engaged above a minimum input token threshold. See [provider pages](lc:oss/python/integrations/chat/index) for details.


Cache usage will be reflected in the [usage metadata](lc:oss/python/langchain/messages#token-usage) of the model response.
...
If a model invokes a tool server-side, the content of the response message will include content representing the invocation and result of the tool. Accessing the [content blocks](lc:oss/python/langchain/messages#standard-content-blocks) of the response will return the server-side tool calls and results in a provider-agnostic format:

```lc-tabs
[
 {
  "label": "Invoke with server-side tool use",
  "lang": "python",
  "code": "from langchain.chat_models import init_chat_model\n\nmodel = init_chat_model(\"gpt-5.4-mini\")\n\ntool = {\"type\": \"web_search\"}\nmodel_with_tools = model.bind_tools([tool])\n\nresponse = model_with_tools.invoke(\"What was a positive news story from today?\")\nprint(response.content_blocks)"
 },
 {
  "label": "Result expandable",
  "lang": "python",
  "code": "[\n    {\n        \"type\": \"server_tool_call\",\n        \"name\": \"web_search\",\n        \"args\": {\n            \"query\": \"positive news stories today\",\n            \"type\": \"search\"\n        },\n        \"id\": \"ws_abc123\"\n    },\n    {\n        \"type\": \"server_tool_result\",\n        \"tool_call_id\": \"ws_abc123\",\n        \"status\": \"success\"\n    },\n    {\n        \"type\": \"text\",\n        \"text\": \"Here are some positive news stories from today...\",\n        \"annotations\": [\n            {\n                \"end_index\": 410,\n                \"start_index\": 337,\n                \"title\": \"article title\",\n                \"type\": \"citation\",\n                \"url\": \"...\"\n            }\n        ]\n    }\n]"
 }
]
```
This represents a single conversational turn; there are no associated [ToolMessage](lc:oss/python/langchain/messages#tool-message) objects that need to be passed in as in client-side [tool-calling](#tool-calling).

See the [integration page](lc:oss/python/integrations/chat/index) for your given provider for available tools and usage details.
...
>     - [OpenRouter via `ChatOpenRouter`](lc:oss/python/integrations/chat/openrouter) (`langchain-openrouter`)
>     - [LiteLLM via `ChatLiteLLM` / `ChatLiteLLMRouter`](lc:oss/python/integrations/chat/index) (`langchain-litellm`)
...
> [!NOTE]
>
> When using direct chat model class instantiation, the parameter name may vary by provider. Check the respective [reference](lc:oss/python/integrations/providers/overview) for details.
...
> [!NOTE]
>
> Proxy support varies by integration. Check the specific model provider's [reference](lc:oss/python/integrations/providers/overview) for proxy configuration options.
...
A number of model providers return token usage information as part of the invocation response. When available, this information will be included on the `AIMessage` objects produced by the corresponding model. For more details, see the [messages](lc:oss/python/langchain/messages) guide.


> [!NOTE]
>
> Some provider APIs, notably OpenAI and Azure OpenAI chat completions, require users opt-in to receiving token usage data in streaming contexts. See the [streaming usage metadata](lc:oss/python/integrations/chat/openai#streaming-usage-metadata) section of the integration guide for details.
...
- Debugging with [LangSmith](lc:langsmith/observability) tracing
...
> [!TIP]
>
> For model configuration details, see [Models](lc:oss/python/langchain/models). For dynamic model selection patterns, see [Dynamic model in middleware](lc:oss/python/langchain/middleware/overview#dynamic-model).
