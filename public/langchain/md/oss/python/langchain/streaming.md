...
> [!NOTE]
>
> Persisting conversation history with `thread_id` requires the agent to be configured with a [checkpointer](lc:oss/python/langchain/long-term-memory). On [LangSmith deployments](lc:langsmith/deployment) a checkpointer is provisioned automatically. Locally, pass one explicitly, for example `create_agent(..., checkpointer=InMemorySaver())`. The remaining snippets on this page omit `thread_id` for brevity, but you should pass it in production.
...
[
 {
  "label": "title=\"Streaming custom updates\"",
  "lang": "python",
 ...
 },
 {
  "label": "title=\"Output\"",
  "lang": "shell",
  "code": "Looking up data for city: San Francisco\nAcquired data for city: San Francisco"
 }
]
```
...
[
 {
  "label": "title=\"Streaming multiple modes\"",
  "lang": "python",
...
Some models perform internal reasoning before producing a final answer. You can stream these thinking / reasoning tokens as they're generated by filtering [standard content blocks](lc:oss/python/langchain/messages#standard-content-blocks) for the `type` `"reasoning"`.
...
>
>     See the [reasoning section](lc:oss/python/langchain/models#reasoning) and your [provider's integration page](lc:oss/python/integrations/providers/overview) for configuration details.
>
...
This works the same way regardless of the model provider—LangChain normalizes provider-specific formats (Anthropic `thinking` blocks, OpenAI `reasoning` summaries, etc.) into a standard `"reasoning"` content block type via the [`content_blocks`](lc:oss/python/langchain/messages#standard-content-blocks) property.

To stream reasoning tokens directly from a chat model (without an agent), see [streaming with chat models](lc:oss/python/langchain/models#reasoning).

### Streaming tool calls

You may want to stream both:

1. Partial JSON as [tool calls](lc:oss/python/langchain/models#tool-calling) are generated
2. The completed, parsed tool calls that are executed

Specifying [`stream_mode="messages"`](#llm-tokens) will stream incremental [message chunks](lc:oss/python/langchain/messages#streaming-and-chunks) generated by all LLM calls in the agent. To access the completed messages with parsed tool calls:

1. If those messages are tracked in the [state](lc:oss/python/langchain/short-term-memory) (as in the model node of [`create_agent`](lc:oss/python/langchain/agents)), use `stream_mode=["messages", "updates"]` to access completed messages through [state updates](#agent-progress) (demonstrated below).
...
> [!NOTE]
>
> If completed messages are tracked in an agent's [state](lc:oss/python/langchain/short-term-memory), you can use `stream_mode=["messages", "updates"]` as demonstrated in the [Streaming tool calls](#streaming-tool-calls) section to access completed messages during streaming.
...
Consider the below example, where we incorporate a [stream writer](#custom-updates) into a simplified [guardrail middleware](lc:oss/python/langchain/guardrails#after-agent-guardrails). This middleware demonstrates tool calling to generate a structured "safe / unsafe" evaluation (one could also use [structured outputs](lc:oss/python/langchain/models#structured-output) for this):
```python
...
To handle human-in-the-loop [interrupts](lc:oss/python/langchain/human-in-the-loop), we build on the [above example](#streaming-tool-calls):

1. We configure the agent with [human-in-the-loop middleware and a checkpointer](lc:oss/python/langchain/human-in-the-loop#configuring-interrupts)
2. We collect interrupts generated during the `"updates"` stream mode
3. We respond to those interrupts with a [command](lc:oss/python/langchain/human-in-the-loop#responding-to-interrupts)
...
We next collect a [decision](lc:oss/python/langchain/human-in-the-loop#interrupt-decision-types) for each interrupt. Importantly, the order of decisions must match the order of actions we collected.
...
We can then resume by passing a [command](lc:oss/python/langchain/human-in-the-loop#responding-to-interrupts) into the same streaming loop:
```python
interrupts = []
for chunk in agent.stream(
...
3. We specify [`subgraphs=True`](lc:oss/python/langgraph/use-subgraphs#stream-subgraph-outputs) when creating the stream
...
- Working with [multi-agent](lc:oss/python/langchain/multi-agent/index) systems to control which agents stream their output
- Mixing models that support streaming with those that do not
- Deploying to [LangSmith](lc:langsmith/observability) and wanting to prevent certain model outputs from being streamed to the client
...
See the [LangGraph streaming guide](lc:oss/python/langgraph/streaming#disable-streaming-for-specific-chat-models) for more details.
...
[
 {
  "label": "v2 (new)",
  "lang": "python",
  "code": "# Unified format \u2014 no more tuple unpacking\nfor chunk in agent.stream(\n    {\"messages\": [{\"role\": \"user\", \"content\": \"What is the weather in SF?\"}]},\n    stream_mode=[\"updates\", \"custom\"],\n    version=\"v2\",\n):\n    print(chunk[\"type\"])  # \"updates\" or \"custom\"\n    print(chunk[\"data\"])  # payload"
 },
 {
  "label": "v1 (current default)",
  "lang": "python",
  "code": "# Must unpack (mode, data) tuples\nfor mode, chunk in agent.stream(\n    {\"messages\": [{\"role\": \"user\", \"content\": \"What is the weather in SF?\"}]},\n    stream_mode=[\"updates\", \"custom\"],\n):\n    print(mode)   # \"updates\" or \"custom\"\n    print(chunk)  # payload"
 }
]
```
...
See the [LangGraph streaming docs](lc:oss/python/langgraph/streaming#stream-output-format-v2) for more details on the v2 format, including type narrowing, Pydantic/dataclass coercion, and subgraph streaming.

## Related

- [Frontend streaming](lc:oss/python/langchain/frontend/overview)—Build React UIs with `useStream` for real-time agent interactions
- [Streaming with chat models](lc:oss/python/langchain/models#stream)—Stream tokens directly from a chat model without using an agent or graph
- [Reasoning with chat models](lc:oss/python/langchain/models#reasoning)—Configure and access reasoning output from chat models
- [Standard content blocks](lc:oss/python/langchain/messages#standard-content-blocks)—Understand the normalized content block format used for reasoning, text, and other content types
- [Streaming with human-in-the-loop](lc:oss/python/langchain/human-in-the-loop#streaming-with-human-in-the-loop)—Stream agent progress while handling interrupts for human review
- [LangGraph streaming](lc:oss/python/langgraph/streaming)—Advanced streaming options including `values`, `debug` modes, and subgraph streaming
