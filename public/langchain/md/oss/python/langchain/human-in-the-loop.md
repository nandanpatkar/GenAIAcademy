The Human-in-the-Loop (HITL) [middleware](lc:oss/python/langchain/middleware/built-in#human-in-the-loop) lets you add human oversight to agent tool calls.
...
It does this by checking each tool call against a configurable policy. If intervention is needed, the middleware issues an `interrupt` that halts execution. The graph state is saved using LangGraph's [persistence layer](lc:oss/python/langgraph/persistence), so execution can pause safely and resume later.
...
The [middleware](lc:oss/python/langchain/middleware/built-in#human-in-the-loop) defines four built-in ways a human can respond to an interrupt:
...
To use HITL, add the [middleware](lc:oss/python/langchain/middleware/built-in#human-in-the-loop) to the agent's `middleware` list when creating the agent.
...
>     See the [LangGraph interrupts documentation](lc:oss/python/langgraph/interrupts) for details.
...
See the [Streaming](lc:oss/python/langchain/streaming) guide for more details on stream modes.
...
For more specialized workflows, you can build custom HITL logic directly using the `interrupt` primitive and [middleware](lc:oss/python/langchain/middleware/overview) abstraction.
