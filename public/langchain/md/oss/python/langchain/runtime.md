...
2. **Store**: a `BaseStore` instance used for [long-term memory](lc:oss/python/langchain/long-term-memory)
...
* Write to the [custom stream](lc:oss/python/langchain/streaming#custom-updates) (ex, tool progress / updates)
...
Use the `Runtime` parameter to access the `Runtime` object inside [node-style hooks](lc:oss/python/langchain/middleware/custom#node-style-hooks).  For [wrap-style hooks](lc:oss/python/langchain/middleware/custom#wrap-style-hooks), the `Runtime` object is available inside the `ModelRequest` parameter.
