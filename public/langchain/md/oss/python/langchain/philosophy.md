...
Today developers can choose how they build agents: use [LangChain](lc:oss/python/langchain/overview) for maximum flexibility and control, or [Deep Agents](lc:oss/python/langchain/overview) which allows for similar flexibility and control but comes with opinionated built-in planning, filesystem tools, subagents, and context management. Both are built on [LangGraph](lc:oss/python/langgraph/overview).

With LangChain, we have two core focuses:

    #### Step: We want to enable developers to build with the best models.
    ...
    #### Step: We want to make it easy to use models to orchestrate more complex flows that interact with other data and computation.

        Models should be used for more than just *text generation* - they should also be used to orchestrate more complex flows that interact with other data. LangChain makes it easy to define [tools](lc:oss/python/langchain/tools) that LLMs can use dynamically, as well as help with parsing of and access to unstructured data.
