...
We will also incorporate [human-in-the-loop review](lc:oss/python/langchain/human-in-the-loop) to allow users to approve, edit, and reject actions (such as outbound emails) as desired.


> [!NOTE]
>
> If you are migrating from the [`langgraph-supervisor`](https://github.com/langchain-ai/langgraph-supervisor-py) package, see [Migrate from langgraph-supervisor](lc:oss/python/migrate/langgraph-supervisor) for before-and-after patterns, including interrupt and resume flows.


### Why use a supervisor?

Multi-agent architectures allow you to partition [tools](lc:oss/python/langchain/tools) across workers, each with their own individual prompts or instructions. Consider an agent with direct access to all calendar and email APIs: it must choose from many similar tools, understand exact formats for each API, and handle multiple domains simultaneously. If performance degrades, it may be helpful to separate related tools and associated prompts into logical groups (in part to manage iterative improvements).

### Concepts

We will cover the following concepts:

- [Multi-agent systems](lc:oss/python/langchain/multi-agent/index)
- [Human-in-the-loop review](lc:oss/python/langchain/human-in-the-loop)
...
[
 ...
 },
 {
  "label": "conda",
  "lang": "bash",
  "code": "conda install langchain -c conda-forge"
 }
]
```


For more details, see our [Installation guide](lc:oss/python/langchain/install).
...
[
 ...
 },
 {
  "label": "Python",
  "lang": "python",
  "code": "os.environ[\"LANGSMITH_TRACING\"] = \"true\"\nos.environ[\"LANGSMITH_API_KEY\"] = getpass.getpass()"
 }
]
```
...
It can be prudent to incorporate [human-in-the-loop review](lc:oss/python/langchain/human-in-the-loop) of sensitive actions. LangChain includes [built-in middleware](lc:oss/python/langchain/human-in-the-loop#configuring-interrupts) to review tool calls, in this case the tools invoked by sub-agents.

Let's add human-in-the-loop review to both sub-agents:
- We configure the `create_calendar_event` and `send_email` tools to interrupt, permitting all [response types](lc:oss/python/langchain/human-in-the-loop) (`approve`, `edit`, `reject`)
- We add a [checkpointer](lc:oss/python/langchain/short-term-memory) **only to the top-level agent**. This is required to pause and resume execution.
...
We can specify decisions for each interrupt by referring to its ID using a `Command`. Refer to the [human-in-the-loop guide](lc:oss/python/langchain/human-in-the-loop) for additional details. For demonstration purposes, here we will accept the calendar event, but edit the subject of the outbound email:
...
>
> For simpler cases with just a few tools, use a single agent. When agents need to have conversations with users, use [handoffs](lc:oss/python/langchain/multi-agent/handoffs) instead. For peer-to-peer collaboration between agents, consider other multi-agent patterns.


## Next steps

Learn about [handoffs](lc:oss/python/langchain/multi-agent/handoffs) for agent-to-agent conversations, explore [context engineering](lc:oss/python/langchain/context-engineering) to fine-tune information flow, read the [multi-agent overview](lc:oss/python/langchain/multi-agent/index) to compare different patterns, and use [LangSmith](https://smith.langchain.com) to debug and monitor your multi-agent system.
