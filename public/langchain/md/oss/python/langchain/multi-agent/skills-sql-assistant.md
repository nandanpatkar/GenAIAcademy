...
> [!NOTE]
>
> For a complete example of a SQL agent with query execution, error correction, and validation, see our [SQL Agent tutorial](lc:oss/python/langchain/sql-agent). This tutorial focuses on the progressive disclosure pattern which can be applied to any domain.
...
The `load_skill` tool returns the full skill content as a string, which becomes part of the conversation as a ToolMessage. For more details on creating and using tools, see the [Tools guide](lc:oss/python/langchain/tools).
...
> [!NOTE]
>
> This guide demonstrates creating custom middleware. For a comprehensive guide on middleware concepts and patterns, see the [custom middleware documentation](lc:oss/python/langchain/middleware/custom).
...
> [!NOTE]
>
> **Production consideration**: This tutorial loads the skill list in `__init__` for simplicity. In a production system, you may want to load skills in the `before_agent` hook instead, allowing them to be refreshed periodically to reflect up-to-date changes (e.g., when new skills are added or existing ones are modified). See the [before_agent hook documentation](lc:oss/python/langchain/middleware/custom#node-style-hooks) for details.
...
Progressive disclosure is fundamentally a **[context engineering](lc:oss/python/langchain/context-engineering) technique** - you're managing what information is available to the agent and when. This tutorial focused on loading database schemas, but the same principles apply to other types of context.
...
- Learn about [middleware](lc:oss/python/langchain/middleware/overview) for more dynamic agent behaviors
- Explore [context engineering](lc:oss/python/langchain/context-engineering) techniques for managing agent context
- Explore the [handoffs pattern](lc:oss/python/langchain/multi-agent/handoffs-customer-support) for sequential workflows
- Read the [subagents pattern](lc:oss/python/langchain/multi-agent/subagents-personal-assistant) for parallel task routing
- See [multi-agent patterns](lc:oss/python/langchain/multi-agent/index) for other approaches to specialized agents
