In the **custom workflow** architecture, you define your own bespoke execution flow using [LangGraph](lc:oss/python/langgraph/overview). You have complete control over the graph structure—including sequential steps, conditional branches, loops, and parallel execution.
...
Each node in your workflow can be a simple function, an LLM call, or an entire [agent](lc:oss/python/langchain/agents) with [tools](lc:oss/python/langchain/tools). You can also compose other architectures within a custom workflow—for example, embedding a multi-agent system as a single node.
...
The [router pattern](lc:oss/python/langchain/multi-agent/router) is an example of a custom workflow. This tutorial walks through building a router that queries GitHub, Notion, and Slack in parallel, then synthesizes results.
>
...
A common use case is combining [retrieval](lc:oss/python/deepagents/retrieval) with an agent. This example builds a WNBA stats assistant that retrieves from a knowledge base and can fetch live news.

### Custom RAG workflow

The workflow demonstrates three types of nodes:

- **Model node** (Rewrite): Rewrites the user query for better retrieval using [structured output](lc:oss/python/langchain/structured-output).
...
> [!NOTE]
>
> In production, use a persistent vector store such as [Valkey](lc:oss/python/integrations/vectorstores/valkey), [Databricks Vector Search](lc:oss/python/integrations/vectorstores/databricks_vector_search), or [MongoDB Atlas](lc:oss/python/integrations/vectorstores/mongodb_atlas) instead of `InMemoryVectorStore`. See [all vector stores](lc:oss/python/integrations/vectorstores/index).
