`ParallelSearchRetriever` is a LangChain [`BaseRetriever`](lc:oss/python/deepagents/retrieval) backed by [Parallel](https://platform.parallel.ai/)'s [Search API](https://docs.parallel.ai/search/search-quickstart). It returns `list[Document]` with rich `metadata` (`url`, `title`, `publish_date`, `search_id`, `excerpts`, `query`) and slots into any RAG pipeline.


> [!NOTE]
>
> Looking for an LLM-callable tool that returns the raw search response instead of `Document`s? See [ParallelSearchTool](lc:oss/python/integrations/tools/parallel_search).
...
| :--- | :--- | :---: | :---: |
| `ParallelSearchRetriever` | `langchain-parallel` | ❌ | <a href="https://pypi.org/project/langchain-parallel/" target="_blank">
...
For detailed documentation, head to the `ParallelSearchRetriever` API reference or the [Parallel Search API guides](https://docs.parallel.ai/search/search-quickstart).
