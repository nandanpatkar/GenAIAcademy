...
If you want to get automated tracing from individual queries, you can also set your [LangSmith](lc:langsmith/observability) API key by uncommenting below:
...
This retriever lives in the `langchain-elasticsearch` package. For demonstration purposes, we will also install `langchain-community` to generate text [embeddings](lc:oss/python/integrations/embeddings/index).
...
For vector search, we are going to use random embeddings just for illustration. For real use cases, pick one of the available LangChain [Embeddings](lc:oss/python/integrations/embeddings/index) classes.
