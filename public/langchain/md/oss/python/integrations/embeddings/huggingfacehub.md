...
- **Self-hosted at scale** via [Text Embeddings Inference (TEI)](lc:oss/python/integrations/embeddings/text_embeddings_inference): Hugging Face's production inference server. Point `OpenAIEmbeddings` from `langchain-openai` at TEI's OpenAI-compatible API.
...
See the dedicated [Sentence Transformers guide](lc:oss/python/integrations/embeddings/sentence_transformers) for model selection, GPU configuration, and query/document prompts.
