...
>
>     Note that certain Anthropic models can also be accessed via AWS Bedrock and Google VertexAI. See the [`ChatBedrock`](lc:oss/python/integrations/chat/bedrock) and [`ChatVertexAI`](lc:oss/python/integrations/chat/google_vertex_ai) integrations to use Anthropic models via these services.
>
>     For Anthropic models on AWS Bedrock with the same API as `ChatAnthropic`, use [`ChatAnthropicBedrock`](lc:oss/python/integrations/chat/bedrock#chatanthropicbedrock) from `langchain-aws`.
...
| :--- | :--- | :---: |  :---: | :---: | :---: |
| `ChatAnthropic` | `langchain-anthropic` | beta | ✅ [(npm)](https://js.langchain.com/docs/integrations/chat/anthropic) | <a href="https://pypi.org/project/langchain-anthropic/" target="_blank">
...
![PyPI - Latest version](https://img.shields.io/pypi/v/langchain-anthropic?style=flat-square&label=%20&color=orange)
</a> |
...
### Installation
    ...
    [
     ...
     },
     {
      "label": "uv",
      "lang": "bash",
      "code": "uv add langchain-anthropic"
     }
    ]
    ```
...
> **Inference geography**
>
>     To control where model inference runs for [data residency](https://platform.claude.com/docs/en/build-with-claude/data-residency), pass `inference_geo` when you create `ChatAnthropic`. See Anthropic documentation for supported values.
...
Learn more about supported invocation methods in our [models](lc:oss/python/langchain/models#invocation) guide.
...
Using `content_blocks` will render the content in LangChain's standard format that is consistent across other model providers. Read more about [content blocks](lc:oss/python/langchain/messages#standard-content-blocks).
...
See below for a full example with [`create_agent`](lc:oss/python/langchain/agents).
...
For Claude Sonnet and earlier models, you will need to specify a token budget. For Claude Opus 4.6+, you can use adaptive thinking which automatically determines the budget.
    ...
    [
     ...
     {
      "label": "Adaptive (Opus 4.6+)",
      "lang": "python",
      "code": "from langchain_anthropic import ChatAnthropic\n\nmodel = ChatAnthropic(\n    model=\"claude-opus-4-8\",\n    max_tokens=5000,\n    thinking={\"type\": \"adaptive\"}, # [!code highlight]\n)\n\nresponse = model.invoke(\"What is the cube root of 50.653?\")\nprint(json.dumps(response.content_blocks, indent=2))"
     },
     {
      "label": "Invocation param",
      "lang": "python",
...
`effort` is an alias for the standard [`reasoning_effort`](lc:oss/python/langchain/models#reasoning) parameter, at both construction and call time. If both are set, `effort` wins.
...
### End to end example with LangGraph
    ...
    Here we demonstrate an end-to-end example in which we populate a LangChain [vector store](lc:oss/python/integrations/vectorstores/index) with sample documents and equip Claude with a tool that queries those documents.
...
Anthropic also lets you specify your own splits using [custom document](https://platform.claude.com/docs/en/build-with-claude/citations#custom-content-documents) types. LangChain [text splitters](lc:oss/python/integrations/splitters/index) can be used to generate meaningful splits for this purpose. See the below example, where we split the LangChain `README.md` (a markdown document) and pass it to Claude as context:
...
Anthropic supports [caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching) of elements of your prompts, including messages, tool definitions, tool results, images and documents. This allows you to reuse large documents, instructions, [few-shot documents](lc:langsmith/create-few-shot-evaluators), and other data to reduce latency and costs.
...
> [!NOTE]
>
> For LangChain agents, prefer [`AnthropicPromptCachingMiddleware`](lc:oss/python/integrations/middleware/anthropic#prompt-caching) when you want LangChain to optimize stable system prompt and tool content. The middleware is a LangChain agent/harness optimization and is not the same as the invocation-level `cache_control` shown below, which mirrors the Anthropic API behavior.
...
Below, we implement a simple chatbot that incorporates this feature. We follow the LangChain [chatbot tutorial](lc:oss/python/langchain/quickstart), but add a custom [reducer](lc:oss/python/langgraph/graph-api#reducers) that automatically marks the last content block in each user message with `cache_control`:
...
You can access this feature in individual model calls, or by specifying the [response format](lc:oss/python/langchain/structured-output) of a LangChain [agent](lc:oss/python/langchain/agents). See below for examples.

### Individual model calls

Use the [`with_structured_output`](lc:oss/python/langchain/models#structured-output) method to generate a structured model response. Specify `method="json_schema"` to enable Anthropic's native structured output feature; otherwise the method defaults to using function calling.
...
Specify `response_format` with [`ProviderStrategy`](lc:oss/python/langchain/structured-output) to engage Anthropic's structured output feature when generating its final response.
...
Anthropic supports a variety of built-in client and server-side [tools](lc:oss/python/langchain/tools).
...
> **Middleware vs tools**
>
>     For client-side tools (e.g. [bash](#bash-tool), [text editor](#text-editor), [memory](#memory-tool)), you may opt to use [middleware](lc:oss/python/integrations/middleware/anthropic), which provide production-ready implementations that contain built-in execution, state management, and security policies.
>
...
> [!TIP]
>
> For a "batteries-included" implementation, consider using [`ClaudeBashToolMiddleware`](lc:oss/python/integrations/middleware/anthropic#bash-tool) which provides persistent sessions, Docker isolation, output redaction, and startup/shutdown commands out of the box.
...
> [!TIP]
>
> For a "batteries-included" implementation, consider using [`StateClaudeTextEditorMiddleware`](lc:oss/python/integrations/middleware/anthropic#text-editor) or [`FilesystemClaudeTextEditorMiddleware`](lc:oss/python/integrations/middleware/anthropic#text-editor) which provide LangGraph state integration or filesystem persistence, path validation, and other features.
...
> [!TIP]
>
> For a "batteries-included" implementation, consider using [`StateClaudeMemoryMiddleware`](lc:oss/python/integrations/middleware/anthropic#memory) or [`FilesystemClaudeMemoryMiddleware`](lc:oss/python/integrations/middleware/anthropic#memory) which provide LangGraph state integration or filesystem persistence, automatic system prompt injection, and other features.
