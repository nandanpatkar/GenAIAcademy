...
For a complete list of changes, see the [migration guide](lc:oss/python/migrate/langchain-v1).
...
Great agents require [context engineering](lc:oss/python/langchain/context-engineering): getting the right information to the model at the right time. Middleware helps you control dynamic prompts, conversation summarization, selective tool access, state management, and guardrails through a composable abstraction.

#### Prebuilt middleware

LangChain provides a few [prebuilt middlewares](lc:oss/python/langchain/middleware/overview#built-in-middleware) for common patterns, including:
...
For more information, see [the complete middleware guide](lc:oss/python/langchain/middleware/overview).

### Built on LangGraph

Because `create_agent` is built on [LangGraph](lc:oss/python/langgraph/overview), you automatically get built in support for long running and reliable agents via:
...
- **Backward compatible**: Standard content can be [loaded lazily](lc:oss/python/langchain/messages#standard-content-blocks), so there are no associated breaking changes

For more information, see our guide on [content blocks](lc:oss/python/langchain/messages#standard-content-blocks).
...
See our [migration guide](lc:oss/python/migrate/langchain-v1) for help updating your code to LangChain v1.
