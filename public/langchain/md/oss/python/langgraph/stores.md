Stores let agents persist information across threads, including user preferences, accumulated knowledge, and facts that should survive beyond a single conversation. Unlike [checkpointers](lc:oss/python/langgraph/checkpointers), which save the full graph state scoped to one thread, stores hold arbitrary key-value data accessible from any thread.

![Model of shared state](/langchain/images/oss/images/shared_state.png)


> [!NOTE]
>
> **Agent Server handles stores automatically**
> When using the [Agent Server](lc:langsmith/agent-server), you do not need to implement or configure stores manually. The API handles all storage infrastructure for you behind the scenes.
...
> [!NOTE]
>
> See [store integrations](lc:oss/python/integrations/long-term-memory/index) for the full list of available providers.
...
When you use LangSmith locally (e.g., in [Studio](lc:langsmith/studio)) or [hosted](lc:langsmith/platform-setup), the base store is available to use by default and you do not need to specify it during graph compilation. To enable semantic search, however, you **do** need to configure the indexing settings in your `langgraph.json` file. For example:
...
See the [deployment guide](lc:langsmith/semantic-search) for more details and configuration options.
...
- [Add a custom store to Agent Server](lc:langsmith/custom-store) — deploying your implementation
- [Checkpointers](lc:oss/python/langgraph/checkpointers) — thread-scoped state persistence
