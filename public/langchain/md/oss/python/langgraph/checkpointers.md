...
![Checkpoints](/langchain/images/oss/images/checkpoints.jpg)


> [!NOTE]
>
> **Agent Server handles checkpointing automatically**
> When using the [Agent Server](lc:langsmith/agent-server), you do not need to implement or configure checkpointers manually. The server handles all persistence infrastructure for you behind the scenes.


> [!TIP]
>
> Trace checkpointed state and debug how your agent resumes across sessions with [LangSmith](https://smith.langchain.com). Follow the [tracing quickstart](lc:langsmith/trace-with-langgraph) to get set up.
...
- **Human-in-the-loop**: Checkpointers facilitate [human-in-the-loop workflows](lc:oss/python/langgraph/interrupts) by allowing humans to inspect, interrupt, and approve graph steps. Checkpointers are needed for these workflows as the person has to be able to view the state of a graph at any point in time, and the graph has to be able to resume execution after the person has made any updates to the state. See [Interrupts](lc:oss/python/langgraph/interrupts) for examples.
- **Memory**: Checkpointers allow for ["memory"](lc:oss/python/concepts/memory) between interactions. In the case of repeated human interactions (like conversations) any follow up messages can be sent to that thread, which will retain its memory of previous ones. See [Add memory](lc:oss/python/langgraph/add-memory) for information on how to add and manage conversation memory using checkpointers.
- **Time travel**: Checkpointers allow for ["time travel"](lc:oss/python/langgraph/use-time-travel), allowing users to replay prior graph executions to review and / or debug specific graph steps. In addition, checkpointers make it possible to fork the graph state at arbitrary checkpoints to explore alternative trajectories.
...
A thread is a unique ID or thread identifier assigned to each checkpoint saved by a checkpointer. It contains the accumulated state of a sequence of [runs](lc:langsmith/runs). When a run is executed, the [state](lc:oss/python/langgraph/graph-api#state) of the underlying graph of the assistant will be persisted to the thread.
...
The checkpointer uses `thread_id` as the primary key for storing and retrieving checkpoints. Without it, the checkpointer cannot save state or resume execution after an [interrupt](lc:oss/python/langgraph/interrupts), since the checkpointer uses `thread_id` to load the saved state.
...
See [Subgraphs](lc:oss/python/langgraph/use-subgraphs) for more details on working with subgraph state and checkpoints.
...
![State](/langchain/images/oss/images/get_state.jpg)
...
Replay re-executes steps from a prior checkpoint. Invoke the graph with a prior `checkpoint_id` to re-run nodes after that checkpoint. Nodes before the checkpoint are skipped (their results are already saved). Nodes after the checkpoint re-execute, including any LLM calls, API requests, or [interrupts](lc:oss/python/langgraph/interrupts) — which are always re-triggered during replay.

See [Time travel](lc:oss/python/langgraph/use-time-travel) for full details and code examples on replaying past executions.

![Replay](/langchain/images/oss/images/re_play.png)

### Update state

You can edit the graph state using `update_state`. This creates a new checkpoint with the updated values — it does not modify the original checkpoint. The update is treated the same as a node update: values are passed through [reducer](lc:oss/python/langgraph/graph-api#reducers) functions when defined, so channels with reducers _accumulate_ values rather than overwrite them.

You can optionally specify `as_node` to control which node the update is treated as coming from, which affects which node executes next. See [Time travel: `as_node`](lc:oss/python/langgraph/use-time-travel#from-a-specific-node) for details.


![Update](/langchain/images/oss/images/checkpoints_full_story.jpg)
...
`DeltaChannel` stores only incremental deltas instead of the full accumulated value, substantially reducing checkpoint size for append-heavy channels. See [DeltaChannel](lc:oss/python/langgraph/pregel#deltachannel) for usage and the storage-vs-latency tradeoff.
...
> [!NOTE]
>
> See [checkpointer integrations](lc:oss/python/integrations/checkpointers/index) for available providers.
