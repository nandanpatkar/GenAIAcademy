Build a [retrieval](lc:oss/python/deepagents/retrieval) agent with LangGraph that decides when to search a vector store versus answering the user directly.

LangChain offers built-in [agent](lc:oss/python/langchain/agents) implementations built on [LangGraph](lc:oss/python/langgraph/overview) primitives. When you need deeper customization, implement the agent directly in LangGraph. This tutorial walks through one retrieval-agent pattern.
...
![Hybrid RAG](/langchain/images/images/langgraph-hybrid-rag-tutorial.png)

### Concepts

This tutorial covers the following concepts:

- [Retrieval](lc:oss/python/deepagents/retrieval) using
  - [document loaders](lc:oss/python/integrations/document_loaders/index),
  - [text splitters](lc:oss/python/integrations/splitters/index), [embeddings](lc:oss/python/integrations/embeddings/index), and
  - [vector stores](lc:oss/python/integrations/vectorstores/index)
- The LangGraph [Graph API](lc:oss/python/langgraph/graph-api), including state, nodes, edges, and conditional edges.
...
#### Step: Create the retriever tool
...
With the retriever tool ready, start building the agent as a LangGraph graph. In the [Graph API](lc:oss/python/langgraph/graph-api), a graph is made of:

- **[State](lc:oss/python/langgraph/graph-api#state)**: Shared data that nodes read and update. This tutorial uses [`MessagesState`](lc:oss/python/langgraph/graph-api#messagesstate), which stores a `messages` list of [chat messages](lc:oss/python/langchain/messages).


- **[Nodes](lc:oss/python/langgraph/graph-api#nodes)**: Functions that take the current state, run a step (for example, call a model or a tool), and return state updates.
- **[Edges](lc:oss/python/langgraph/graph-api#edges)**: Connections that define which node runs next, including [conditional edges](lc:oss/python/langgraph/graph-api#conditional-edges) that branch based on the state.
...
A normal edge always sends the graph to the same next node. A [conditional edge](lc:oss/python/langgraph/graph-api#conditional-edges) chooses the next node at runtime by running a function over the current state. After retrieval, use that pattern to grade whether the documents are relevant: continue to answer generation if they are, or rewrite the question and try again if they are not.


#### Step: Add document grading
...
#### Step: Test with irrelevant documents
...
#### Step: Test with relevant documents
...
![Agentic RAG graph](/langchain/images/oss/images/agentic-rag-output.png)
...
- [Retrieval](lc:oss/python/langchain/retrieval)
- [Graph API](lc:oss/python/langgraph/graph-api)
- [Agents](lc:oss/python/langchain/agents)
- [Build a RAG agent](lc:oss/python/deepagents/rag)
- [Build a semantic search engine](lc:oss/python/langchain/knowledge-base)
