The [**Functional API**](lc:oss/python/langgraph/functional-api) allows you to add LangGraph's key features ([persistence](lc:oss/python/langgraph/persistence), [memory](lc:oss/python/langgraph/add-memory), [human-in-the-loop](lc:oss/python/langgraph/interrupts), and [streaming](lc:oss/python/langgraph/streaming)) to your applications with minimal changes to your existing code.


> [!TIP]
>
> For conceptual information on the functional API, see [Functional API](lc:oss/python/langgraph/functional-api).
...
The **Functional API** and the [**Graph API**](lc:oss/python/langgraph/graph-api) can be used together in the same application as they share the same underlying runtime.
...
read the [**streaming guide**](lc:oss/python/langgraph/streaming) section for more details.
...
> use the `StreamWriter` class directly. See [Async with Python < 3.11](lc:oss/python/langgraph/streaming#async) for more details.
>
> ```python
...
The functional API supports [human-in-the-loop](lc:oss/python/langgraph/interrupts) workflows using the `interrupt` function and the `Command` primitive.

### Basic human-in-the-loop workflow

We will create three [tasks](lc:oss/python/langgraph/functional-api#task):
...
We can now compose these tasks in an [entrypoint](lc:oss/python/langgraph/functional-api#entrypoint):
...
[interrupt()](lc:oss/python/langgraph/interrupts#pause-using-interrupt) is called inside a task, enabling a human to review and edit the output of the previous task. The results of prior tasks-- in this case `step_1`-- are persisted, so that they are not run again following the `interrupt`.
...
Note that we've paused with an `interrupt` after `step_1`. The interrupt provides instructions to resume the run. To resume, we issue a [`Command`](lc:oss/python/langgraph/interrupts#resuming-interrupts) containing the data expected by the `human_feedback` task.
...
To review tool calls before execution, we add a `review_tool_call` function that calls [`interrupt`](lc:oss/python/langgraph/interrupts#pause-using-interrupt). When this function is called, execution will be paused until we issue a command to resume it.
...
Short-term memory allows storing information across different **invocations** of the same **thread id**. See [short-term memory](lc:oss/python/langgraph/functional-api#short-term-memory) for more details.
...
[long-term memory](lc:oss/python/concepts/memory#long-term-memory) allows storing information across different **thread ids**. This could be useful for learning information about a given user in one conversation and using it in another.

## Workflows

* [Workflows and agent](lc:oss/python/langgraph/workflows-agents) guide for more examples of how to build workflows using the Functional API.

## Integrate with other libraries

* [Add LangGraph's features to other frameworks using the functional API](lc:langsmith/deploy-other-frameworks): Add LangGraph features like persistence, memory and streaming to other agent frameworks that do not provide them out of the box.
