...
For conceptual information, see [Graph API overview](lc:oss/python/langgraph/graph-api) and [Functional API overview](lc:oss/python/langgraph/functional-api).


> [!NOTE]
>
> For this example, you will need to set up a [Claude (Anthropic)](https://www.anthropic.com/) account and get an API key. Then, set the `ANTHROPIC_API_KEY` environment variable in your terminal. See [chat model integrations](lc:oss/python/integrations/chat/index) for all available providers. If you use [LangSmith Gateway](lc:langsmith/llm-gateway), you can [bring your own provider keys](lc:langsmith/llm-gateway-quickstart) or use [Gateway Credits](lc:langsmith/llm-gateway-credits) to access models without a provider key.


#### Tab: Use the Graph API
...
> [!TIP]
>
> Trace and debug your agent with [LangSmith](https://smith.langchain.com). Follow the [tracing quickstart](lc:langsmith/trace-with-langgraph) to get set up. When ready for production, see [Deploy](lc:langsmith/deployment) for hosting options.
>
