...
>
> Sign up for [LangSmith](https://smith.langchain.com) to quickly spot issues and improve the performance of your LangGraph projects. LangSmith lets you use trace data to debug, test, and monitor your LLM apps built with LangGraph—read more about how to get started in the [docs](lc:langsmith/observability).


## Define and update state

Here we show how to define and update [state](lc:oss/python/langgraph/graph-api#state) in LangGraph. We will demonstrate:

1. How to use state to define a graph's [schema](lc:oss/python/langgraph/graph-api#schema)
2. How to use [reducers](lc:oss/python/langgraph/graph-api#reducers) to control how state updates are processed.

### Define state

[State](lc:oss/python/langgraph/graph-api#state) in LangGraph can be a `TypedDict`, `Pydantic` model, or dataclass. Below we will use `TypedDict`. See [Use Pydantic models for graph state](#use-pydantic-models-for-graph-state) for detail on using Pydantic.
...
Let's consider a simple example using [messages](lc:oss/python/langgraph/graph-api#messagesstate). This represents a versatile formulation of state for many LLM applications. See our [concepts page](lc:oss/python/langgraph/graph-api#working-with-messages-in-graph-state) for more detail.
...
Let's build an example graph with a single node. Our [node](lc:oss/python/langgraph/graph-api#nodes) is just a Python function that reads our graph's state and makes updates to it. The first argument to this function will always be the state:
...
Let's next define a simple graph containing this node. We use [`StateGraph`](lc:oss/python/langgraph/graph-api#stategraph) to define a graph that operates on this state. We then use [`add_node`](lc:oss/python/langgraph/graph-api#nodes) populate our graph.
...
![Simple graph with single node](/langchain/images/oss/images/graph_api_image_1.png)
...
Each key in the state can have its own independent [reducer](lc:oss/python/langgraph/graph-api#reducers) function, which controls how updates from nodes are applied. If no reducer function is explicitly specified then it is assumed that all updates to the key should override it.
...
* We may want to accept short-hands for [message formats](lc:oss/python/langgraph/graph-api#using-messages-in-your-graph), such as [OpenAI format](https://python.langchain.com/docs/concepts/messages/#openai-format).
...
See [Fault tolerance](lc:oss/python/langgraph/fault-tolerance#timeouts) for the full timeout lifecycle, idle-timeout refresh sources, and `runtime.heartbeat()`.
...
See [Fault tolerance](lc:oss/python/langgraph/fault-tolerance#error-handling) for compensation patterns and `Command` routing.
...
See [Fault tolerance](lc:oss/python/langgraph/fault-tolerance#graph-defaults) for the full precedence rules and applicability table.
...
| `user` | `BaseUser \| None` | The authenticated user, if [custom auth](lc:langsmith/custom-auth) is configured. |
...
When a [graceful shutdown](lc:oss/python/langgraph/fault-tolerance#graceful-shutdown) has been requested, `runtime.drain_requested` is `True`. Read this inside a node to skip expensive work before the next superstep boundary:
...
> [!NOTE]
>
> Requires `langgraph>=1.2`. See [Graceful shutdown](lc:oss/python/langgraph/fault-tolerance#graceful-shutdown) for the full `RunControl` API.
...
To add a sequence of nodes, we use the `add_node` and `add_edge` methods of our [graph](lc:oss/python/langgraph/graph-api#stategraph):
...
* How state updates are [checkpointed](lc:oss/python/langgraph/persistence)
* How interruptions are resumed in [human-in-the-loop](lc:oss/python/langgraph/interrupts) workflows
* How we can "rewind" and branch-off executions using LangGraph's [time travel](lc:oss/python/langgraph/use-time-travel) features

They also determine how execution steps are [streamed](lc:oss/python/langgraph/streaming), and how your application is visualized and debugged using [Studio](lc:langsmith/studio).
...
Let's first define our [state](lc:oss/python/langgraph/graph-api#state). This governs the [schema of the graph](lc:oss/python/langgraph/graph-api#schema), and can also specify how to apply updates. See [Process state updates with reducers](#process-state-updates-with-reducers) for more detail.
...
Our [nodes](lc:oss/python/langgraph/graph-api#nodes) are just Python functions that read our graph's state and make updates to it. The first argument to this function will always be the state:
...
>
> By default, this will **overwrite** the value of the corresponding key. You can also use [reducers](lc:oss/python/langgraph/graph-api#reducers) to control how updates are processed—for example, you can append successive updates to a key instead. See [Process state updates with reducers](#process-state-updates-with-reducers) for more detail.


Finally, we define the graph. We use [StateGraph](lc:oss/python/langgraph/graph-api#stategraph) to define a graph that operates on this state.

We will then use [`add_node`](lc:oss/python/langgraph/graph-api#messagesstate) and [`add_edge`](lc:oss/python/langgraph/graph-api#edges) to populate our graph and define its control flow.
...
* We must specify the entry point of the graph. For this we add an edge with the [START node](lc:oss/python/langgraph/graph-api#start-node).
* The graph halts when there are no more nodes to execute.

We next [compile](lc:oss/python/langgraph/graph-api#compiling-your-graph) our graph. This provides a few basic checks on the structure of the graph (e.g., identifying orphaned nodes). If we were adding persistence to our application via a [checkpointer](lc:oss/python/langgraph/persistence), it would also be passed in here.
...
![Sequence of steps graph](/langchain/images/oss/images/graph_api_image_2.png)
...
In this example, we fan out from `Node A` to `B and C` and then fan in to `D`. With our state, [we specify the reducer add operation](lc:oss/python/langgraph/graph-api#reducers). This will combine or accumulate values for the specific key in the State, rather than simply overwriting the existing value. For lists, this means concatenating the new list with the existing list. See the above section on [state reducers](#process-state-updates-with-reducers) for more detail on updating state with reducers.
...
![Parallel execution graph](/langchain/images/oss/images/graph_api_image_3.png)
...
> [!NOTE]
>
> In the above example, nodes `"b"` and `"c"` are executed concurrently in the same [superstep](lc:oss/python/langgraph/graph-api#graphs). Because they are in the same step, node `"d"` executes after both `"b"` and `"c"` are finished.
>
...
### Exception handling?
  ...
  LangGraph executes nodes within [supersteps](lc:oss/python/langgraph/graph-api#graphs), meaning that while parallel branches are executed in parallel, the entire superstep is **transactional**. If any of these branches raises an exception, **none** of the updates are applied to the state (the entire superstep errors).

  Importantly, when using a [checkpointer](lc:oss/python/langgraph/persistence), results from successful nodes within a superstep are saved, and don't repeat when resumed.
...
![Deferred execution graph](/langchain/images/oss/images/graph_api_image_4.png)
...
![Conditional branching graph](/langchain/images/oss/images/graph_api_image_5.png)
...
![Map-reduce graph with fanout](/langchain/images/oss/images/graph_api_image_6.png)
...
When creating a graph with a loop, we require a mechanism for terminating execution. This is most commonly done by adding a [conditional edge](lc:oss/python/langgraph/graph-api#conditional-edges) that routes to the [END](lc:oss/python/langgraph/graph-api#end-node) node once we reach some termination condition.

You can also set the graph recursion limit when invoking or streaming the graph. The recursion limit sets the number of [super-steps](lc:oss/python/langgraph/graph-api#graphs) that the graph is allowed to execute before it raises an error. Read more about the [recursion limit concept](lc:oss/python/langgraph/graph-api#recursion-limit).
...
![Simple loop graph](/langchain/images/oss/images/graph_api_image_7.png)


This architecture is similar to a [ReAct agent](lc:oss/python/langgraph/workflows-agents) in which node `"a"` is a tool-calling model, and node `"b"` represents the tools.
...
In some applications, we may not have a guarantee that we will reach a given termination condition. In these cases, we can set the graph's [recursion limit](lc:oss/python/langgraph/graph-api#recursion-limit). This will raise a `GraphRecursionError` after a given number of [supersteps](lc:oss/python/langgraph/graph-api#graphs). We can then catch and handle this exception:
...
### Extended example: loops with branches
  ...
  ![Complex loop graph with branches](/langchain/images/oss/images/graph_api_image_8.png)

  This graph looks complex, but can be conceptualized as loop of [supersteps](lc:oss/python/langgraph/graph-api#graphs):
...
>
> **Async streaming**
> See the [streaming guide](lc:oss/python/langgraph/streaming) for examples of streaming with async.
...
We can now create the `StateGraph` with the above nodes. Notice that the graph doesn't have [conditional edges](lc:oss/python/langgraph/graph-api#conditional-edges) for routing! This is because control flow is defined with `Command` inside `node_a`.
...
![Command-based graph navigation](/langchain/images/oss/images/graph_api_image_11.png)
...
If you are using [subgraphs](lc:oss/python/langgraph/use-subgraphs), you might want to navigate from a node within a subgraph to a different subgraph (i.e. a different node in the parent graph). To do so, you can specify `graph=Command.PARENT` in `Command`:
...
> When you send updates from a subgraph node to a parent graph node for a key that's shared by both parent and subgraph [state schemas](lc:oss/python/langgraph/graph-api#schema), you **must** define a [reducer](lc:oss/python/langgraph/graph-api#reducers) for the key you're updating in the parent graph state. See the example below.
...
![Fractal graph visualization](/langchain/images/oss/images/graph_api_image_10.png)
