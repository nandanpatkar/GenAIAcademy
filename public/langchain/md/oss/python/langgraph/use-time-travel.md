...
LangGraph supports time travel through [checkpoints](lc:oss/python/langgraph/checkpointers#checkpoints):
...
Both work by resuming from a prior checkpoint. Nodes before the checkpoint are not re-executed (results are already saved). Nodes after the checkpoint re-execute, including any LLM calls, API requests, and [interrupts](lc:oss/python/langgraph/interrupts) (which may produce different results).
...
> [!WARNING]
>
> Replay re-executes nodes—it doesn't just read from cache. LLM calls, API requests, and [interrupts](lc:oss/python/langgraph/interrupts) fire again and may return different results. Replaying from the final checkpoint (no `next` nodes) is a no-op.
...
![Fork](/langchain/images/oss/images/checkpoints_full_story.jpg)
...
When you call `update_state`, values are applied using the specified node's writers (including [reducers](lc:oss/python/langgraph/graph-api#reducers)). The checkpoint records that node as having produced the update, and execution resumes from that node's successors.
...
- **No execution history**: Setting up state on a fresh thread (common in [testing](lc:oss/python/langgraph/test)).
...
If your graph uses `interrupt` for [human-in-the-loop](lc:oss/python/langgraph/interrupts) workflows, interrupts are always re-triggered during time travel. The node containing the interrupt re-executes, and `interrupt()` pauses for a new `Command(resume=...)`.
...
Time travel with [subgraphs](lc:oss/python/langgraph/use-subgraphs) depends on whether the subgraph has its own checkpointer. This determines the granularity of checkpoints you can time travel from.

#### Tab: Inherited checkpointer (default)
...
See [subgraph persistence](lc:oss/python/langgraph/use-subgraphs#subgraph-persistence) for more on configuring subgraph checkpointers.
