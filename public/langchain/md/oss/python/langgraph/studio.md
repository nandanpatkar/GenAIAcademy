...
The [LangGraph CLI](lc:langsmith/cli) provides a local development server (also called [Agent Server](lc:langsmith/agent-server)) that connects your agent to Studio.
...
> [!NOTE]
>
> For detailed explanations of each key in the JSON object of the configuration file, refer to the [LangGraph configuration file reference](lc:langsmith/cli#configuration-file).
...
[
 ...
 },
 {
  "label": "uv",
  "lang": "shell",
  "code": "uv add langchain langchain-openai"
 }
]
```
...
> [!WARNING]
>
> Safari blocks `localhost` connections to Studio. To work around this, run the above command with `--tunnel` to access Studio via a secure tunnel. You'll need to manually add the tunnel URL to allowed origins by clicking **Connect to a local server** in the Studio UI. See the [troubleshooting guide](lc:langsmith/troubleshooting-studio#safari-connection-issues) for steps.
...
For more information on how to run Studio, refer to the following guides in the [LangSmith docs](lc:langsmith/observability):
