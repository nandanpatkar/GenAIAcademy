...
State is the shared [memory](lc:oss/python/concepts/memory) accessible to all nodes in your agent. Think of it as the notebook your agent uses to keep track of everything it learns and decides as it works through the process.
...
| Unexpected errors | Developer | Let them bubble up | Unknown issues that need debugging |
    ...
    #### Tab: Transient errors

        Add a retry policy to automatically retry network issues and rate limits.


    Combine with `timeout=` to cap each attempt. See [Fault tolerance](lc:oss/python/langgraph/fault-tolerance) for the full lifecycle.
    ...
    #### Tab: Saga / compensation

        After retries are exhausted, run a recovery function that updates state and routes to a compensation branch.


    See [Fault tolerance](lc:oss/python/langgraph/fault-tolerance#error-handling) for the full pattern.
...
> `error_handler` requires `langgraph>=1.2`.
    ...
    To apply the same `retry_policy`, `timeout`, or `error_handler` to every node in a graph without repeating them on each `add_node`, use `StateGraph.set_node_defaults(...)`. Per-node values still take precedence. See [Fault tolerance](lc:oss/python/langgraph/fault-tolerance#graph-defaults).
...
To enable [human-in-the-loop](lc:oss/python/langgraph/interrupts) with `interrupt()`, we need to compile with a [checkpointer](lc:oss/python/langgraph/persistence) to save state between runs:
...
Performance considerations: More nodes doesn't mean slower execution. LangGraph writes checkpoints in the background by default ([async durability mode](lc:oss/python/langgraph/checkpointers#durability-modes)), so your graph continues running without waiting for checkpoints to complete. This means you get frequent checkpoints with minimal performance impact. You can adjust this behavior if needed—use `"exit"` mode to checkpoint only at completion, or `"sync"` mode to block execution until each checkpoint is written.
