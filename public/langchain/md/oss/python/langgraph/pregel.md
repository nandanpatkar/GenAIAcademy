...
LangGraph provides two high-level APIs for creating a Pregel application: the [StateGraph (Graph API)](lc:oss/python/langgraph/graph-api) and the [Functional API](lc:oss/python/langgraph/functional-api).

    #### Tab: StateGraph (Graph API)
    ...
    In the [Functional API](lc:oss/python/langgraph/functional-api), you can use an `@entrypoint` to create a Pregel application. The `entrypoint` decorator allows you to define a function that takes input and returns output.
