...
Interrupts allow you to pause graph execution at specific points and wait for external input before continuing. This enables human-in-the-loop patterns where you need external input to proceed. When an interrupt is triggered, LangGraph saves the graph state using its [persistence](lc:oss/python/langgraph/persistence) layer and waits indefinitely until you resume execution.
...
- **Interrupt payloads surface via `stream.interrupts`:** when using [event streaming](lc:oss/python/langgraph/event-streaming) (`graph.stream_events(..., version="v3")`), the values you pass to `interrupt()` appear on `stream.interrupts`, and `stream.interrupted` is `True` when the run pauses for input.
...
3. **Value is returned** to the caller on `stream.interrupts` when using [event streaming](lc:oss/python/langgraph/event-streaming) (`graph.stream_events(..., version="v3")`), or under `__interrupt__` with the default `invoke()` API; it can be any JSON-serializable value (string, object, array, etc.)
...
The recommended way to drive a graph that may interrupt is [event streaming](lc:oss/python/langgraph/event-streaming) — it surfaces interrupts via `stream.interrupts` and `stream.interrupted`, and exposes the final state through `stream.output`.
...
> [!WARNING]
>
> `Command(resume=...)` is the **only** `Command` pattern intended as input to `invoke()`/`stream()`/`stream_events()`. The other `Command` parameters (`update`, `goto`, `graph`) are designed for [returning from node functions](lc:oss/python/langgraph/graph-api#command). Do not pass `Command(update=...)` as input to continue multi-turn conversations—pass a plain input dict instead.
...
When building interactive agents with human-in-the-loop workflows, you can use [event streaming](lc:oss/python/langgraph/event-streaming) to consume message chunks and state snapshots concurrently while handling interrupts.
...
[
 {
  "label": "Separating logic",
  "lang": "python",
  "code": "def node_a(state: State):\n    # \u2705 Good: interrupting first, then handling\n    # error conditions separately\n    interrupt(\"What's your name?\")\n    try:\n        fetch_data()  # This can fail\n    except Exception as e:\n        print(e)\n    return state"
 },
 {
  "label": "Explicit exception handling",
  "lang": "python",
...
[
 {
  "label": "Skipping interrupts",
  "lang": "python",
  "code": "def node_a(state: State):\n    # \u274c Bad: conditionally skipping interrupts changes the order\n    name = interrupt(\"What's your name?\")\n\n    # On first run, this might skip the interrupt\n    # On resume, it might not skip it - causing index mismatch\n    if state.get(\"needs_age\"):\n        age = interrupt(\"What's your age?\")\n\n    city = interrupt(\"What's your city?\")\n\n    return {\"name\": name, \"city\": city}"
 },
 {
  "label": "Looping interrupts",
  "lang": "python",
...
> [!TIP]
>
> To debug your interrupts, use [LangSmith](lc:langsmith/observability).


### Using LangSmith Studio

You can use [LangSmith Studio](lc:langsmith/studio) to set static interrupts in your graph in the UI before running the graph. You can also use the UI to inspect the graph state at any point in the execution.

![image](/langchain/images/oss/images/static-interrupt.png)
