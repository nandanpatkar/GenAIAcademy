...
3. [Non-determinism](#non-determinism): Only applies to the [Functional API](lc:oss/python/langgraph/functional-api).


> [!TIP]
>
> For a short summary of which graph topology and state changes the runtime supports by default, see [Graph migrations](lc:oss/python/langgraph/graph-api#graph-migrations). The rest of this page covers the patterns you can apply when a change falls outside that supported set.


## Technical compatibility

Technical compatibility is the equivalent of an API breaking change in a microservice. The "API" here is the contract between your graph code and the data already persisted by the [checkpointer](lc:oss/python/langgraph/checkpointers#checkpointer-libraries) for existing threads. When a thread resumes, LangGraph deserializes the saved state, dispatches it to a node by name, and expects the node to return values that fit the state schema.
...
- Use [time travel](lc:oss/python/langgraph/use-time-travel) and `graph.get_state`[get_state] to spot-check existing threads against the new code in a staging deployment before rolling out.
...
**If you deploy to [LangSmith](lc:langsmith/deployment).** Use the Agent Server's thread search to filter by status. The `status` field accepts `idle`, `busy`, `interrupted`, and `error`, so you can bulk-query for `interrupted` or `busy` threads, optionally narrowed with metadata filters. See [Filter by thread status](lc:langsmith/use-threads#filter-by-thread-status) and [List threads](lc:langsmith/use-threads#list-threads).

**Anywhere LangGraph runs.** Use [LangSmith tracing](lc:oss/python/langgraph/observability) to monitor which nodes are being entered and exited in production. This is the most reliable signal that a node or state field is no longer reachable in any active code path.
...
The recommended pattern is to record the relevant *behavioral version* on the state at thread start, then branch on it with a [conditional edge](lc:oss/python/langgraph/graph-api#conditional-edges):
...
This category only applies to the [Functional API](lc:oss/python/langgraph/functional-api) and to [**tasks**](lc:oss/python/langgraph/functional-api#task) or `interrupt` calls inside a [Graph API](lc:oss/python/langgraph/graph-api) **node**. Plain Graph API **nodes** [re-run from the start of the node function](lc:oss/python/langgraph/graph-api#re-execution-and-idempotency) on resume; design side effects to be idempotent, but you do not need to preserve task call order unless you use **tasks** or `interrupt` in that **node**.
...
For a deeper treatment with examples, see [Determinism](lc:oss/python/langgraph/functional-api#determinism) and [Common pitfalls](lc:oss/python/langgraph/functional-api#common-pitfalls) in the Functional API guide.
