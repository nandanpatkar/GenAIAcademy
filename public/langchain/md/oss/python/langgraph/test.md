...
Note that this guide is LangGraph-specific and covers scenarios around graphs with custom structures - if you are just getting started, check out [Test](lc:oss/python/langchain/test/index) that uses LangChain's built-in `create_agent` instead.
...
For agents made up of larger graphs, you may wish to test partial execution paths within your agent rather than the entire flow end-to-end. In some cases, it may make semantic sense to [restructure these sections as subgraphs](lc:oss/python/langgraph/use-subgraphs), which you can invoke in isolation as normal.
...
2. Call your agent's [`update_state`](lc:oss/python/langgraph/use-time-travel) method with an [`as_node`](lc:oss/python/langgraph/use-time-travel#from-a-specific-node) parameter set to the name of the node *before* the one you want to start your test.
