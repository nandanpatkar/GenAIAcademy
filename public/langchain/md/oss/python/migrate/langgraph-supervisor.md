The [`langgraph-supervisor`](https://github.com/langchain-ai/langgraph-supervisor-py) package is no longer actively maintained. Instead use the [subagents](lc:oss/python/langchain/multi-agent/subagents) pattern: a main agent coordinates specialized workers by calling them as [tools](lc:oss/python/langchain/tools).
...
| `output_mode` for message history | Format subagent output in the tool wrapper (see [subagent outputs](lc:oss/python/langchain/multi-agent/subagents#subagent-outputs)) |
...
For a full walkthrough, see [Build a personal assistant with subagents](lc:oss/python/langchain/multi-agent/subagents-personal-assistant).
...
1. **Compile only the outermost graph with a checkpointer.** Leave subagents without `checkpointer=...` so they use [per-invocation persistence](lc:oss/python/langgraph/use-subgraphs#per-invocation-default) and inherit the parent's checkpointer at runtime.
...
When `preview_tool` calls `interrupt`, the exception bubbles through both `create_agent` layers and surfaces as `__interrupt__` on the outer `StateGraph`'s invoke result. Your existing `Command(resume=result)` callback path keeps working.

For more on how interrupts propagate through subgraphs, see [Subgraph persistence: Interrupts](lc:oss/python/langgraph/use-subgraphs#per-invocation-default) and [Checkpointing and state inspection](lc:oss/python/langchain/multi-agent/subagents#checkpointing-and-state-inspection).
...
If you need static subgraph discovery, checkpoint namespaces per tier, or shared state keys between levels, use a custom `StateGraph` with [subgraph nodes](lc:oss/python/langgraph/use-subgraphs#add-a-subgraph-as-a-node) instead.
...
With the subagents pattern, control this in the tool wrapper. Return only the final message for `last_message` behavior, or return a formatted summary of the full conversation for `full_history` behavior. See [Subagent outputs](lc:oss/python/langchain/multi-agent/subagents#subagent-outputs) for patterns that pass additional state back to the supervisor.

## See also

- [Subagents](lc:oss/python/langchain/multi-agent/subagents): Pattern overview and design decisions
- [Build a personal assistant with subagents](lc:oss/python/langchain/multi-agent/subagents-personal-assistant): Step-by-step supervisor tutorial
- [Use subgraphs](lc:oss/python/langgraph/use-subgraphs): Subgraph persistence, interrupts, and state inspection
- [Interrupts](lc:oss/python/langgraph/interrupts): Pause and resume graph execution
- [LangGraph v1 migration guide](lc:oss/python/migrate/langgraph-v1): Migrate from `create_react_agent` to `create_agent`
