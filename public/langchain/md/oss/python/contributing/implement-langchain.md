...
LangChain components are subclasses of base classes in [`langchain-core`](https://github.com/langchain-ai/langchain/tree/master/libs/core). Examples include [chat models](lc:oss/python/integrations/chat/index), [tools](lc:oss/python/integrations/tools/index), [retrievers](lc:oss/python/integrations/retrievers/index), and more.
...
> [!WARNING]
>
> The chat model integration guide is currently WIP. In the meantime, read the [chat model conceptual guide](lc:oss/python/langchain/models) for details on how LangChain chat models function. You may also refer to existing integrations in the [LangChain repo](https://github.com/langchain-ai/langchain/tree/master/libs/partners)
...
> [!WARNING]
>
> The embedding model integration guide is currently WIP. In the meantime, read the [embedding model conceptual guide](lc:oss/python/integrations/embeddings/index) for details on how LangChain embedding models function.
...
> [!WARNING]
>
> The tools integration guide is currently WIP. In the meantime, read the [tools conceptual guide](lc:oss/python/langchain/tools) for details on how LangChain tools function.


    
    #### Tab: Middleware

        [Middleware](lc:oss/python/langchain/middleware/overview) lets you customize agent behavior by hooking into model calls, tool calls, and agent lifecycle events. Middleware classes subclass the `AgentMiddleware` base class.

        Read the [custom middleware guide](lc:oss/python/langchain/middleware/custom) to understand hooks, state updates, and middleware patterns before building an integration.
...
Full reference for hooks, state updates, and patterns.
    ...
    #### Tab: Checkpointers
        ...
        Checkpointers enable [persistence](lc:oss/python/langgraph/persistence) in LangGraph, allowing agents to save and resume state across interactions.
    ...
    #### Tab: Sandboxes
        ...
        Sandbox integrations enable [Deep Agents](lc:oss/python/deepagents/overview) to run code in isolated environments.
        ...
        Validate your integration with the [sandbox standard test suite](lc:oss/python/contributing/standard-tests-langchain#sandbox-integrations). The Python suite uses `SandboxIntegrationTests` from `langchain_tests.integration_tests`; subclass it and provide a `sandbox` fixture that yields a clean `SandboxBackendProtocol` instance.
