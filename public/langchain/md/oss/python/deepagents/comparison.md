This page explains how [LangChain Deep Agents](lc:oss/python/deepagents/overview) compares to the [Claude Agent SDK](https://platform.anthropic.com/docs/en/agent-sdk/overview). Both are harnesses for building custom agents, but they make different tradeoffs around execution environments, deployment, and vendor coupling.


> [!NOTE]
>
> Deep Agents is used in production by [OpenSWE](https://github.com/langchain-ai/open-swe) and [LangSmith Fleet](lc:langsmith/fleet/index).
...
| **Execution backend** | Pluggable: [local, virtual filesystem, remote sandbox, or custom](lc:oss/python/deepagents/backends) | Local filesystem of the sandbox it runs in |
| **Model provider** | Any (Anthropic, OpenAI, Google, 100+ others) | Claude (Anthropic, Bedrock, Vertex, Azure) |
| **Per-provider/model tuning** | [Harness profiles](lc:oss/python/deepagents/profiles) (beta): declarative bundles of system prompt, tool, middleware, and subagent tweaks, registered per provider or specific model | Configure in code at each model call site |
| **Deployment** | [Managed Deep Agents](lc:langsmith/managed-deep-agents-overview) in LangSmith, or self-host a [standalone image](lc:langsmith/deploy-standalone-server) via [`langgraph build`](lc:langsmith/cli#build) | [Self-host](https://code.claude.com/docs/en/agent-sdk/hosting). You build the server, auth, and streaming layer. [Claude managed agents](https://platform.claude.com/docs/en/managed-agents/overview) is a separate product |
| **Multi-tenancy** | [Built-in](lc:oss/python/deepagents/going-to-production#multi-tenancy): scoped threads, per-user sandboxes, RBAC | Build it yourself |
...
Deep Agents supports both, and lets you pick a [backend](lc:oss/python/deepagents/backends#quickstart) to wire them together. In practice, this means you can:
...
Deep Agents handles this directly: configure a sandbox [per user or per assistant](lc:oss/python/deepagents/going-to-production#lifecycle) in the harness, with scoped threads, run history, and [RBAC](lc:oss/python/deepagents/going-to-production#team-access-control-rbac) included. If you use [LangSmith Sandbox](lc:langsmith/sandbox-auth-proxy), you also get an auth proxy out of the box so end users can call third-party APIs from the sandbox without you provisioning credentials per user.
...
Deep Agents deployments include an [agent server](lc:langsmith/agent-server) out of the box: streaming endpoints, thread management, run history, webhooks, and [authentication](lc:langsmith/auth).
...
- **Managed:** create, run, and operate deep agents with [Managed Deep Agents](lc:langsmith/managed-deep-agents-overview) in LangSmith.
- **Self-hosted:** run [`langgraph build`](lc:langsmith/cli#build) to produce a [standalone Docker image](lc:langsmith/deploy-standalone-server) you can deploy anywhere.


> [!TIP]
>
> For a managed agent platform that works across any model provider, use [LangSmith Fleet](lc:langsmith/fleet/index). [Claude managed agents](https://platform.claude.com/docs/en/managed-agents/overview) is limited to the Anthropic ecosystem.
