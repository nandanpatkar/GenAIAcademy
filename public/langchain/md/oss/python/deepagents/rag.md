...
These applications use a technique known as Retrieval Augmented Generation, or [RAG](lc:oss/python/deepagents/retrieval).

[Deep Agents](lc:oss/python/deepagents/overview) gives you primitives for RAG: custom retrieval tools, a [filesystem backend](lc:oss/python/deepagents/backends), [subagents](lc:oss/python/deepagents/subagents), [skills](lc:oss/python/deepagents/skills), and [grading rubrics](lc:oss/python/deepagents/rubric). You can combine them in different ways depending on your corpus size, latency requirements, and how strictly answers must be grounded in source data.
...
- **Todo-driven investigation**: The user asks a question. If you [opt into task planning](lc:oss/python/deepagents/overview#task-planning), the agent uses the planning tool to create a todo list of documentation pages or search queries to investigate. It retrieves results for each item, then synthesizes a response from the collected evidence.
- **Retrieve, offload, and delegate**: The user asks a question. The agent retrieves matching chunks and writes them to the filesystem backend rather than keeping full text in the orchestrator context. Subagents read, search, and summarize individual files in parallel. For large documents, the agent can paginate through files with built-in search tools or run a [code interpreter](lc:oss/deepagents/code/overview) to produce tables, timelines, or visuals from source data.


> [!NOTE]
>
> Grading rubrics require `deepagents>=0.6.5` and are currently in [beta](lc:langsmith/release-stages).
...
Pass that question to a [Deep Agent](lc:oss/python/deepagents/overview) with no custom tools and no access to the documentation corpus, to see what the model comes up with:
...
Without retrieval, the agent cannot look up current LangChain documentation. Responses tend to be generic, may omit guidance such as [subagent streaming](lc:oss/python/deepagents/frontend/subagent-streaming), or include outdated information.
...
- A [chat model integration](lc:oss/python/integrations/chat/index) for the agent
- OpenAI (or another [embeddings integration](lc:oss/python/integrations/embeddings/index)) for indexing
...
[
 {
  "label": "pip wrap",
  "lang": "bash",
  "code": "pip install deepagents \"langchain[openai]\" langchain-text-splitters requests numpy"
 },
 {
  "label": "uv wrap",
  "lang": "bash",
...
For any other provider, please see the respective [chat model](lc:oss/python/integrations/chat/index) documentation.

#### Step: Set up LangSmith

RAG applications run retrieval and generation in sequence. When you run the examples in this tutorial, [LangSmith](lc:langsmith/observability) logs a trace for each query so you can inspect retrieval, tool calls, and model responses.
...
> [!TIP]
>
> If you are building a production agent, we also recommend you set up [LangSmith Engine](lc:langsmith/engine) which monitors your traces, detects issues, and proposes fixes.
...
2. **[Split](#split-documents)**: Use [text splitters](lc:oss/python/integrations/splitters/index) to break large `Document`s into smaller chunks. This is useful both for indexing data and passing it to a model, as large chunks are harder to search over and either do not fit in a model's finite context window or use more tokens than necessary.
3. **[Embed](#select-an-embeddings-model)**: [Embeddings](lc:oss/python/integrations/embeddings/index) models convert each chunk into a numeric vector that captures its meaning, enabling similarity search over your content.
4. **[Store](#store-chunks-and-embeddings-in-vectorstore)**: Use a [VectorStore](lc:oss/python/integrations/vectorstores/index) to index chunks and their embeddings for retrieval.

![index_diagram](/langchain/images/images/rag_indexing.png)
...
> [!NOTE]
>
> For a more detailed tutorial on indexing, vector stores, and retrieval, see [Semantic search](lc:oss/python/langchain/knowledge-base).
...
If you want to learn more about text splitters, check out the [`TextSplitter` interface](https://reference.langchain.com/python/langchain-text-splitters/base/TextSplitter) and [text splitter integrations](lc:oss/python/integrations/splitters/index).


### Select an embeddings model

An [embedding](lc:oss/python/integrations/embeddings/index) is a numeric vector that captures the meaning of each documentation chunk. An `Embeddings` model converts those chunks into vectors so that similar meanings land close together in vector space, enabling you to retrieve relevant sections when a user asks a question.

You can choose from many different [embedding integrations](lc:oss/python/integrations/embeddings/index) which all use the same `Interface`[Embeddings]:
...
A [`VectorStore`](lc:oss/python/integrations/vectorstores/index) persists document chunks and their embeddings, enabling similarity search to retrieve relevant sections when a user asks a question.
You can choose from many different [vector store integrations](lc:oss/python/integrations/vectorstores/index) which all use the same `Interface`[VectorStore].
...
1. **Retrieve**: Given a user input, relevant splits are retrieved from storage using a [Retriever](lc:oss/python/integrations/retrievers/index).
2. **Generate**: A [model](lc:oss/python/langchain/models) produces an answer using a prompt that includes both the question and the retrieved data.

![retrieval_diagram](/langchain/images/images/rag_retrieval_generation.png)
...
You implemented one RAG pattern with `create_deep_agent`. Combine it with other Deep Agents capabilities or try a different pattern from [RAG patterns](#rag-patterns):


- Add [Skills](lc:oss/python/deepagents/skills) to package retrieval workflows and domain-specific search guidance
- Use [Grading rubrics](lc:oss/python/deepagents/rubric) to verify answers are grounded in retrieved source material
- [Evaluate a RAG application](lc:langsmith/evaluate-rag-tutorial) with LangSmith datasets and evaluators
- Read [Context engineering](lc:oss/python/deepagents/context-engineering) for offloading and subagent isolation strategies
- Deploy your application with [LangSmith Deployment](lc:langsmith/deployment)
