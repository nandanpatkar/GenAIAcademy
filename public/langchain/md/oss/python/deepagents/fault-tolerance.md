...
| User-fixable errors (missing information, unclear instructions) | Human | Pause with `interrupt()` | [Human-in-the-loop](lc:oss/python/deepagents/human-in-the-loop) |
...
```
  ...
  For the full human-in-the-loop guide, see [Human-in-the-loop](lc:oss/python/deepagents/human-in-the-loop).

  

  #### Tab: Provider outage

  If your primary model provider goes down entirely, use `ModelFallbackMiddleware` to switch to an alternative model:
...
For the full configuration, see [Rate limiting](lc:oss/python/langchain/models#rate-limiting).
...
For the full configuration options and usage patterns, including async handlers and composing with retry middleware, see [Prebuilt middleware](lc:oss/python/langchain/middleware/built-in#tool-error).
