...
On the frontend, connect with `useStream` the same way as with `createAgent`. Pass a [type parameter](lc:oss/python/langchain/frontend/overview) for type-safe stream state. Deep agent patterns use `stream.subagents`, selector helpers such as `useMessages(stream, subagent)`, and custom state values like `stream.values.todos` to render subagent-specific UIs.
...
The [LangChain frontend patterns](lc:oss/python/langchain/frontend/overview), including
...
[LangGraph frontend patterns](lc:oss/python/langgraph/frontend/overview). They show how
