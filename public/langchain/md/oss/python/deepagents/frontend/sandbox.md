...
agent to a [sandbox](lc:oss/python/deepagents/sandboxes) so it can read,
...
`useStream` configuration, see [Going to production](lc:oss/python/deepagents/going-to-production).
...
See [Sandbox lifecycle](lc:oss/python/deepagents/going-to-production#lifecycle) for thread-scoped
vs assistant-scoped sandboxes, async [graph factory](lc:langsmith/graph-rebuild)
...
[thread](lc:langsmith/use-threads) ID. That keeps conversations isolated and
...
For [multi-tenant](lc:oss/python/deepagents/going-to-production#multi-tenancy) apps,
...
Configure the deep agent with a [sandbox backend](lc:oss/python/deepagents/sandboxes)
as described in [Execution environment](lc:oss/python/deepagents/going-to-production#execution-environment).
...
> [!NOTE]
>
> Similar to the example in [Going to production](lc:oss/python/deepagents/going-to-production#lifecycle), the
...
`upload_files`. See [File transfers](lc:oss/python/deepagents/going-to-production#file-transfers)
for seeding patterns, provider examples, and syncing
[memories](lc:oss/python/deepagents/memory) or [skills](lc:oss/python/deepagents/skills) into
the sandbox. For LangSmith sandboxes, pass `templateName` from a
[sandbox snapshot](lc:langsmith/sandbox-snapshots) when creating the container.
...
See [application structure](lc:oss/python/langgraph/application-structure) and
[LangSmith Deployments](lc:oss/python/deepagents/going-to-production#langsmith-deployments)
...
[LangSmith Deployment](lc:langsmith/deployment), and pass a stable
`thread_id` on each run. See
[Frontend](lc:oss/python/deepagents/going-to-production#frontend) in
[Going to production](lc:oss/python/deepagents/going-to-production) for those settings
and for [invoking the agent](lc:oss/python/deepagents/going-to-production#invoking-the-agent)
...
- **Use thread-scoped sandboxes** for production apps. See
  [Sandbox lifecycle](lc:oss/python/deepagents/going-to-production#lifecycle).
...
- **Seed the sandbox with a real project**. See
  [File transfers](lc:oss/python/deepagents/going-to-production#file-transfers).
- **Keep secrets out of the sandbox**. Use the
  [sandbox auth proxy](lc:oss/python/deepagents/going-to-production#managing-secrets)
  instead of environment variables or file uploads for API keys.
- **Add guardrails before launch**. Configure
  [rate limits](lc:oss/python/deepagents/fault-tolerance#rate-limiting),
  [error handling](lc:oss/python/deepagents/fault-tolerance#error-handling), and
  [data privacy](lc:oss/python/deepagents/going-to-production#data-privacy) middleware
