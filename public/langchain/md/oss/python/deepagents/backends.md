...
> [!TIP]
>
> When you deploy on [LangSmith Deployment](lc:langsmith/deployment), a store is provisioned automatically. Use [LangSmith](lc:langsmith/observability) tracing to debug file paths, permission denials, and cross-thread storage. Follow the [observability quickstart](lc:langsmith/observability-quickstart) to get set up.
>
...
> [!TIP]
>
> To generate a durable repository wiki that agents can read through filesystem tools, see [OpenWiki](lc:oss/openwiki/overview).
...
| [Sandbox](lc:oss/python/deepagents/sandboxes) | `agent = create_deep_agent(model="google_genai:gemini-3.6-flash", backend=sandbox)` <br></br>Execute code in isolated environments. Sandboxes provide filesystem tools plus the `execute` tool for running shell commands. Choose from LangSmith, AgentCore, Daytona, Deno, E2B, Modal, Runloop, or local VFS. |
...
> - Web servers or HTTP APIs - use `StateBackend`, `StoreBackend`, or a [sandbox backend](lc:oss/python/deepagents/sandboxes) instead
>
> **Security risks:**
...
> 1. Enable [Human-in-the-Loop (HITL) middleware](lc:oss/python/deepagents/human-in-the-loop) to review sensitive operations.
> 1. Exclude secrets from accessible filesystem paths (especially in CI/CD).
> 1. Use a [sandbox backend](lc:oss/python/deepagents/sandboxes) for production environments requiring filesystem interaction.
...
>
> **Recommended safeguards:**
> 1. Enable [Human-in-the-Loop (HITL) middleware](lc:oss/python/deepagents/human-in-the-loop) to review and approve operations before execution. This is **strongly recommended**.
> 2. Run in dedicated development environments only. Never use on shared or production systems.
> 3. Use a [sandbox backend](lc:oss/python/deepagents/sandboxes) for production environments requiring shell execution.
>
...
- When you're deploying your agent through [LangSmith Deployment](lc:langsmith/deployment) (a store is automatically provisioned for your agent).
...
> [!NOTE]
>
> When no namespace factory is provided, the legacy default uses the `assistant_id` from LangGraph config metadata. This means all users of the same [assistant](lc:langsmith/assistants) share the same storage. For multi-user [going to production](lc:oss/python/deepagents/going-to-production), always provide a namespace factory.
...
> [!NOTE]
>
> **Before you begin:** `ContextHubBackend` requires a Context Hub repo set up in LangSmith. Read the [Context Hub concepts](lc:langsmith/context-engineering-concepts) page first if you're unfamiliar with agent repos and skill repos.
...
This means your agent's context is intentionally spread across repos: one repo per agent, separate repos per skill. That separation lets skills be versioned, shared, and reused across multiple agents independently. If this feels fragmented, see [Linked repos](lc:langsmith/context-engineering-concepts#linked-repos) for the rationale.
...
Implement a custom backend to connect Deep Agents to storage systems such as databases, object stores, and remote filesystems. See [community-built backends](lc:oss/python/integrations/backends/index) for examples.

### Implement the backend protocol

Subclass `BackendProtocol` and implement the following methods:
...
Use [permissions](lc:oss/python/deepagents/permissions) to declaratively control which files and directories the agent can read or write. Permissions apply to the built-in filesystem tools and are evaluated before the backend is called.
...
For the full set of options including rule ordering, subagent permissions, and composite backend interactions, see the [permissions guide](lc:oss/python/deepagents/permissions).
