Memory lets your agent learn and improve across conversations. Deep Agents makes memory first class with filesystem-backed memory: the agent reads and writes memory as files, and you control where those files are stored using [backends](lc:oss/python/deepagents/backends).


> [!TIP]
>
> To generate a repository wiki that coding agents discover through [`AGENTS.md`](https://agents.md/), see [OpenWiki](lc:oss/openwiki/overview).


> [!NOTE]
>
> This page covers **long-term memory**: memory that persists across conversations. For short-term memory (conversation history and scratch files within a single session), see the [context engineering](lc:oss/python/deepagents/context-engineering) guide. Short-term memory is managed automatically as part of the agent's [state](lc:oss/python/langgraph/graph-api#state).
>
...
1. **Point the agent at memory files.** Pass file paths to `memory=` when creating the agent. You can also pass [skills](lc:oss/python/deepagents/skills) via `skills=` for procedural memory (reusable instructions that tell the agent *how* to perform a task). A [backend](lc:oss/python/deepagents/backends) controls where files are stored and who can access them.
2. **Agent reads memory.** The agent can load memory files into the system prompt at startup, or read them on demand during the conversation. For example, [skills](lc:oss/python/deepagents/skills) use on-demand loading: the agent reads only skill descriptions at startup, then reads the full skill file only when it matches a task. This keeps context lean until a capability is needed.
3. **Agent updates memory (optional).** When the agent learns new information, it can use its built-in `edit_file` tool to update memory files. Updates can happen during the conversation (the default) or in the background between conversations via [background consolidation](#background-consolidation). Changes are persisted and available in the next conversation. Not all memory is writable: developer-defined [skills](lc:oss/python/deepagents/skills) and [organization policies](#organization-level-memory) are typically read-only. See [read-only vs writable memory](#read-only-vs-writable-memory) for details.
...
Give the agent its own persistent identity that evolves over time. Agent-scoped memory is shared across all users, so the agent builds up its own persona, accumulated knowledge, and learned preferences through every conversation. As it interacts with users, it develops expertise, refines its approach, and remembers what works. It can also learn and update [skills](lc:oss/python/deepagents/skills) when it has write access.
...
Give each user their own memory file. The agent remembers preferences, context, and history per user while core agent instructions stay fixed. Users can also have per-user [skills](lc:oss/python/deepagents/skills) if stored in a user-scoped backend.
...
|---|---|---|
| **Duration** | How long does it last? | [Short-term](lc:oss/python/deepagents/context-engineering) (single conversation) or [long-term](#scoped-memory) (across conversations) |
| **Information type** | What kind of information is it? | [Episodic](#episodic-memory) (past experiences), [procedural](lc:oss/python/deepagents/skills) (instructions and skills), or [semantic](lc:oss/python/concepts/memory#semantic-memory) (facts) |
...
| **Retrieval** | How are memories read? | Loaded into prompt (default) or on demand (e.g., [skills](lc:oss/python/deepagents/skills)) |
...
Deep Agents already use [checkpointers](lc:oss/python/langgraph/checkpointers#checkpoints) which is the mechanism that supports episodic memory: every conversation is persisted as a checkpointed thread.
...
Use [permissions](lc:oss/python/deepagents/permissions) to enforce that org-level memory is read-only, or [policy hooks](lc:oss/python/deepagents/backends#add-policy-hooks) for custom validation logic.
...
A [cron job](lc:langsmith/cron-jobs) runs the consolidation agent on a fixed schedule. The agent searches recent conversations and synthesizes them into memory. Match the schedule to your usage patterns so consolidation runs roughly track real activity.
...
> [!NOTE]
>
> All cron schedules are interpreted in **UTC**. See [cron jobs](lc:langsmith/cron-jobs) for details on managing and deleting cron jobs.
...
For more on deploying agents with background processes, see [going to production](lc:oss/python/deepagents/going-to-production).
...
|---|---|---|
| **Read-write** (default) | User preferences, agent self-improvement, learned [skills](lc:oss/python/deepagents/skills) | Agent updates files via `edit_file` tool |
| **Read-only** | Organization policies, compliance rules, shared knowledge bases, developer-defined [skills](lc:oss/python/deepagents/skills) | Populate via application code or the [Store API](lc:langsmith/custom-store). Use [permissions](lc:oss/python/deepagents/permissions) to deny writes to specific paths, or [policy hooks](lc:oss/python/deepagents/backends#add-policy-hooks) for custom validation logic. |
...
- Add **human-in-the-loop** validation before the agent writes to shared memory. Use an [interrupt](lc:oss/python/langgraph/interrupts) to require human approval for writes to sensitive paths.

To enforce read-only memory, use [permissions](lc:oss/python/deepagents/permissions) to declaratively deny writes to specific paths. For custom validation logic (rate limiting, audit logging, content inspection), use [backend policy hooks](lc:oss/python/deepagents/backends#add-policy-hooks).
...
> [!TIP]
>
> Use [LangSmith tracing](lc:langsmith/trace-with-langgraph) to audit what your agent writes to memory. Every file write appears as a tool call in the trace.


## See also

- [OpenWiki](lc:oss/openwiki/overview): Generate and maintain repository wikis that coding agents find through `AGENTS.md`
- [Backends](lc:oss/python/deepagents/backends): Choose where memory files are stored
- [Context engineering](lc:oss/python/deepagents/context-engineering): Short-term memory, offloading, and summarization
- [Skills](lc:oss/python/deepagents/skills): On-demand procedural memory
