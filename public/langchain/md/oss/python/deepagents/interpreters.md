...
Where [sandboxes](lc:oss/python/deepagents/sandboxes) are a code-first way for acting on an environment (such as running commands, installing dependencies, and editing files), interpreters are a code-first way for composing tools, preserving state, and deciding what information should return to the model.


> [!WARNING]
>
> Interpreters are in [**beta**](lc:oss/python/versioning). APIs and lifecycle behavior may change between releases.
...
Use [sandboxes](lc:oss/python/deepagents/sandboxes) for code against an environment: shell commands, package installs, tests, filesystem edits, and OS-level execution.
...
| Many independent units of work, multiple perspectives, or recursive analysis over large inputs | Interpreter with [dynamic subagents](lc:oss/python/deepagents/dynamic-subagents) |
| Shell commands, package installs, tests, or full OS filesystem access | [Sandboxes](lc:oss/python/deepagents/sandboxes) |
...
- **Subagents**, through [dynamic subagents](lc:oss/python/deepagents/dynamic-subagents). When the agent has subagents configured, the interpreter exposes a `task()` global for dispatching them from code.
...
The following overview below covers when to use dynamic subagents and a minimal `task()` pattern. For configuration, orchestration examples, workflow triggers, and safety notes, see [Dynamic subagents](lc:oss/python/deepagents/dynamic-subagents).

Dynamic subagents let the interpreter dispatch configured [subagents](lc:oss/python/deepagents/subagents) from code using the built-in `task()` global. A task that spans many independent units, such as reviewing every file in a directory or triaging a batch of tickets, becomes a loop that fans out work and synthesizes the results.
...
Because interpreter snapshots are stored in graph state, a [checkpointer](lc:oss/python/langgraph/checkpointers) also captures them in checkpoint history. Add one when you need durable threads or [time travel](lc:oss/python/langgraph/use-time-travel):
...
| Filesystem access | No | Add the [built-in filesystem tools](lc:oss/python/deepagents/overview#virtual-filesystem-access) via the PTC allowlist |
...
| Shell commands, package installs, tests, OS-level execution | No | Use a [sandbox backend](lc:oss/python/deepagents/sandboxes) |
