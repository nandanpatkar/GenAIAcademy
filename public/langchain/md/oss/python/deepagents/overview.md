...
See the [Quickstart](lc:oss/python/deepagents/quickstart) and [Customization guide](lc:oss/python/deepagents/customization) to get started building your own agents and applications with Deep Agents.


> [!TIP]
>
> Trace requests, debug agent behavior, and evaluate outputs with [LangSmith](https://smith.langchain.com). Follow the [observability quickstart](lc:langsmith/observability-quickstart) to get set up. When ready for production, see [Going to production](lc:oss/python/deepagents/going-to-production) for LangSmith deployment options.
...
Deep Agents is an ["agent harness"](lc:oss/python/concepts/products#agent-harnesses-like-the-deep-agents-sdk). It is the same core tool calling loop as other agent frameworks, but with built-in capabilities that make agents reliable for real tasks:
...
[`deepagents`](https://pypi.org/project/deepagents/) is a standalone library built on top of [LangChain](lc:oss/python/langchain/overview)'s core building blocks for agents. It uses the [LangGraph](lc:oss/python/langgraph/overview) runtime for durable execution, streaming, human-in-the-loop, and other features.


[LangChain](lc:oss/python/langchain/overview) is the framework that provides the core building blocks for your agents.
To learn more about the differences between LangChain, LangGraph, and Deep Agents, see [Frameworks, runtimes, and harnesses](lc:oss/python/concepts/products). For a side-by-side comparison with Anthropic's harness, see [Deep Agents vs. Claude Agent SDK](lc:oss/python/deepagents/comparison).

For building custom agents without these built-in capabilities, consider using LangChain's [`create_agent`](lc:oss/python/langchain/agents) or building a custom [LangGraph](lc:oss/python/langgraph/overview) workflow.
...
Pass custom functions, LangChain tools, or tools from any [MCP server](lc:oss/python/deepagents/tools#mcp-tools) with the `tools=` parameter. Deep Agents fully support the [Model Context Protocol (MCP)](lc:oss/python/langchain/mcp), letting you connect to databases, APIs, file systems, and more through a standard interface.
...
For more information on defining custom tools, using MCP servers, and the full list of built-in harness tools, see [Tools](lc:oss/python/deepagents/tools).

### Virtual filesystem access

The harness provides a configurable virtual filesystem which can be backed by different [pluggable backends](lc:oss/python/deepagents/backends): in-memory state, local disk, LangGraph store, composite routing, or a custom backend with [permission rules](lc:oss/python/deepagents/permissions) for read and write access.
...
| `execute` | Run shell commands in the environment (available with [sandbox backends](lc:oss/python/deepagents/sandboxes) only) |
...
| Type | Extensions |
| ---- | ---------- |
| [Image](lc:oss/python/langchain/messages#multimodal) | `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, `.heic`, `.heif` |
| [Video](lc:oss/python/langchain/messages#multimodal) | `.mp4`, `.mpeg`, `.mov`, `.avi`, `.flv`, `.mpg`, `.webm`, `.wmv`, `.3gpp` |
| [Audio](lc:oss/python/langchain/messages#multimodal) | `.wav`, `.mp3`, `.aiff`, `.aac`, `.ogg`, `.flac` |
| [File](lc:oss/python/langchain/messages#multimodal) | `.pdf`, `.ppt`, `.pptx` |

### Running without the default filesystem tools

    To hide the filesystem tools listed above from the model, register a [harness profile](lc:oss/python/deepagents/profiles#harness-profiles) with `excluded_tools`:
    ...
    Removing `FilesystemMiddleware` itself via `excluded_middleware` is intentionally rejected—it is required scaffolding in the [Deep Agents stack](lc:oss/python/deepagents/customization#deep-agents-stack). Use `excluded_tools` to hide only the model-visible tool surface and leave the middleware in place. To remove the `task` tool, see [Running without subagents](lc:oss/python/deepagents/subagents#running-without-subagents).
...
For more information, see [backends](lc:oss/python/deepagents/backends). To generate a durable repository wiki that agents can read from the filesystem, see [OpenWiki](lc:oss/openwiki/overview).
...
Permissions do not apply to [sandbox backends](lc:oss/python/deepagents/sandboxes), which support arbitrary command execution via the `execute` tool. For custom validation logic, use [backend policy hooks](lc:oss/python/deepagents/backends#add-policy-hooks).

For the full rule structure, examples, and subagent inheritance, see [Permissions](lc:oss/python/deepagents/permissions).

### Code execution

Deep Agents supports code execution in two ways:

- [Sandbox backends](lc:oss/python/deepagents/sandboxes) expose an `execute` tool for shell commands in an isolated environment.
- [Interpreters](lc:oss/python/deepagents/interpreters) add an `eval` tool that runs JavaScript in a scoped QuickJS runtime.
...
For sandbox setup, providers, and file transfer APIs, see [Sandboxes](lc:oss/python/deepagents/sandboxes). For the QuickJS runtime and programmatic tool calling, see [Interpreters](lc:oss/python/deepagents/interpreters).

### Streaming

[Event streaming](lc:oss/python/deepagents/event-streaming) exposes agent runs as typed projections for messages, tool calls, values, and output. Deep Agents add `stream.subagents` so each delegated task gets its own handle with independent message, tool-call, and nested subagent streams.
...
For more information, see [Skills](lc:oss/python/deepagents/skills).
...
For configuration details and examples, see [Memory](lc:oss/python/deepagents/customization#memory). To generate a repository wiki that coding agents discover through `AGENTS.md`, see [OpenWiki](lc:oss/openwiki/overview).
...
For configuration details, see [Context engineering](lc:oss/python/deepagents/context-engineering). For multimodal inputs and tool outputs, see [Multimodal](lc:oss/python/deepagents/multimodal).
...
For other providers, see [Middleware integrations](lc:oss/python/integrations/middleware/index) for available provider-specific caching middleware.
...
- UIs that stream progress from agent state (see [Todo list](lc:oss/python/deepagents/frontend/todo-list))
...
For configuration options and behavior details, see [To-do list](lc:oss/python/langchain/middleware/built-in#to-do-list).
...
- **Configurable strategy**: Use the [default `general-purpose` subagent](lc:oss/python/deepagents/subagents#default-subagent) (enabled by default) or define [custom subagents](lc:oss/python/deepagents/subagents#custom-subagents).
...
### Running without subagents (no `task` tool)
    ...
    To run an agent without the `task` tool, see [Running without subagents](lc:oss/python/deepagents/subagents#running-without-subagents). Do not try removing `SubAgentMiddleware` via `excluded_middleware`—that is intentionally rejected. Instead, disable the auto-added subagent via the [harness profile](lc:oss/python/deepagents/profiles#harness-profiles) and pass no synchronous subagents via `subagents=`. Async subagents are unaffected. See the [full stack](lc:oss/python/deepagents/customization#full-stack) for the complete ordering.
