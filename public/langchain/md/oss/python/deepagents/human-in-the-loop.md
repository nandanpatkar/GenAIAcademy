Some tool operations may be sensitive and require human approval before execution. Deep Agents support human-in-the-loop workflows through LangGraph's interrupt capabilities. You can configure which tools require approval using the `interrupt_on` parameter. When `interrupt_on` is set, `HumanInTheLoopMiddleware` is added to the [Deep Agents stack](lc:oss/python/deepagents/customization#deep-agents-stack). If a run is cancelled or interrupted before a tool returns a result, `PatchToolCallsMiddleware` in the same stack repairs the message history automatically.
...
See the [LangChain human-in-the-loop documentation](lc:oss/python/langchain/human-in-the-loop#conditional-interrupts) for additional configuration options and examples.
