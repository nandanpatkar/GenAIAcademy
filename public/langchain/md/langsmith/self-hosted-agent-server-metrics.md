The [Agent Server](lc:langsmith/agent-server) emits metrics through an OpenTelemetry (OTel) client. Metrics use the `lg_api_` name prefix by default (override with `METRIC_PREFIX`).
...
- [Self-hosted overview](lc:langsmith/deploy-to-self-hosted-overview)
- [Configure Agent Server for scale](lc:langsmith/agent-server-scale)
- [Troubleshooting for self-hosted deployments](lc:langsmith/diagnostics-self-hosted)
- [Agent Server changelog](lc:langsmith/agent-server-changelog)
