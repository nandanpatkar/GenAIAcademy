Managed Deep Agents turns a local [project directory](lc:langsmith/managed-deep-agents-cli#project-file-reference) into a hosted LangGraph deployment. Knowing what the `mda` CLI compiles, what a deploy creates, and which parts the runtime owns helps you reason about behavior, secrets, and state.
...
`mda dev` runs the compiled app in LangSmith Studio so you can test it. `mda deploy` validates the project, syncs deploy-owned context to Context Hub, uploads the build, triggers a hosted build, and reconciles any cron schedules once the deployment is live. For secrets routing, deploy flags, and operational tips, see [Deploy an agent](lc:langsmith/managed-deep-agents-deploy). For the full step list and flags, see the [CLI reference](lc:langsmith/managed-deep-agents-cli#deploy-projects).
...
Each deployment has a [Context Hub](lc:langsmith/use-the-context-hub) repo that stores deploy-owned context and runtime-created memory:
...
Edit instructions and skills in your project and redeploy. Memory is runtime-owned, so deploy preserves `memories/**` instead of overwriting it. For more information about hot/cold tiers, identity remounts, and local `.mda/__contexthub__`, see [Memory](lc:langsmith/managed-deep-agents-memory).
...
When you declare [identity](lc:langsmith/managed-deep-agents-identity), Managed Deep Agents scopes threads and remounts the matching memory slice for the authenticated actor or tenant so callers cannot open each other's conversations or memory. Without identity, the deployment uses shared agent memory.

Scheduled runs choose their thread behavior explicitly. An ephemeral thread is cleaned up after the run, while a persistent thread reuses a stable thread ID so state accumulates. For the thread modes and when to use each, see [Schedules](lc:langsmith/managed-deep-agents-schedules).

## Sandboxes

A [sandbox](lc:langsmith/sandboxes) gives the agent an isolated environment for code execution and filesystem work. Configure one by exporting `sandbox` from `sandbox/index.ts` or `sandbox/__init__.py`, and use `sandbox/setup.sh` to provision it the first time it is created. Sandboxes default to one per thread; set `scope` to `agent` to share one across the agent process. Connectors can also provision files, CLIs, and credentials when a sandbox starts. For configuration options and examples, see [Configure a sandbox](lc:langsmith/managed-deep-agents-deploy#configure-a-sandbox).
...
For authoring, defaults, and provider setup, see [Connectors](lc:langsmith/managed-deep-agents-connectors/index).
...
- [Overview](lc:langsmith/managed-deep-agents-overview): when to use Managed Deep Agents and beta limits.
- [Identity](lc:langsmith/managed-deep-agents-identity): authenticate callers and scope threads and memory.
- [Memory](lc:langsmith/managed-deep-agents-memory): persist preferences across threads with Context Hub `/memories`.
- [Evals](lc:langsmith/managed-deep-agents-evals): compile a Harbor handoff and run Harbor-style tasks.
- [Connectors](lc:langsmith/managed-deep-agents-connectors/index): load MCP tools, expose LangSmith capabilities, and prepare GitHub sandboxes.
- [Channels](lc:langsmith/managed-deep-agents-channels/index): receive Slack or GitHub events and reply from messaging channels.
- [Deploy an agent](lc:langsmith/managed-deep-agents-deploy): the full deploy workflow, secrets, and troubleshooting.
- [CLI reference](lc:langsmith/managed-deep-agents-cli): every `mda` command, flag, and project file rule.
