This tutorial shows you how to collect user feedback for [Agent Server](lc:langsmith/agent-server) runs and automatically link them to [traces](lc:langsmith/observability-concepts#traces) in LangSmith. When creating a run, include the keys in the `feedback_keys` field of the request body. The response will return a pre-signed URL for each key, which your client can use to collect user feedback for the Agent Server run.

LangSmith uses feedback to continuously improve the implementation of your agent. To learn more about how feedback works in LangSmith, refer to [LangSmith feedback](lc:langsmith/observability-concepts#feedback).
...
2. The `feedback` object from the response contains a pre-signed URL for each key. For example, the `feedback` object is:
    ...
    ```
3. Request the returned URL (e.g. `POST /api/v1/feedback/tokens/{token_id}`) to associate the feedback key with the trace generated from the Agent Server run. For more details, refer to the [LangSmith API reference](lc:langsmith/smith-api-ref).
...
When the user chooses a feedback option, `POST` to the corresponding URL. `GET` is also supported. See the [LangSmith API reference](lc:langsmith/smith-api-ref) for more details.
