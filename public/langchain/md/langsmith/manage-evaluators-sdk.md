Use the LangSmith SDK to create and manage [evaluators](lc:langsmith/evaluation-concepts#evaluators) programmatically. Evaluators created through the SDK are [workspace-level](lc:langsmith/administration-overview#workspaces) resources that appear in the **Evaluators** table in the [LangSmith UI](https://smith.langchain.com), the same as [evaluators created in the UI](lc:langsmith/evaluators#create-an-evaluator-in-the-ui). You can attach them to datasets to run [offline evaluations](lc:langsmith/evaluation-concepts#offline-evaluations) and to tracing projects to run [online evaluations](lc:langsmith/evaluation-concepts#online-evaluations). Use the SDK to automate evaluator management and integrate evaluation into your existing workflows.
...
The examples on this page initialize the client with no arguments, so it reads the `LANGSMITH_API_KEY` and `LANGSMITH_ENDPOINT` environment variables. Configure your [API key](lc:langsmith/create-account-api-key) through environment variables rather than hardcoding it.
...
An LLM-as-a-judge evaluator references a prompt from the [prompt hub](lc:langsmith/prompt-engineering-quickstart) and maps your run or example fields to the prompt variables.
...
- [Manage evaluators](lc:langsmith/evaluators): View and manage evaluators in the LangSmith UI.
- [Set up LLM-as-a-judge online evaluators](lc:langsmith/online-evaluations-llm-as-judge): Configure LLM-as-a-judge online evaluators in the LangSmith UI.
- [Set up online code evaluators](lc:langsmith/online-evaluations-code): Configure online code evaluators in the LangSmith UI.
