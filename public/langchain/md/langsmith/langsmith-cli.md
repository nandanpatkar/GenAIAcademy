The LangSmith CLI is a command-line tool for querying and managing your LangSmith data. It's designed for both developers and AI coding agents and outputs JSON by default for scripting, with a `--format pretty` option for human-readable tables. Use it when you need scriptable access to your LangSmith data, such as bulk exports, automation, or giving a coding agent direct access to your [traces, runs, and datasets](lc:langsmith/observability-concepts).
...
[
 {
  "label": "macOS / Linux (recommended)",
  "lang": "bash",
  "code": "curl -fsSL https://cli.langsmith.com/install.sh | sh"
 },
 {
  "label": "Windows",
  "lang": "powershell",
  "code": "irm https://cli.langsmith.com/install.ps1 | iex"
 },
 {
  "label": "Homebrew",
  "lang": "bash",
  "code": "brew install langchain-ai/tap/langsmith-cli"
 },
 {
  "label": "Scoop",
  "lang": "powershell",
  "code": "scoop install langsmith-cli"
 },
 {
  "label": "GitHub Releases",
  "lang": "bash",
  "code": "# Download the latest binary for your platform:\n# https://github.com/langchain-ai/langsmith-cli/releases"
 },
 {
  "label": "Go install",
  "lang": "bash",
  "code": "go install github.com/langchain-ai/langsmith-cli/cmd/langsmith@latest"
 }
]
```
...
> [!NOTE]
>
> Querying a [SmithDB](lc:langsmith/smithdb-sdk-migration)-backed deployment requires LangSmith CLI `v0.2.44` or later.
...
This opens a browser-based authorization flow and stores OAuth tokens in `~/.langsmith/config.json` under the selected [profile](lc:langsmith/profile-configuration). Select a profile with `--profile` or `LANGSMITH_PROFILE`:
...
For the full profile configuration reference, see [Profile configuration](lc:langsmith/profile-configuration).

You can also authenticate with an API key directly.

Set your [API key](lc:langsmith/create-account-api-key) as an environment variable:
...
If you're using LangSmith [self-hosted](lc:langsmith/self-hosted), also set the endpoint:
...
See [Sandbox CLI](lc:langsmith/sandbox-cli) for the full sandbox command reference.
