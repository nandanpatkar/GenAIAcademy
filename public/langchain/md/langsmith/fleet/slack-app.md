...
### Step 2. Add a Slack app to an agent
  ...
  #### Step: Add Slack to the agent

    In [Fleet](https://smith.langchain.com/agents), select your agent and open its configuration sidebar. Expand the **Channels** drawer and select **Slack**. Click **Add to Slack**. If you have not connected Slack yet, Fleet runs the [manager authorization](#step-1-connect-the-fleet-slack-manager) first.

    (Optional) Expand **Advanced** and enable **Allow bot triggers** to let messages from other Slack bots start a run.
  
  #### Step: Confirm the install
...
Every self-hosted instance needs this one-time setup before agents can use Slack. It enables [Slack tools](#add-slack-tools) for your agents and turns on the Slack channel, which is what lets you add custom apps in Step 2.
  ...
  #### Step: Configure Helm
    ...
    Choose a provider ID, for example `slack-oauth-provider`. Add it to your [`langsmith_config.yaml`](lc:langsmith/kubernetes#configure-your-helm-charts), along with the organization that holds your OAuth providers, and deploy:
  ...
  #### Step: Create a Slack app for the provider
  ...
  #### Step: Register the provider in LangSmith
  ...
  #### Step: Add the redirect URI to Slack
...
Start this flow from the agent you want the bot to run, so Fleet links the app for you when you finish.
  ...
  #### Step: Open the Slack setup dialog
...
> Do not create the Slack app outside this flow. The generated manifest sets the event subscription URL, interactivity URL, OAuth redirect URL, and scopes that Fleet needs. An app created by hand does not receive events.
  ...
  #### Step: Enter your app credentials
...
For more information, see [Human-in-the-loop](lc:langsmith/fleet/essentials#human-in-the-loop).
...
> [!NOTE]
>
> Set each tool to **Auto** to run it without approval, or **Ask** to require approval before it runs. For more information, see [Human-in-the-loop](lc:langsmith/fleet/essentials#human-in-the-loop).
...
If you receive a private message saying you are not allowed to tag the bot, your Slack ID is not authorized for that agent. The agent's owner needs to [share the agent](lc:langsmith/fleet/manage-agent-settings#change-access-to-the-agent) with you, either by sharing run access with the whole LangSmith workspace or with you individually.
