...
For setup instructions, see [Change access to the agent](lc:langsmith/fleet/manage-agent-settings#change-access-to-the-agent).
...
For setup instructions, see [Agent identity](lc:langsmith/fleet/agent-identity).
...
For more on RBAC, see [Role-based access control](lc:langsmith/rbac).

#### Create a role with tool permissions

  #### Step: Open role settings

    Navigate to **Settings > Roles** and click **Create role**.
  
  #### Step: Configure MCP Servers permissions
...
For details on policy structure, operators, and managing policies via the API, see [Attribute-based access control](lc:langsmith/abac).
...
Agent actions in Fleet are captured in a structured [LangSmith trace](lc:langsmith/observability), including tool calls, decisions, and outputs. You can inspect, search, and export traces.
...
For setup instructions, see [Human-in-the-loop](lc:langsmith/fleet/essentials#human-in-the-loop).
