...
Agent identity controls whose [credentials](lc:langsmith/fleet/workspace-admin) the agent uses when it interacts with apps and services.

See [Agent identity](lc:langsmith/fleet/agent-identity) for more information.
...
- **Channels**: Connect the places your agent runs in, such as Slack, Gmail, and Microsoft Teams. See [Channels](lc:langsmith/fleet/channels).
- **Sharing**: Control who can use the agent, with options for private, workspace, or specific people. See [Change access to the agent](lc:langsmith/fleet/manage-agent-settings#change-access-to-the-agent).
...
- **Schedule**: Run your agent on a recurring basis. See [Schedules](lc:langsmith/fleet/schedules).
...
See [Channels](lc:langsmith/fleet/channels) for setup instructions and supported channel types.
...
> [!NOTE]
>
> When an agent is triggered from Slack, it raises the approval request directly in the Slack thread with **Approve** and **Deny** buttons, so you can respond without leaving Slack. See [Approve or deny actions in Slack](lc:langsmith/fleet/slack-app#approve-or-deny-actions-in-slack).
...
1. In the [LangSmith UI](https://smith.langchain.com), open your agent.
...
>
> For agents that run on automated [schedules](lc:langsmith/fleet/schedules#add-a-schedule), we recommend [disabling the approval requirement](lc:langsmith/fleet/manage-agent-settings#disable-required-approval-for-memory-updates) so the agent can persist information without manual intervention.
...
For more information, see [Skills](lc:langsmith/fleet/skills).
...
Tools work regardless of how the agent was triggered. For example, you can start a task in the Fleet chat UI and have the agent send you a [Slack message](lc:langsmith/fleet/slack-app#add-slack-tools) when it's done.

See [Tool integrations](lc:langsmith/fleet/tools) for more information.

## Traces

Traces are a series of steps that your agent takes to go from input to output. You can use [LangSmith](lc:langsmith/observability) to visualize these execution steps.
...
For more information, see [LangSmith Observability](lc:langsmith/observability).
...
- [Connect apps and services](lc:langsmith/fleet/tools)
- [Use remote servers for tools](lc:langsmith/fleet/remote-mcp-servers)
