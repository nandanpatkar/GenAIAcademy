...
Fleet uses [workspace secrets](lc:langsmith/set-up-hierarchy#configure-workspace-settings) to store API keys for models and tools. The following secret types are available:

- **Model provider key**: By default, Fleet uses models managed by LangChain and does not require a model-provider API key. An OpenAI or Anthropic API key is required only when you use [custom models](lc:langsmith/fleet/essentials#custom-models). When set, the agent graphs load this key from workspace secrets for inference.
...
- **MCP server configuration**: Fleet can pull tools from one or more remote [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) servers. Configure MCP servers and headers in your [workspace](lc:langsmith/administration-overview#workspaces) settings. Fleet automatically discovers tools and applies the configured headers when calling them. For more information, refer to the [Remote MCP servers](lc:langsmith/fleet/remote-mcp-servers) page.


> [!NOTE]
>
> Custom models are available for enterprise deployments. See [Custom models](lc:langsmith/fleet/essentials#custom-models) for more information.
...
1. In the [LangSmith UI](https://smith.langchain.com), navigate to  **Settings** and then move to the **Secrets** tab.
