...
You can invoke LangSmith Fleet agents from your applications using the [LangGraph SDK](lc:langsmith/reference) or the REST API. Fleet agents run on [Agent Server](lc:langsmith/agent-server), so you can use the same API methods as any other [LangSmith deployment](lc:langsmith/deployment).
...
- A [Personal Access Token (PAT)](lc:langsmith/create-account-api-key) for authentication
- (SDK only) The [LangGraph SDK](lc:langsmith/reference) installed:
    ...
    [
     {
      "label": "Python",
      "lang": "bash",
      "code": "pip install langgraph-sdk python-dotenv"
     },
     {
      "label": "TypeScript",
      "lang": "bash",
      "code": "yarn add @langchain/langgraph-sdk"
     }
    ]
    ```

### Authentication
To authenticate with your agent's Fleet deployment, provide a LangSmith [Personal Access Token (PAT)](lc:langsmith/create-account-api-key) to the `api_key` argument when instantiating the LangGraph SDK client, or via the `X-API-Key` header. If using `X-API-Key`, you must also set the `X-Auth-Scheme` header to `langsmith-api-key`.
...
Create a `.env` file in your project root with your [Personal Access Token](lc:langsmith/create-account-api-key):
...
> 
    Use a [Personal Access Token (PAT)](lc:langsmith/create-account-api-key) tied to your LangSmith account. Set the `X-Auth-Scheme` header to `langsmith-api-key` for authentication.
...
- `X-Api-Key:` your [Personal Access Token](lc:langsmith/create-account-api-key)
- `X-Auth-Scheme: langsmith-api-key`

For the full API specification, see the [Agent Server API reference](lc:langsmith/server-api-ref).
...
In the [LangSmith UI](https://smith.langchain.com), open your agent and export it as a `.zip` file.

![fleet-export-code](/langchain/images/langsmith/images/fleet-export-code.gif.poster.png "heavy:17.4:https://docs.langchain.com/langsmith/images/fleet-export-code.gif")
