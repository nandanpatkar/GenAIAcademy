...
For deeper hook, state, and context details, see [Custom middleware](lc:oss/python/langchain/middleware/custom).

## Add a middleware module

Put middleware code under `middleware/` in your project and import it from the agent entry. For the full project layout, see the [CLI project file reference](lc:langsmith/managed-deep-agents-cli#project-file-reference).
...
The `interrupt_on` field applies the same interrupt behavior as LangChain's [human-in-the-loop middleware](lc:oss/python/langchain/guardrails#human-in-the-loop). For decision types (approve, edit, reject), conditional interrupts, and permission rules, see the Deep Agents [Human-in-the-loop](lc:oss/python/deepagents/human-in-the-loop) and [Permissions](lc:oss/python/deepagents/permissions) guides.
...
- **On a deployed agent**, resume the paused run through the LangGraph server API with a `Command(resume=...)` payload. See [Human-in-the-loop using server API](lc:langsmith/add-human-in-the-loop).
...
Human-in-the-loop needs durable thread state to pause and resume. The managed runtime owns the checkpointer, so no extra setup is required. For how threads persist, see [How Managed Deep Agents work](lc:langsmith/managed-deep-agents-how-it-works#threads-and-memory).
...
For examples, see [Custom middleware](lc:oss/python/langchain/middleware/custom).
