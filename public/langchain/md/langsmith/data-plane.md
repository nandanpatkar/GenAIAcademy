The _data plane_ consists of your [Agent Servers](lc:langsmith/agent-server) (deployments), their supporting infrastructure, and the "listener" application that continuously polls for updates from the [LangSmith control plane](lc:langsmith/control-plane).

## Server infrastructure

In addition to the [Agent Server](lc:langsmith/agent-server) itself, the following infrastructure components for each server are also included in the broad definition of "data plane":
...
The data plane "listener" application periodically calls [control plane APIs](lc:langsmith/control-plane#control-plane-api) to:
...
PostgreSQL stores server resources (threads, runs, assistants, crons) and items saved in the [long-term memory store](lc:oss/python/langgraph/persistence#memory-store). It is also the default backend for [checkpoints](lc:oss/python/langgraph/persistence) (graph execution state). You can optionally store checkpoints in MongoDB instead—see [Configure checkpointer backend](lc:langsmith/configure-checkpointer). PostgreSQL is always required regardless of the checkpointer backend.
...
This section describes various features of the data plane. For platform-specific behavior, see [Cloud platform features](lc:langsmith/cloud-platform-features) or [Deploy to self-hosted](lc:langsmith/deploy-to-self-hosted-overview).

### Autoscaling

[Dedicated type](lc:langsmith/cloud-platform-features#deployment-types) deployments automatically scale across containers. Scaling is based on 3 metrics:

1. CPU utilization
2. Memory utilization
3. Number of pending (in progress) [runs](lc:langsmith/runs)
...
These metrics don't all apply to every container type. [Queue workers](lc:langsmith/agent-server#runtime-architecture) scale on pending run count—when the backlog grows, more workers spin up to drain it. [API servers](lc:langsmith/agent-server#runtime-architecture) scale on CPU and memory, responding to client request volume. This means a spike in run submissions won't slow down read operations like fetching thread state. For self-hosted configuration details, see [Configure Agent Server for scale](lc:langsmith/agent-server-scale).
...
> [!NOTE]
>
> Available for [Cloud](lc:langsmith/cloud) (with an externally managed MongoDB instance) and [Standalone](lc:langsmith/deploy-standalone-server) deployments.
...
See [Configure checkpointer backend](lc:langsmith/configure-checkpointer) for setup instructions.
