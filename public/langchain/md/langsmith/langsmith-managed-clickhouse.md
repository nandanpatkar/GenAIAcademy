> [!TIP]
>
> Please read the [LangSmith architectural overview](lc:langsmith/self-hosted) and [guide on connecting to external ClickHouse](lc:langsmith/self-host-external-clickhouse) before proceeding with this guide.
...
![LangSmith managed ClickHouse architecture.](/langchain/images/langsmith/images/managed-clickhouse-light.png)


![LangSmith managed ClickHouse architecture.](/langchain/images/langsmith/images/managed-clickhouse-dark.png)


## Requirements

- **You must use a supported blob storage option.** Read the [blob storage guide](lc:langsmith/self-host-blob-storage) for more information.
...
- You must have a LangSmith self-hosted instance running. You can use our managed ClickHouse service with [Kubernetes](lc:langsmith/kubernetes) installations.
...
This [reference doc](lc:langsmith/feedback-data-format) explains the stored feedback format, which is the LangSmith's way of representing evaluation scores and annotations on runs.
...
This [reference doc](lc:langsmith/run-data-format) explains the format of stored runs (spans), which are the building blocks of traces.
