...
Trace your [Pipecat](https://pipecat.ai/) voice agents to LangSmith with the LangSmith Pipecat integration. For high-level conventions, see [Voice tracing fundamentals](lc:langsmith/trace-voice-fundamentals).
...
[
 {
  "label": "pip",
  "lang": "bash",
  "code": "pip install \"langsmith[pipecat]\" \"pipecat-ai[openai,local,tracing]\""
 },
 {
  "label": "uv",
  "lang": "bash",
  "code": "uv add \"langsmith[pipecat]\" \"pipecat-ai[openai,local,tracing]\""
 }
]
```
...
If your LLM stage is an in-process [LangGraph or LangChain](lc:oss/python/langgraph/overview) agent, its model and tool runs should nest inside Pipecat's `llm` span rather than forming a separate trace. To achieve this:
...
To group a conversation's runs into a LangSmith [thread](lc:langsmith/threads) for thread-level views and token and cost aggregation, call `set_thread_id` once per conversation before its spans are emitted:
...
The integration attaches the recording to the conversation root when it ends. For the underlying attachment API, see [Upload files with traces](lc:langsmith/upload-files-with-traces).
