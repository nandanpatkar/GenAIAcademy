...
Contract usage tracking is available for [**Enterprise plan**](lc:langsmith/pricing-plans) customers with prepaid commitments. You must have the [`organization:manage` permission](lc:langsmith/organization-workspace-operations) to access this feature.

For details on viewing your prepaid contract consumption, refer to [Contract usage](lc:langsmith/view-usage#contract-usage).
...
>
> - [Data Retention Conceptual Docs](lc:langsmith/usage-and-billing#data-retention)
> - [Usage Limiting Conceptual Docs](lc:langsmith/usage-and-billing#usage-limits)
...
The first step of any optimization process is to understand current usage. For details on the usage graph, granular usage, invoices, and contract usage, refer to [View usage](lc:langsmith/view-usage).
...
![P2usagelimitsempty v2](/langchain/images/langsmith/images/p2usagelimitsempty-v2.png)
...
LangSmith has two trace tiers: base traces and extended traces. Base traces have the base retention and are short-lived (14 days), while extended traces have extended retention and are long-lived (400 days by default, [customizable for Enterprise customers](lc:langsmith/data-purging-compliance#customize-extended-retention-policy)). For more information, refer to the [data retention conceptual docs](lc:langsmith/usage-and-billing#data-retention).
...
> [!NOTE]
>
> The extended data retention limit can cause features other than tracing to stop working once reached. If you plan to use this feature, read more about its [functionality and side effects](lc:langsmith/usage-and-billing#side-effects-of-extended-data-retention-traces-limit).


### Other methods of managing traces

#### Customize extended retention period ([Enterprise](lc:langsmith/pricing-plans) only)

[Enterprise](lc:langsmith/pricing-plans) customers can customize the extended data retention period at the workspace level to meet compliance requirements. The default is 400 days, but this can be adjusted based on your organization's needs. For more information, refer to [Customize extended retention policy](lc:langsmith/data-purging-compliance#customize-extended-retention-policy).

#### Change project-level default retention

Data retention settings are adjustable per tracing project. At the project level, you choose between two tiers: base (14 days) or extended (400 days). To customize the extended duration beyond 400 days, use [workspace-level configuration](lc:langsmith/data-purging-compliance#customize-extended-retention-policy) (Enterprise only).
...
![P1projectretention](/langchain/images/langsmith/images/p1projectretention.png)

#### Apply extended data retention to a percentage of traces

You may not want all traces to expire after 14 days. You can automatically extend the retention of traces that match some criteria by creating an [automation rule](lc:langsmith/rules). You might want to apply extended data retention to specific types of traces, such as:
...
2. Name your rule and optionally apply filters or a sample rate. For more information on configuring filters, refer to [filtering techniques](lc:langsmith/filter-traces-in-application#filter-operators).


> [!NOTE]
>
> When an automation rule matches any [run](lc:langsmith/observability-concepts#runs) within a [trace](lc:langsmith/observability-concepts#traces), then all runs within the trace are upgraded to extended data retention (400 days by default, [customizable for Enterprise customers](lc:langsmith/data-purging-compliance#customize-extended-retention-policy)).


For example, this is the expected configuration to keep 10% of all traces for extended data retention:

![P2sampletraces](/langchain/images/langsmith/images/P2SampleTraces.png)
...
- **Compute**: The vCPU and memory a deployment uses while resources are provisioned, measured in LangChain Compute Units (LCU). A [Serverless](lc:langsmith/cloud-platform-features#serverless) deployment can [scale to zero (beta)](lc:langsmith/cloud-platform-features#serverless) after a period of inactivity, so compute charges stop only once it has scaled down. A [Dedicated](lc:langsmith/cloud-platform-features#dedicated) deployment is always-on and consumes compute continuously.
