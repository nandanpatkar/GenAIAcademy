...
- The `mda` CLI from `managed-deepagents` (same install as [CLI reference](lc:langsmith/managed-deep-agents-cli#install)).
...
If the project exports [identity](lc:langsmith/managed-deep-agents-identity) (`identity.ts` or `identity.py`), every eval task must include `identity.json`. Scaffolding adds a default fixture automatically. Customize the fixture when your agent or tests depend on a specific actor, tenant, or claims.
...
- [Identity](lc:langsmith/managed-deep-agents-identity) — when tasks need `identity.json`
- [CLI reference](lc:langsmith/managed-deep-agents-cli) — full `mda` command surface
- [Deploy an agent](lc:langsmith/managed-deep-agents-deploy) — ship the agent after local evals pass
