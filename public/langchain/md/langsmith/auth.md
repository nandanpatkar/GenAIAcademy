...
To enable an agent to act on behalf of the user, use [custom authentication middleware](lc:langsmith/custom-auth). This will allow the agent to interact with external systems like MCP servers, external databases, and even other agents on behalf of the user.

For more information, see the [Use custom auth](lc:langsmith/custom-auth#enable-agent-authentication) guide.

### Agent authentication with MCP

For information on how to authenticate an agent to an MCP server, see the [MCP conceptual guide](lc:oss/python/langchain/mcp).
...
Store authorization differs from threads and assistants. Handlers must rewrite the mutable `namespace` field in `value` to scope data per user rather than returning metadata filters. For a walkthrough, see [Isolate store per user](lc:langsmith/store-auth).
...
* Check out the introductory tutorial on [setting up authentication](lc:langsmith/set-up-custom-auth)
* See the how-to guide on implementing a [custom auth handlers](lc:langsmith/custom-auth)
