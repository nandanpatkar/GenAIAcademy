...
>
> **Prerequisites**
> This guide assumes familiarity with [LangSmith](lc:langsmith/observability), [Persistence](lc:oss/python/langgraph/persistence), and [Cross-thread persistence](lc:oss/python/langgraph/stores) concepts.


LangSmith persists both [checkpoints](lc:oss/python/langgraph/checkpointers#checkpoints) (thread state) and [cross-thread memories](lc:oss/python/langgraph/stores) (store items). You can configure Time-to-Live (TTL) policies in [`langgraph.json`](lc:langsmith/application-structure#configuration-file) to manage the lifecycle of this data automatically, preventing indefinite accumulation.
...
After configuring TTLs in `langgraph.json`, deploy or restart your LangGraph application for the changes to take effect. Use [`langgraph dev`](lc:langsmith/local-dev-testing#langgraph-dev) for local development or [`langgraph up`](lc:langsmith/local-dev-testing#langgraph-up) for Docker deployment.

For details on other configurable options, refer to the [LangGraph CLI reference page](lc:langsmith/cli#configuration-file).
