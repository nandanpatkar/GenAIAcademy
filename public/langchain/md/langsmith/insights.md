...
> [!NOTE]
>
> Insights is available for LangSmith Plus and Enterprise [plans](lc:langsmith/pricing-plans).


## Prerequisites

- A [model configuration](lc:langsmith/model-configurations) set up for Insights in your workspace.
- [Permissions](lc:langsmith/organization-workspace-operations#projects) to create rules in LangSmith (required to generate new Insights Reports).
- [Permissions](lc:langsmith/organization-workspace-operations#projects) to view tracing projects in LangSmith (required to view existing Insights Reports).
...
1. If you haven't already, [configure a model](lc:langsmith/model-configurations) for Insights in your workspace settings.
...
You can generate Insights Reports over data stored outside LangSmith using the [Python SDK](lc:langsmith/smith-python-sdk). This allows you to analyze chat histories from your production systems, logs, or other sources.
...
1. Return a link to your results in the [LangSmith UI](https://smith.langchain.com).
...
![](/langchain/images/langsmith/images/insights-job-results.png)
...
![](/langchain/images/langsmith/images/insights-nav.gif.poster.png "heavy:2.7:https://docs.langchain.com/langsmith/images/insights-nav.gif")
...
| `all_thread_messages` | Full message history for the thread (only available for projects with [threads](lc:langsmith/threads)) | `{{all_thread_messages}}` |
...
> [!NOTE]
>
> For projects with [threads](lc:langsmith/threads), Insights analyzes full conversations. Only the most recent root run from each thread is used for `run.*` variables. Use `all_thread_messages` to access the complete conversation history.
