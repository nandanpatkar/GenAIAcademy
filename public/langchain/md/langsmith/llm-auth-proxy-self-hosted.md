...
- Authenticate [Playground](lc:langsmith/custom-endpoint#use-the-model-in-the-playground) or [LLM-as-judge evaluation](lc:langsmith/evaluation) requests against your own provider gateway.
...
![Architecture diagram showing LangSmith issuing a signed JWT to the self-hosted auth proxy, which validates the JWT, applies customer-defined auth, and forwards the request to the upstream model provider.](/langchain/images/langsmith/images/llm-auth-proxy-architecture-light.png)


![Architecture diagram showing LangSmith issuing a signed JWT to the self-hosted auth proxy, which validates the JWT, applies customer-defined auth, and forwards the request to the upstream model provider.](/langchain/images/langsmith/images/llm-auth-proxy-architecture-dark.png)
...
> [!NOTE]
>
> The auth proxy currently supports the [Playground](lc:langsmith/prompt-engineering-concepts), [Evals](lc:langsmith/evaluation), [Fleet](lc:langsmith/fleet/index), [Chat](lc:langsmith/chat), and [Insights](lc:langsmith/insights) features.
...
![LLM Auth Proxy settings in LangSmith showing the Enable LLM auth proxy checkbox, JWT audience field, and Allowed URLs radio buttons with Allow all selected.](/langchain/images/langsmith/images/llm-auth-proxy-settings-light.png)

    
![LLM Auth Proxy settings in LangSmith showing the Enable LLM auth proxy checkbox, JWT audience field, and Allowed URLs radio buttons with Allow all selected.](/langchain/images/langsmith/images/llm-auth-proxy-settings-dark.png)
...
To enable HTTP traffic to the auth proxy for [Chat and Insights](lc:langsmith/deploy-self-hosted-full-platform#enable-fleet-insights-and-chat), set this environment variable in the respective `extraEnv` sections: `config.polly.agent.extraEnv` (for Chat, which was formerly called Polly) and `config.insights.agent.extraEnv`.
