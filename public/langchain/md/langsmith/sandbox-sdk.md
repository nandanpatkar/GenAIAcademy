The [LangSmith SDK](lc:langsmith/reference) provides a programmatic interface to create and interact with sandboxes.
...
Pass a snapshot ID or name when you want to boot from a reusable custom filesystem image; see [Snapshots](lc:langsmith/sandbox-snapshots) for that flow.
...
For more details, including use cases, REST API access, and a full FastAPI example, see [Service URLs](lc:langsmith/sandbox-service-urls).
