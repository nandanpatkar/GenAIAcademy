...
> [!WARNING]
>
> This page describes the legacy hybrid deployment model, which uses a LangChain-managed control plane to orchestrate Agent Servers in your cloud. For the current hybrid model, see [Hybrid](lc:langsmith/hybrid).
...
> [!NOTE]
>
> Learn more about the [control plane](lc:langsmith/control-plane), [data plane](lc:langsmith/data-plane), and [Agent Server](lc:langsmith/agent-server) architecture concepts.
...
When running LangSmith in a hybrid model, you authenticate with a [LangSmith API key](lc:langsmith/create-account-api-key).

### Workflow

1. Use the `langgraph-cli` or [Studio](lc:langsmith/studio) to test your graph locally.
1. Build a Docker image using the `langgraph build` command.
1. Deploy your Agent Server from the [control plane UI](lc:langsmith/control-plane#control-plane-ui).
...
![Hybrid deployment: LangChain-hosted control plane (LangSmith UI/APIs) manages deployments. Your cloud runs a listener, Agent Server instances, and backing stores (Postgres/Redis) on Kubernetes.](/langchain/images/langsmith/images/hybrid-with-deployment-light.png)


![Hybrid deployment: LangChain-hosted control plane (LangSmith UI/APIs) manages deployments. Your cloud runs a listener, Agent Server instances, and backing stores (Postgres/Redis) on Kubernetes.](/langchain/images/langsmith/images/hybrid-with-deployment-dark.png)
...
In order to enable this egress, you may need to update internal firewall rules or cloud resources (such as Security Groups) to [allow certain IP addresses](lc:langsmith/cloud#ingress-into-langchain-saas).
...
2. A valid `Ingress` controller is installed on your cluster. For more information about configuring ingress for your deployment, refer to [Create an ingress for installations](lc:langsmith/self-host-ingress). We highly recommend using the modern [Gateway API](lc:langsmith/self-host-ingress#option-2%3A-gateway-api) in a production setup.
3. If you plan to have the listener watch multiple namespaces, you **MUST** use the [Gateway API](lc:langsmith/self-host-ingress#option-2%3A-gateway-api) or an [Istio Gateway](lc:langsmith/self-host-ingress#option-3%3A-istio-gateway) instead of the [standard ingress](lc:langsmith/self-host-ingress#option-1%3A-standard-ingress) resource. A standard ingress resource can only route traffic to services in the same namespace, whereas a Gateway or Istio Gateway can route traffic to services across multiple namespaces.
...
2. Create a listener from the LangSmith UI. The `Listener` data model is configured for the actual ["listener" application](lc:langsmith/data-plane#listener-application).
...
3. A [Helm chart](https://github.com/langchain-ai/helm/tree/main/charts/langgraph-dataplane) is provided to install the necessary components in your Kubernetes cluster.
    - `langgraph-dataplane-listener`: This is a service that listens to LangChain's [control plane](lc:langsmith/control-plane) for changes to your deployments and creates/updates downstream CRDs. This is the ["listener" application](lc:langsmith/data-plane#listener-application).
...
2. It is required to use the [Gateway API](lc:langsmith/self-host-ingress#option-2%3A-gateway-api) or an [Istio Gateway](lc:langsmith/self-host-ingress#option-3%3A-istio-gateway). Relying on the [standard ingress](lc:langsmith/self-host-ingress#option-1%3A-standard-ingress) resource can cause conflicts with Ingress objects created by other data planes in the same cluster. Because behavior in these cases depends on the specific ingress controller, this may result in unpredictable or undesired outcomes.

## Listeners

In the hybrid option, one or more ["listener" applications](lc:langsmith/data-plane#listener-application) can run depending on how your LangSmith workspaces and Kubernetes clusters are organized.
