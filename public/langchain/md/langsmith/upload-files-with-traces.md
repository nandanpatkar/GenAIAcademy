...
In the [Python SDK](lc:langsmith/smith-python-sdk), you can use the `Attachment` type to add files to your traces. Each `Attachment` requires:
...
In the [TypeScript SDK](lc:langsmith/smith-js-ts-sdk), you can add attachments to traces by using `Uint8Array` or `ArrayBuffer` as data types. Each attachment's MIME type is specified within `extractAttachments`:
...
- [Manage datasets](lc:langsmith/manage-datasets)
