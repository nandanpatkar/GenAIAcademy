LangSmith stores each [run](lc:langsmith/observability-concepts) as a structured record. Understanding this format is useful when exporting traces, querying the API, or building integrations.
...
| `prompt` | A [prompt template](lc:langsmith/prompt-template-format) that formats inputs before passing them to a model. |
...
Set `run_type`[run_type] when instrumenting with [`@traceable`](lc:langsmith/annotate-code#use-%40traceable-%2F-traceable) or [`RunTree`](lc:langsmith/annotate-code#use-the-runtree-api):
...
- [Log LLM calls](lc:langsmith/log-llm-trace)
- [Log retriever traces](lc:langsmith/log-retriever-trace)
