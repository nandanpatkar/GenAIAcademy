...
- Move to a [replicated ClickHouse cluster](lc:langsmith/self-host-external-clickhouse#ha-replicated-clickhouse-cluster). Adding replicas of ClickHouse helps with read performance, but we recommend staying below 5 replicas (start with 3).
...
> [!WARNING]
>
> If you still notice slow reads with the above configuration, we recommend moving to a [replicated Clickhouse cluster setup](lc:langsmith/self-host-external-clickhouse#ha-replicated-clickhouse-cluster)
