...
![A thread groups a session](/langchain/images/langsmith/images/thread-trajectory-light.png)


![A thread groups a session](/langchain/images/langsmith/images/thread-trajectory-dark.png)
...
[Learn how to configure threads](lc:langsmith/threads).
...
> [!NOTE]
>
> The [Messages view](lc:langsmith/view-traces#messages-view), which renders trajectories in the LangSmith UI, is in **[beta](lc:langsmith/release-stages)**.


[Learn how trajectories render in the Messages view](lc:langsmith/messages-view-integrations).
...
> 
Use **[Chat](lc:langsmith/chat)** to analyze traces, runs, and threads. Chat helps you understand agent performance, debug issues, and gain insights from conversation threads without manually digging through data.

### Projects

A _project_ is a container for all the traces related to a single application or service.

[Log traces to a project](lc:langsmith/log-traces-to-project).
...
For more on how feedback is stored, refer to the [Feedback data format guide](lc:langsmith/feedback-data-format).

### Tags

_Tags_ are strings you can attach to runs to categorize, filter, and group them in the LangSmith UI.

[Learn how to attach tags to your traces](lc:langsmith/add-metadata-tags).
...
[Learn how to add metadata to your traces](lc:langsmith/add-metadata-tags).
...
[Browse all integrations](lc:langsmith/integrations).
...
[Learn how to add manual instrumentation](lc:langsmith/annotate-code).

## Data retention

LangSmith (SaaS) retains trace data for 180 days from ingestion. After that, traces are permanently deleted, with limited metadata retained for usage statistics. For details on retention tiers and pricing, refer to [Usage and billing: Data retention](lc:langsmith/usage-and-billing#data-retention).


> [!NOTE]
>
> To keep data beyond the retention period, add it to a [dataset](lc:langsmith/manage-datasets). Datasets persist indefinitely, even after the source trace is deleted.


To delete traces before their expiration date, see [Manage a trace](lc:langsmith/manage-trace#delete-a-trace).
