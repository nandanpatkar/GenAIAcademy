LangSmith Deployment runs any framework. For agents not built on Deep Agents, LangChain, or LangGraph, deploy using either the [`deployments-wrap-sdk`](https://pypi.org/project/deployments-wrap-sdk/) package (Google ADK) or the [LangGraph Functional API](lc:oss/python/langgraph/functional-api) (Claude Agent SDK, Strands, CrewAI, AutoGen, and other libraries).


> [!TIP]
>
> For new builds, consider [Deep Agents](lc:oss/python/deepagents/overview), an open-source harness for agents that plan, use tools, delegate to subagents, and work over long horizons. Deep Agents deploy directly to LangSmith Deployment, with [Managed Deep Agents](lc:langsmith/managed-deep-agents-overview) available for a fully hosted runtime.


## Supported frameworks

The following frameworks have end-to-end examples in this guide. Each example exports a LangGraph-compatible graph from `agent.py` that [Agent Server](lc:langsmith/agent-server) can serve:
...
![](/langchain/images/images/providers/light/claude.svg)

        
![](/langchain/images/images/providers/dark/claude.svg)
...
![](/langchain/images/images/providers/light/bedrock.svg)

        
![](/langchain/images/images/providers/dark/bedrock.svg)
...
![](/langchain/images/images/providers/light/crewai.svg)

        
![](/langchain/images/images/providers/dark/crewai.svg)
...
![](/langchain/images/images/providers/light/autogen.svg)

        
![](/langchain/images/images/providers/dark/autogen.svg)
...
#### Step: Wrap with the Functional API
...
For full setup details, see [Trace Claude Agent SDK applications](lc:langsmith/trace-claude-agent-sdk).

#### Tab: Strands Agents

Set your [LangSmith API key](lc:langsmith/create-account-api-key) and project name. If you use Amazon Bedrock, also configure AWS credentials.
...
> [!NOTE]
>
> If you're [self-hosting LangSmith](lc:langsmith/self-hosted), configure the OpenTelemetry OTLP endpoint and headers for your deployment. See [Trace Strands Agents applications](lc:langsmith/trace-with-strands-agents).
...
For full setup details, see [Trace Strands Agents applications](lc:langsmith/trace-with-strands-agents).
...
For full setup details, see [Trace CrewAI applications](lc:langsmith/trace-with-crewai).
...
For full setup details, see [Trace AutoGen applications](lc:langsmith/trace-with-autogen).
...
[`langgraph.json`](lc:langsmith/application-structure#configuration-file-concepts) points Agent Server at the exported symbol:
...
[
 ...
 },
 {
  "label": ".env",
  "lang": "bash",
  "code": "LANGSMITH_API_KEY=your-langsmith-api-key\nLANGSMITH_TRACING=true\nLANGSMITH_PROJECT=my-claude-agent\nANTHROPIC_API_KEY=your-anthropic-api-key"
 }
]
```
...
[Traces](lc:langsmith/observability) show agent invocations, tool calls, and LLM interactions in the [LangSmith UI](https://smith.langchain.com). For framework-specific tracing options, see the links in [Configure tracing](#configure-tracing).

## Run locally

Start the local Agent Server with the [LangGraph CLI](lc:langsmith/cli):
...
This serves the agent at `http://127.0.0.1:2024` and opens [LangSmith Studio](lc:langsmith/studio). Send a request with `curl`:
...
#### Tab: Claude Agent SDK, CrewAI, AutoGen
...
> [!NOTE]
>
> [Verify that the LangGraph API runs locally](lc:langsmith/local-dev-testing) before deploying. If `langgraph dev` fails, deployment to LangSmith will fail as well.
...
For environment configuration, deployment types, and revision management, see [Deploy to cloud](lc:langsmith/deploy-to-cloud). For self-hosted setups, see [Self-hosted deployments](lc:langsmith/self-hosted). For Docker-only hosting without the control plane, see [Deploy standalone](lc:langsmith/deploy-standalone-server).
