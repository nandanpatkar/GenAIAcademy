...
One-time setup to enable the LLM Gateway for your LangSmith [organization](lc:langsmith/administration-overview#organizations). [Organization admins](lc:langsmith/rbac#organization-admin) should complete this before individual users can route calls through the gateway.

## Prerequisites

You need [`organization:manage` permission](lc:langsmith/organization-workspace-operations) in LangSmith. [Step 2 Option A](lc:langsmith/llm-gateway-admin-setup#option-a-create-a-custom-workspace-role-recommended) also requires a plan that includes [RBAC](lc:langsmith/rbac) (custom roles).
...
- **Spend limits:** hard caps at the organization, workspace, API key, or user level. Refer to [Spend policies](lc:langsmith/llm-gateway-spend-policies).
- **Data protection:** detect and redact PII and secrets before they reach the model. Refer to [Data protection](lc:langsmith/llm-gateway-data-protection).
...
Create workspace-scoped [Service Keys](lc:langsmith/administration-overview#service-keys) for users who need gateway access. Each key should be attached to a role that includes `gateway:invoke` and `workspaces:read`.

Use workspace-scoped keys, not organization-scoped keys. See [API key scoping](lc:langsmith/llm-gateway-access#api-key-scoping) for details.

Share the key and the gateway endpoint with each user, or distribute them via MDM (mobile device management) for company-wide coding agent rollouts. For per-agent configuration instructions, refer to [Set up coding agents](lc:langsmith/llm-gateway-coding-agents).

## Verification

Ask a user to run the [verification cURL from the quickstart](lc:langsmith/llm-gateway-quickstart#2-make-a-call). A `200` response confirms the gateway, the API key, provider secrets, and role permissions are all configured correctly. The call will appear as a trace in the **gateway** tracing project in the workspace.

## Next steps

- [Quickstart](lc:langsmith/llm-gateway-quickstart): share with your users as the getting-started guide.
- [Set up coding agents](lc:langsmith/llm-gateway-coding-agents): configure Claude Code, Codex, and other agents org-wide.
- [Traces, Engine, and access control](lc:langsmith/llm-gateway-access): deep dive on roles, scoped keys, trace routing, and who can see what.
