Model configurations define the model and parameters that LangSmith features use when calling an AI provider. A single shared library of configurations spans your entire [workspace](lc:langsmith/administration-overview#workspaces), so any configuration you create is available across the following features without duplication:

- [**Playground**](lc:langsmith/prompt-engineering-concepts)
- [**Evaluators**](lc:langsmith/evaluation)
- [**Fleet**](lc:langsmith/fleet/index)
- [**Chat**](lc:langsmith/chat)
- [**Insights**](lc:langsmith/insights)

[Workspace admins](lc:langsmith/rbac#workspace-admin) can create, edit, and delete configurations and control which providers and models are available per feature. Non-admin members can view configurations but cannot modify them.
...
- **Organization** (organization admins): turn a provider on or off for the **entire organization**—every [workspace](lc:langsmith/administration-overview#workspaces) and every feature. Managed in **Settings** > **Model providers**.
...
Managing organization-wide providers requires the [Organization Admin](lc:langsmith/rbac#organization-admin) role (the `organization:manage` permission), and is available on LangSmith [Cloud](lc:langsmith/cloud) and [Self-hosted](lc:langsmith/self-hosted) for organizations (not personal accounts).
...
The **Configurations** table is a shared library of named model configurations for your workspace. Configurations you create in LangSmith (including from the [Playground](lc:langsmith/managing-model-configurations)) appear here and you can reuse them across all features.
...
> [!NOTE]
>
> OAuth client credentials are available on LangSmith [Cloud](lc:langsmith/cloud) and [Self-hosted](lc:langsmith/self-hosted) deployments running version `0.16.0-rc.6` or later.


When a model configuration sits behind an OAuth2 gateway, you can store the OAuth `client_credentials` directly on the configuration instead of distributing a static API key. LangSmith exchanges those credentials for a short-lived bearer token at request time, attaches it as `Authorization: Bearer <token>` on the outbound LLM call, and refreshes the token before it expires. This is a per-configuration self-service alternative to routing the workspace through the [LLM auth proxy](lc:langsmith/llm-auth-proxy-self-hosted); the two are mutually exclusive per configuration.

OAuth client credentials are available on every [plan](lc:langsmith/pricing-plans) that supports custom model configurations. The **Use Custom OAuth** toggle applies to bearer-token providers (OpenAI, Anthropic, OpenAI-compatible endpoints, and similar) and is not supported for Bedrock, Google Vertex AI, or Google GenAI, which authenticate with native cloud identity. The toggle is also hidden for the **LangServe (Deprecated)** preset.

### Configure OAuth on a model configuration

Configuring OAuth requires the [Workspace Admin](lc:langsmith/rbac#workspace-admin) role, or a [custom role](lc:langsmith/rbac#custom-roles) with the `workspaces:manage-model-configs` permission. Members without it see the OAuth fields disabled, with a masked secret hint. In the [LangSmith UI](https://smith.langchain.com):
...
![Model configuration Create modal in LangSmith Settings with Use Custom OAuth toggled on, showing Token URL, Client ID, masked Client Secret, and Token Endpoint Auth Method set to HTTP Basic.](/langchain/images/langsmith/images/model-config-oauth-create-light.png)


![Model configuration Create modal in LangSmith Settings with Use Custom OAuth toggled on, showing Token URL, Client ID, masked Client Secret, and Token Endpoint Auth Method set to HTTP Basic.](/langchain/images/langsmith/images/model-config-oauth-create-dark.png)
...
Routing between OAuth and the [LLM auth proxy](lc:langsmith/llm-auth-proxy-self-hosted) is per-configuration, not per-organization. Each request resolves to OAuth or the LLM auth proxy based on the configuration's OAuth state. A single multi-model job (for example, [Insights](lc:langsmith/insights) with separate Thinking and Summarization models) can mix the two flows because each model is resolved independently.
...
- [**Playground**](lc:langsmith/prompt-engineering-concepts): chat runs and experiment runs.
- [**Evaluators**](lc:langsmith/evaluation): LLM-as-judge configuration, Reuse, Preview Test, and Evaluator Details Test all skip the workspace-secrets prompt when every prompt resolves to an OAuth-enabled configuration.
- [**Insights**](lc:langsmith/insights): Thinking and Summarization configurations are resolved independently.
- [**Chat**](lc:langsmith/chat)
- [**Fleet**](lc:langsmith/fleet/index)
...
- **Encryption at rest**: client secrets are Fernet-encrypted with the same derivation used for [workspace secrets](lc:langsmith/administration-overview#workspaces).
