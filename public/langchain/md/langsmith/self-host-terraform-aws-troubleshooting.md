...
> [!TIP]
>
> Before upgrading, review the [LangSmith self-hosted changelog](lc:langsmith/self-hosted-changelog) for breaking changes and required variable updates. Run `aws eks update-kubeconfig --region <region> --name <cluster-name>` before running any `kubectl` commands.
