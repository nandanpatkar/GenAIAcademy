...
> [!TIP]
>
> To use MongoDB instead of PostgreSQL for checkpoint storage, see [Configure checkpointer backend](lc:langsmith/configure-checkpointer). This page is for implementing a fully custom storage backend.
...
Add the `checkpointer` key to your [`langgraph.json` configuration file](lc:langsmith/application-structure#configuration-file-concepts). The `path` points to the async context manager you [defined earlier](#define-the-checkpointer).
...
- [Build a custom checkpointer](lc:oss/python/langgraph/checkpointers#build-a-custom-checkpointer) including delta channel support.
- [Use a custom store](lc:langsmith/custom-store) to replace the built-in long-term memory store.
- Learn about [persistence and memory](lc:oss/python/langgraph/persistence) in LangGraph.
