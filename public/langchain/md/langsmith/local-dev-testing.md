This guide covers how to develop and test [Agent Server](lc:langsmith/agent-server) applications locally. The [LangGraph CLI](lc:langsmith/cli) provides two commands for local development, each optimized for different stages of your workflow:
...
> [!TIP]
>
> For full reference details, refer to the [LangGraph CLI reference](lc:langsmith/cli) page.
...
| Stage | Tool | Purpose |
|-------|------|---------|
| **Develop & Test Locally** | [`langgraph dev`](lc:langsmith/cli#dev) | Write and iterate on your graph with hot reloading |
| **Validate** | [`langgraph up`](lc:langsmith/cli#up) | Test production-like behavior with full stack |
| **Deploy** | [`langgraph deploy`](lc:langsmith/cli#deploy) | Deploy to production with confidence |
...
1. **Deploy**: Push to production via the [LangSmith UI](lc:langsmith/deployment-quickstart) or [Control Plane API](lc:langsmith/api-ref-control-plane).

## `langgraph dev`

The [`langgraph dev`](lc:langsmith/cli#dev) command runs a lightweight server directly in your environment, designed for speed and convenience during active development. The key features include:
...
> If you use [`langgraph new`](lc:langsmith/cli) without specifying a template, you will be presented with an interactive menu that will allow you to choose from a list of available templates.
...
The [`langgraph up`](lc:langsmith/cli#up) command orchestrates a full Docker-based stack that mirrors production infrastructure, helping catch deployment issues before production. The key features include:
...
- All [dependencies](lc:langsmith/setup-app-requirements-txt) install correctly in the container.
- Application starts without errors.
- Graph executes successfully.
- All [environment variables](lc:langsmith/env-var-cloud) work correctly.
- [Authentication/authorization](lc:langsmith/cli#adding-custom-authentication) works as expected.

## Dependencies configuration

Both `langgraph dev` and `langgraph up` read your application's [dependencies](lc:langsmith/application-structure#dependencies) from your [configuration files](lc:langsmith/application-structure#configuration-file), but they run in different environments:
...
The `dependencies` field tells the [CLI](lc:langsmith/cli) **where** to find your application code. The `dependencies` field can point to:
...
When you run [`langgraph up`](lc:langsmith/cli#up), the CLI follows these steps to install your application's dependencies:

1. [`langgraph.json`](lc:langsmith/application-structure#configuration-file) tells the CLI **where** to look for your application code. The `dependencies: ["."]` field points to the current directory.
1. **Find package configuration**: The CLI looks in that directory for a package configuration file ([`pyproject.toml`](lc:langsmith/setup-pyproject), [`requirements.txt`](lc:langsmith/setup-app-requirements-txt), or [`package.json`](lc:langsmith/setup-javascript)).
...
For more information on the installer, refer to [CLI configuration file](lc:langsmith/cli#configuration-file).
...
[Cloud deployments](lc:langsmith/cloud) don't use your local Docker.
...
**Solution**: Stop conflicting services or use the [`--port`](lc:langsmith/cli#dev) flag.
...
- [**Cloud**](lc:langsmith/cloud): Fastest setup, fully managed (recommended).
- [**Self-hosted**](lc:langsmith/self-hosted): Full control in your infrastructure.

For more details, refer to the [Platform setup comparison](lc:langsmith/platform-setup).

**Then deploy your app:**
- [Deploy to Cloud quickstart](lc:langsmith/deployment-quickstart): Quick setup guide.
- [Full Cloud setup guide](lc:langsmith/deploy-to-cloud): Comprehensive deployment documentation.

**Explore features:**
- **[Studio](lc:langsmith/studio)**: Visualize, interact with, and debug your application with the Studio UI. Try the [Studio quickstart](lc:langsmith/quick-start-studio).
- **API References**: [LangSmith Deployment API](https://langchain-ai.github.io/langgraph/cloud/reference/api/api_ref/), [Python SDK](lc:langsmith/langgraph-python-sdk), [JS/TS SDK](lc:langsmith/langgraph-js-ts-sdk)

## Related resources

- [CLI Reference](lc:langsmith/cli): Detailed documentation for all CLI commands
- [Application Structure](lc:langsmith/application-structure): How to structure your LangGraph application
- [Troubleshooting](lc:langsmith/troubleshooting-studio): Common issues and solutions
- [Setting up with pyproject.toml](lc:langsmith/setup-pyproject): Configure Python dependencies
- [Setting up with requirements.txt](lc:langsmith/setup-app-requirements-txt): Alternative dependency configuration
