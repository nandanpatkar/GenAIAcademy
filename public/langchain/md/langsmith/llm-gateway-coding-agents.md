...
Claude Code can use the standard Anthropic Messages format, while Codex and Deep Agents Code can use the standard OpenAI-compatible formats. Gemini CLI uses Google's native API and requires [direct model access](lc:langsmith/llm-gateway-direct-model-access).

## Prerequisites

- Your [Organization admin](lc:langsmith/rbac#organization-admin) has enabled the gateway and completed any required [provider setup](lc:langsmith/llm-gateway-admin-setup).
- You have a workspace-scoped [LangSmith API key](lc:langsmith/create-account-api-key) with `gateway:invoke` and `workspaces:read` [permissions](lc:langsmith/organization-workspace-operations).
- For bring-your-own-key models, your workspace has the corresponding provider secret. [Gateway Credits models](lc:langsmith/llm-gateway-credits) do not require a provider secret.
...
Gemini CLI sends Google's native Generate Content requests, which the standard endpoint does not expose. Follow [Direct model access](lc:langsmith/llm-gateway-direct-model-access#configure-provider-sdks) to configure the `/gemini` route, then run:
...
To use a bring-your-own-key model, keep the standard base URL and pass a provider-prefixed model after `openai:`, for example, `openai:anthropic/claude-opus-5`. For provider-native integrations and model IDs, see [Direct model access](lc:langsmith/llm-gateway-direct-model-access#configure-langchain-and-deep-agents).
...
1. A workspace-scoped [LangSmith API key](lc:langsmith/create-account-api-key) per user or team, depending on your policy granularity.
...
- [Gateway Credits](lc:langsmith/llm-gateway-credits): call hosted models without a provider secret.
- [Direct model access](lc:langsmith/llm-gateway-direct-model-access): configure provider-native routes for coding agents that require them.
- [Spend policies](lc:langsmith/llm-gateway-spend-policies): set cost limits on developer LLM usage.
- [Traces, Engine, and access control](lc:langsmith/llm-gateway-access): understand where gateway traces appear.
