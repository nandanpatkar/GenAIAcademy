...
Size the infrastructure for your target tier before deploying. For per-tier recommendations, refer to [Scaling guidance](lc:langsmith/self-host-scale).


> [!NOTE]
>
> For production workloads, also plan to provision external [LangChain Managed ClickHouse](lc:langsmith/langsmith-managed-clickhouse) or a self-managed external ClickHouse cluster. In-cluster ClickHouse is supported for dev/POC only.
...
> [!TIP]
>
> For a condensed cheat sheet of `make` targets, required variables, and common constraints, see the [AWS quick reference](lc:langsmith/self-host-terraform-aws-quick-reference).
...
See the [AWS variables reference](lc:langsmith/self-host-terraform-aws-variables) for every input variable.
...
For details on each add-on, see [LangSmith Deployment](lc:langsmith/deploy-self-hosted-full-platform).
...
- Reference the [AWS variables](lc:langsmith/self-host-terraform-aws-variables) and the [quick reference](lc:langsmith/self-host-terraform-aws-quick-reference).
- Review the [AWS architecture](lc:langsmith/self-host-terraform-aws-architecture) for platform layers, IRSA, and module dependencies.
- When something breaks, check the [AWS troubleshooting guide](lc:langsmith/self-host-terraform-aws-troubleshooting).
- Enable agent deployment in the UI with [LangSmith Deployment](lc:langsmith/deploy-self-hosted-full-platform).
