...
For the end-to-end install, refer to the [deploy guide](lc:langsmith/self-host-terraform-azure-deploy). For how the modules fit together, refer to the [architecture reference](lc:langsmith/self-host-terraform-azure-architecture).
...
| `clickhouse_source` | `in-cluster` | no | `in-cluster` (dev/POC only) or `external` for [LangChain Managed ClickHouse](lc:langsmith/langsmith-managed-clickhouse), recommended for production. |
