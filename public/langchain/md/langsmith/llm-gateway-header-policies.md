...
A [spend policy](lc:langsmith/llm-gateway-spend-policies) or [rate limit policy](lc:langsmith/llm-gateway-rate-limit-policies) can carry a condition on a custom request header, so traffic from a single subject splits into separate limits by header value. Use this to cap each of your own end customers, tenants, or teams without issuing a separate [LangSmith API key](lc:langsmith/create-account-api-key) for each one.

For example, a policy scoped to a [workspace](lc:langsmith/administration-overview#workspaces) with the condition `X-Gateway-Customer-Id: acme` limits only the requests from that workspace that carry that header value. Requests from the same workspace carrying `X-Gateway-Customer-Id: globex` count against a different policy.
...
> [!WARNING]
>
> Creating and managing policies requires the `organization:manage` permission. For the full permissions breakdown, refer to [Traces, Engine, and access control](lc:langsmith/llm-gateway-access).
...
[
 {
  "label": "curl",
  "lang": "bash",
  "code": "curl https://gateway.smith.langchain.com/openai/v1/chat/completions \\\n    -H \"Authorization: Bearer $LANGSMITH_API_KEY\" \\\n    -H \"Content-Type: application/json\" \\\n    -H \"X-Gateway-Customer-Id: acme\" \\\n    -d '{\"model\":\"gpt-4o-mini\",\"messages\":[{\"role\":\"user\",\"content\":\"ping\"}]}'"
 },
 {
  "label": "OpenAI SDK",
  "lang": "python",
...
Create one spend policy per end customer through the [LangSmith REST API](lc:langsmith/smith-api-ref):
...
- [Spend policies](lc:langsmith/llm-gateway-spend-policies): set cost caps for organizations, workspaces, users, and API keys.
- [Rate limit policies](lc:langsmith/llm-gateway-rate-limit-policies): limit requests and tokens in a rolling window.
- [Traces and access control](lc:langsmith/llm-gateway-access): understand where gateway traces land and who can configure policies.
