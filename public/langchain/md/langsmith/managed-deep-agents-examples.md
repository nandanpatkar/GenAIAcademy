...
For the packaging rules behind this layout, see [How Managed Deep Agents work](lc:langsmith/managed-deep-agents-how-it-works#compilation) and the [CLI project file reference](lc:langsmith/managed-deep-agents-cli#project-file-reference).
...
`interrupt_on` (Python) and `interruptOn` (TypeScript) pause the run before the `query_db` tool executes, so a human can approve the call. For decision types and how to respond to interrupts, see [Human-in-the-loop](lc:langsmith/managed-deep-agents-middleware#human-in-the-loop). For the full list of fields, see the [agent definition reference](lc:langsmith/managed-deep-agents-cli#agent-definition-reference).
...
| --- | --- | --- |
| `tools/query_db.py` or `query-db.ts` | Read-only database lookup tool | [Custom tools](lc:langsmith/managed-deep-agents-tools) |
| `middleware/audit.py` or `audit.ts` | Audit logging before model calls | [Custom middleware](lc:langsmith/managed-deep-agents-middleware) |
| `connectors/mcp.py` or `mcp.ts` | LangChain docs MCP server | [MCP connector](lc:langsmith/managed-deep-agents-connectors/mcp) |
| `schedules/daily_check_in.py` or `daily-check-in.ts` | Daily 9am Pacific cron run | [Schedules](lc:langsmith/managed-deep-agents-schedules) |
| `skills/research/SKILL.md` | On-demand research procedure | [Deploy an agent](lc:langsmith/managed-deep-agents-deploy#configure-instructions-skills-and-memory) |
| `sandbox/` | Managed LangSmith sandbox | [Configure a sandbox](lc:langsmith/managed-deep-agents-deploy#configure-a-sandbox) |
...
- [Quickstart](lc:langsmith/managed-deep-agents-quickstart): create and deploy a first agent.
- [Tutorial](lc:langsmith/managed-deep-agents-tutorial): build an agent step by step.
- [How Managed Deep Agents work](lc:langsmith/managed-deep-agents-how-it-works): compilation, deploy lifecycle, and Context Hub.
