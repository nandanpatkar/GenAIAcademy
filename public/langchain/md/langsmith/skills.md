...
[
 {
  "label": "Local (current project)",
  "lang": "bash",
  "code": "npx skills add langchain-ai/langsmith-skills --skill '*' --yes"
 },
 {
  "label": "Global (all projects)",
  "lang": "bash",
  "code": "npx skills add langchain-ai/langsmith-skills --skill '*' --yes --global"
 },
 {
  "label": "Link to a specific agent (e.g., Claude Code)",
  "lang": "bash",
  "code": "npx skills add langchain-ai/langsmith-skills --agent claude-code --skill '*' --yes --global"
 }
]
```
...
After installing the skills, set environment variables used by all LangSmith skills, helper scripts, and the [LangSmith CLI](lc:langsmith/langsmith-cli):
...
- [Traces](lc:langsmith/observability-concepts#traces): Add tracing to apps; list, filter, inspect, and export traces for debugging and analysis.
- [Datasets](lc:langsmith/evaluation-concepts#datasets): Turn traces into evaluation datasets (final_response, single_step, trajectory, RAG) and optionally upload to LangSmith.
- [Evaluators](lc:langsmith/evaluation-concepts#evaluators): Define code or LLM‑as‑judge evaluators and attach them to datasets (offline) or projects (online).

Each skill directory ships with a `SKILL.md` plus optional `scripts/` helpers you can run or adapt. These skills are designed to plug into compatible coding agents (such as Claude Code or Deep Agents Code), though you can also reuse the helper scripts directly if you prefer not to wire up a full agent. For heavier querying, exports, or automation, you can pair these skills with the [LangSmith CLI](lc:langsmith/langsmith-cli) to script against the same projects, datasets, and evaluators from your terminal.
