...
Coupled with [LangSmith's prebuilt JSON schema types](lc:langsmith/dataset-json-types), these allow you to do easy preprocessing of your data before saving it into your datasets.
...
The LLM run collection schema is built to collect data from LangChain `BaseChatModel` runs or traced runs from the [LangSmith OpenAI wrapper](lc:langsmith/annotate-code#use-%40traceable-%2F-traceable).
...
For enablement on new datasets, see our [dataset management how-to guide](lc:langsmith/manage-datasets-in-application).
