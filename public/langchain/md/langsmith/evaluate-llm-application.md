...
> [!NOTE]
>
> [Evaluations](lc:langsmith/evaluation-concepts#evaluation-lifecycle) | [Evaluators](lc:langsmith/evaluation-concepts#evaluators) | [Datasets](lc:langsmith/evaluation-concepts#datasets)
...
We've optionally enabled tracing to capture the inputs and outputs of each step in the pipeline. To understand how to annotate your code for tracing, please refer to [Custom instrumentation](lc:langsmith/annotate-code).

## Create or select a dataset

We need a [Dataset](lc:langsmith/evaluation-concepts#datasets) to evaluate our application on. Our dataset will contain labeled [examples](lc:langsmith/evaluation-concepts#examples) of toxic and non-toxic text.
...
For more details on datasets, refer to the [Manage datasets](lc:langsmith/manage-datasets) page.
...
You can also define an evaluator in the [LangSmith UI](https://smith.langchain.com). You can [create evaluators in the UI](lc:langsmith/llm-as-judge) under the **Evaluators** tab. These evaluators will be [automatically triggered with every new experiment](lc:langsmith/bind-evaluator-to-dataset).
...
![Experiments table with the edit pencil icon visible on a hovered row.](/langchain/images/langsmith/images/experiments-table-edit-icon-light.png)


![Experiments table with the edit pencil icon visible on a hovered row.](/langchain/images/langsmith/images/experiments-table-edit-icon-dark.png)
...
![Edit Experiment panel showing metadata key-value pairs and the Add Metadata button.](/langchain/images/langsmith/images/edit-experiment-panel-light.png)


![Edit Experiment panel showing metadata key-value pairs and the Add Metadata button.](/langchain/images/langsmith/images/edit-experiment-panel-dark.png)
...
The reserved `models`, `prompts`, and `tools` keys automatically populate dedicated columns in the experiments table. Click a value in one of those columns to filter or group by it. For full details, see [Filter and group by models, prompts, and tools](lc:langsmith/analyze-an-experiment#filter-and-group-by-models-prompts-and-tools-in-the-experiments-tab-view).

## Explore the results

Each invocation of `evaluate()` creates an [experiment](lc:langsmith/evaluation-concepts#experiment) that you can view in the LangSmith UI or query via the SDK. See [Analyze an experiment](lc:langsmith/analyze-an-experiment) for more details.

Experiments run against a dataset are listed in the experiments table.


![Experiments table showing a list of experiments with columns for experiment name, description, dataset, feedback score, and more.](/langchain/images/langsmith/images/experiments-table-light.png)


![Experiments table showing a list of experiments with columns for experiment name, description, dataset, feedback score, and more.](/langchain/images/langsmith/images/experiments-table-dark.png)
...
![Experiment view showing a table of examples with columns for input, output, reference output, feedback score, and more.](/langchain/images/langsmith/images/experiment-view-light.png)


![Experiment view showing a table of examples with columns for input, output, reference output, feedback score, and more.](/langchain/images/langsmith/images/experiment-view-dark.png)
...
![Experiment view details panel showing the inputs, outputs, reference outputs, and trace for a single example.](/langchain/images/langsmith/images/experiment-view-details-panel-light.png)


![Experiment view details panel showing the inputs, outputs, reference outputs, and trace for a single example.](/langchain/images/langsmith/images/experiment-view-details-panel-dark.png)
...
* [Run an evaluation via the REST API](lc:langsmith/run-evals-api-only)
* [Run an evaluation from the Playground](lc:langsmith/run-evaluation-from-playground)
