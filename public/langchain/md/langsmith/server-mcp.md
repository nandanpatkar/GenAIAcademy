...
[Agent Server](lc:langsmith/agent-server) implements MCP using the [Streamable HTTP transport](https://spec.modelcontextprotocol.io/specification/2025-03-26/basic/transports/#streamable-http). This allows LangGraph **agents** to be exposed as **MCP tools**, making them usable with any MCP-compliant client supporting Streamable HTTP.

The MCP endpoint is available at `/mcp` on [Agent Server](lc:langsmith/agent-server).

You can set up [custom authentication middleware](lc:langsmith/custom-auth) to authenticate a user with an MCP server to get access to user-scoped tools within your LangSmith deployment.
...
[
 {
  "label": "pip",
  "lang": "bash",
  "code": "pip install \"langgraph-api>=0.2.3\" \"langgraph-sdk>=0.1.61\""
 },
 {
  "label": "uv",
  "lang": "bash",
  "code": "uv add \"langgraph-api>=0.2.3\" \"langgraph-sdk>=0.1.61\""
 }
]
```
...
The default [MessagesState](lc:oss/python/langgraph/graph-api#messagesstate) uses `AnyMessage`, which supports many message types but is too general for direct LLM exposure.
...
For more details, see the [low-level concepts guide](lc:oss/python/langgraph/graph-api#state).
...
>
> **Prerequisites**
> You have added your own [custom auth middleware](lc:langsmith/custom-auth) that populates the `langgraph_auth_user` object, making it accessible through configurable context for every node in your graph.
...
_This can also be done by [rebuilding your graph at runtime](lc:langsmith/graph-rebuild) to have a different configuration for a new run_
...
The `/mcp` endpoint uses the same authentication as the rest of the LangGraph API. Refer to the [authentication guide](lc:langsmith/auth) for setup details.
