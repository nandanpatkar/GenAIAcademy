When running LangSmith on [Amazon Web Services (AWS)](https://aws.amazon.com/), [self-hosted](lc:langsmith/self-hosted) mode deploys a complete LangSmith platform with observability functionality.
...
## Initial setup
  ...
  #### Step: Deploy to Kubernetes
    ...
    Follow the [Kubernetes installation guide](lc:langsmith/kubernetes). LangSmith is tested on Amazon Elastic Kubernetes Service (EKS).
...
Analytics database
  ...
  #### Step: Set up authentication
    ...
    **Key pages:**
    - [S3 IRSA configuration](lc:langsmith/self-host-blob-storage#amazon-s3)
    - [RDS IAM authentication](lc:langsmith/self-host-external-postgres#iam-authentication)
    - [ElastiCache IAM authentication](lc:langsmith/self-host-external-redis#iam-authentication)
...
![Architecture diagram showing AWS relations to LangSmith services](/langchain/images/langsmith/images/aws-architecture-self-hosted.png)
...
-  **Storage & databases:**
  ...
  - ClickHouse + [Amazon EBS](https://aws.amazon.com/ebs/): analytics and trace storage.
    - We recommend using an [externally managed ClickHouse solution](lc:langsmith/self-host-external-clickhouse) unless security or compliance reasons
...
- Configure your LangSmith instance to [export telemetry data](lc:langsmith/export-backend) and continuously monitor via [CloudWatch Logs](https://docs.aws.amazon.com/AmazonCloudWatch/latest/logs/WhatIsCloudWatchLogs.html).
- The preferred method to manage [LangSmith deployments](lc:langsmith/deployment) is to create a CI process that builds [Agent Server](lc:langsmith/agent-server) images and pushes them to [ECR](https://aws.amazon.com/ecr/). Create a test deployment for pull requests before deploying a new revision to staging or production upon PR merge.
...
- Evaluate model outputs via [LangSmith datasets](lc:langsmith/manage-datasets).
