Adding [instrumentation](lc:langsmith/observability-concepts#manual-instrumentation) directly to your code gives you precise control over which functions your application traces, what inputs and outputs are logged, and how your [trace](lc:langsmith/observability-concepts#traces) hierarchy is structured. The three core instrumentation approaches are:
...
For LangChain (Python or JS/TS), refer to the [LangChain-specific instructions](lc:langsmith/trace-with-langchain).

> 
If you're using an LLM provider or agent framework with a built-in LangSmith integration, refer to the [integrations overview](lc:langsmith/integrations) instead
...
- `LANGSMITH_API_KEY`: your [LangSmith API key](lc:langsmith/create-account-api-key).
- By default, LangSmith logs traces to a project named `default`. To log to a different project, set `LANGSMITH_PROJECT`. For more details, refer to [Log traces to a specific project](lc:langsmith/log-traces-to-project).

## Use `@traceable` / `traceable`

Apply `@traceable` (Python), [`traceable`](https://reference.langchain.com/javascript/langsmith/traceable) (TypeScript), `traceable` (Kotlin) or `Tracing.traceFunction` (Java) to any function to make it a traced run. LangSmith handles context propagation across nested calls automatically.
...
By default, LangSmith assigns a random ID to each run. You can override this when you need to know the run ID ahead of time (for example, to attach [feedback](lc:langsmith/attach-user-feedback) immediately after a run), correlate LangSmith runs with IDs from an external system, or make runs idempotent using a deterministic ID.
...
- `trace` context manager (Python only): Pass `run_id` directly to the `trace` context manager constructor:
...
- If you are using LangChain, refer to the [LangChain tracing guide](lc:langsmith/trace-with-langchain#ensure-all-traces-are-submitted-before-exiting).
- If you are using the [LangSmith SDK](lc:langsmith/reference) standalone, you can use the `flush` method before exit:
...
- [Observability concepts](lc:langsmith/observability-concepts): background on runs, traces, and the LangSmith data model
- [Run (span) data format](lc:langsmith/run-data-format): schema reference for run fields including `dotted_order`, `trace_id`, and `parent_run_id`
- [Log user feedback using the SDK](lc:langsmith/attach-user-feedback): common use case for pre-specifying a run ID
- [Access the current run (span) within a traced function](lc:langsmith/access-current-span): read or modify the active run from inside a trace
- [Log traces to a specific project](lc:langsmith/log-traces-to-project): route traces to a named project instead of `default`
- [Trace with API](lc:langsmith/trace-with-api): low-level REST API alternative to the SDK
