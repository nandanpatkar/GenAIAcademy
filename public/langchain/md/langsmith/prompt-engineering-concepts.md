...
> [!TIP]
>
> Learn more about template variable syntax and formatting options in the [Prompt template format](lc:langsmith/create-a-prompt#template-format) guide.
...
> 
Open the [Playground](https://smith.langchain.com/playground) to create and test your first prompt template. For a step-by-step, refer to [Create a prompt](lc:langsmith/create-a-prompt).
...
For details on how to use these formats in the Playground, see [Template format](lc:langsmith/create-a-prompt#template-format).
...
[Tools](lc:langsmith/use-tools) are interfaces the LLM can use to interact with the outside world. Tools consist of a name, description, and JSON schema of arguments used to call the tool.
...
In the UI, you can compare a commit with its previous version by toggling **Diff** in the top-right corner of the [Prompt detail page](lc:langsmith/manage-prompts#prompt-detail-page).
...
- **Environments**: The `staging` and `production` tags are reserved for the [Environments](lc:langsmith/manage-prompts#environments) feature, which lets you promote commits between named deployment targets and switch versions without changing your code.
...
> [!NOTE]
>
> **Not to be confused with resource tags**: Commit tags reference specific prompt versions. [Resource tags](lc:langsmith/set-up-resource-tags) are key-value pairs used to organize workspace resources.


For detailed information on creating and managing commit tags, see [Manage prompts](lc:langsmith/manage-prompts#commit-tags).
...
> 
Use the **[Chat](lc:langsmith/chat)** in the Playground to optimize prompts, generate tools, and create output schemas with AI assistance.

## Testing multiple prompts

You can add multiple prompts to your Playground to compare outputs and evaluate performance:

![Add prompt to Playground](/langchain/images/langsmith/images/add-prompt-to-playground.gif.poster.png "heavy:3.0:https://docs.langchain.com/langsmith/images/add-prompt-to-playground.gif")
...
![Test over dataset in Playground](/langchain/images/langsmith/images/test-over-dataset-in-playground.gif.poster.png "heavy:4.2:https://docs.langchain.com/langsmith/images/test-over-dataset-in-playground.gif")
