LangSmith uses a hierarchical structure to organize your work: [_organizations_](lc:langsmith/administration-overview#organizations), [_workspaces_](lc:langsmith/administration-overview#workspaces), [_applications_](lc:langsmith/administration-overview#applications), and [_resources_](lc:langsmith/administration-overview#resources). This structure lets you balance collaboration with access control, allowing you to choose the right level of isolation for your team's needs.

The LangSmith permission system builds on this hierarchy. With [role-based access control (RBAC)](lc:langsmith/rbac), user [permissions](lc:langsmith/organization-workspace-operations) are scoped to one or more workspaces, enforcing isolation between workspaces. With more fine-grained [attribute-based access control](lc:langsmith/organization-workspace-operations#access-policies) (ABAC), access can be further restricted or granted based on attributes such as tags or applications within a workspace (for example, allowing users to access only development resources or only resources associated with a specific application).
...
> [!TIP]
>
> For details on setting up organizations and workspaces, refer to [Set up hierarchy](lc:langsmith/set-up-hierarchy).
...
- **Pros:** A single workspace allows all team resources to be shared, making collaboration and iteration within a team straightforward. It also simplifies promotion from development to production. For example, the same [prompt](lc:langsmith/prompt-context-hub#prompts) can be versioned and promoted to production using tags, without copying or duplication.
- **Cons:** The primary trade-off is limited isolation between environments of the same team. Development, test, and production resources coexist within the same application, so teams must rely on tagging and conventions to avoid accidental impact on production. [RBAC](lc:langsmith/rbac) is scoped at the workspace level. [ABAC](lc:langsmith/organization-workspace-operations#access-policies) provides more granular permissions within a workspace by restricting access based on resource attributes, such as allowing a user to access only development resources.

## Collaborative workspaces

In this model (multiple teams per workspace), multiple teams share a single workspace within an organization and use applications and [ABAC](lc:langsmith/organization-workspace-operations#access-policies) to separate resources and govern access. As a result, shared resources such as [prompts](lc:langsmith/prompt-context-hub#prompts) and [deployments](lc:langsmith/deployment) can be reused across teams, while access to sensitive resources like [traces](lc:langsmith/observability-concepts#traces) and [datasets](lc:langsmith/evaluation-concepts#datasets) is limited to the owning team.
