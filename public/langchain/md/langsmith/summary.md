...
![summary_eval.png](/langchain/images/langsmith/images/summary-eval.png)
...
* `runs: list[Run]`: A list of the full [Run](lc:langsmith/run-data-format) objects generated by the two experiments on the given example. Use this if you need access to intermediate steps or metadata about each run.
* `examples: list[Example]`: All of the dataset [Example](lc:langsmith/example-data-format) objects, including the example inputs, outputs (if available), and metadata (if available).
