LangSmith provides several tools to help you manage your [_prompts_](lc:langsmith/prompt-engineering-concepts) effectively. This page describes the following features:
...
Select a prompt from the [**Prompts** table](lc:langsmith/create-a-prompt#view-your-prompts) to open its detail page, which uses a two-pane layout: commit history and environments appear on the left, and commit details appear on the right.
...
For more information on how to use prompts in code, refer to [Managing prompts programmatically](lc:langsmith/manage-prompts-programmatically).
...
- **Workspace authorized users** (default): any workspace user with the [`prompts:tag`](lc:langsmith/organization-workspace-operations#prompts) permission can create, update, and delete tags and delete the prompt.
...
![Commit Playground](/langchain/images/langsmith/images/commit-playground.png)
