When running LangSmith on [Microsoft Azure](https://azure.microsoft.com/), [self-hosted](lc:langsmith/self-hosted) mode deploys a complete LangSmith platform with observability functionality.
...
## Initial setup
  ...
  #### Step: Deploy to Kubernetes
    ...
    Follow the [Kubernetes installation guide](lc:langsmith/kubernetes). LangSmith is tested on Azure Kubernetes Service (AKS).
...
Analytics database
  ...
  #### Step: Set up authentication
    ...
    **Key pages:**
    - [Azure Blob managed identity](lc:langsmith/self-host-blob-storage#azure-blob-storage)
    - [Azure Database Entra authentication](lc:langsmith/self-host-external-postgres#iam-authentication)
    - [Azure Cache Entra authentication](lc:langsmith/self-host-external-redis#iam-authentication)
...
![Architecture diagram showing Azure relations to LangSmith services](/langchain/images/langsmith/images/azure-architecture-self-hosted.png)


![Architecture diagram showing Azure relations to LangSmith services](/langchain/images/langsmith/images/azure-architecture-self-hosted-dark.png)
...
- **Storage services**: The platform requires persistent storage for traces, metadata and caching. On Azure the recommended services are:
    ...
    -  **ClickHouse** for high-volume analytics of traces. We recommend using an [externally managed ClickHouse solution](lc:langsmith/self-host-external-clickhouse). If, for security or compliance reasons, that is not an option, deploy a ClickHouse cluster on AKS using the open-source operator. Ensure replication across [availability zones](https://learn.microsoft.com/en-us/azure/reliability/availability-zones-overview) for durability. Clickhouse is not required for a hybrid deployment.
...
Configure your LangSmith instance to [export telemetry data](lc:langsmith/export-backend) so you can use Azure's services to monitor it.
...
- The preferred method to manage [LangSmith deployments](lc:langsmith/deployment) is to create a CI process that builds [Agent Server](lc:langsmith/agent-server) images and pushes them to [Azure Container Registry](https://azure.microsoft.com/en-us/products/container-registry). Create a test deployment for pull requests before deploying a new revision to staging or production upon PR merge.
