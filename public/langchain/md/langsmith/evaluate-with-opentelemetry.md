...
* A dataset created in LangSmith with examples to evaluate. You can create a dataset via the [LangSmith UI](lc:langsmith/evaluation-concepts#datasets) or via the [SDK](lc:langsmith/manage-datasets-programmatically).
...
[
 {
  "label": "Python",
  "lang": "bash",
  "code": "pip install langsmith strands-agents strands-agents-tools opentelemetry-sdk opentelemetry-exporter-otlp"
 },
 {
  "label": "TypeScript",
  "lang": "bash",
  "code": "npm install langsmith @strands-agents/sdk @opentelemetry/api @opentelemetry/sdk-trace-node @opentelemetry/sdk-trace-base @opentelemetry/exporter-trace-otlp-http @opentelemetry/resources"
 }
]
```
...
> [!NOTE]
>
> If you're [self-hosting LangSmith](lc:langsmith/self-hosted), replace `OTEL_EXPORTER_OTLP_ENDPOINT` with your self-hosted URL and append `/api/v1/otel`. For example: `OTEL_EXPORTER_OTLP_ENDPOINT = "https://ai-company.com/api/v1/otel"`.
>
...
This guide assumes that a dataset has been created in LangSmith with examples to evaluate. You can create a dataset via the [LangSmith UI](lc:langsmith/evaluation-concepts#datasets) or via the [SDK](lc:langsmith/manage-datasets-programmatically).
...
To learn more about evaluators, see [Evaluators](lc:langsmith/evaluation-concepts#evaluators).
...
For details on setting up OpenTelemetry tracing with LangSmith, see [Trace with OpenTelemetry](lc:langsmith/trace-with-opentelemetry).
...
For a complete list of supported OpenTelemetry attributes, see [Trace with OpenTelemetry](lc:langsmith/trace-with-opentelemetry#supported-opentelemetry-attribute-and-event-mapping).
...
After running the evaluation, you can [analyze the experiment](lc:langsmith/analyze-an-experiment) in the LangSmith UI to see:
