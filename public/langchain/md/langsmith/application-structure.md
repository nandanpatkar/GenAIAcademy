...
For details on all supported keys in the JSON file, refer to the [LangGraph configuration file reference](lc:langsmith/cli#configuration-file).


> [!TIP]
>
> The [LangGraph CLI](lc:langsmith/cli) defaults to using the configuration file `langgraph.json` in the current directory.
...
To deploy with LangSmith, you need:
  ...
  #### Tab: Python
    ...
    1. **A compiled graph**: `Compile`[StateGraph.compile] the graph to create a deployable application.
...
For end-to-end examples, refer to the deployment guides for [Google ADK](lc:langsmith/deploy-google-adk) and [Claude Agent SDK, Strands, CrewAI, and AutoGen](lc:langsmith/deploy-other-frameworks). LangSmith added support for Google ADK through [`deployments-wrap-sdk`](https://pypi.org/project/deployments-wrap-sdk/), an extensible package for wrapping agent SDKs to run on LangSmith Deployment.

## Environment variables

If you're working with a deployed LangGraph application [locally](lc:langsmith/local-dev-testing), you can configure environment variables in the `env` key of the [configuration file](#configuration-file-concepts).
