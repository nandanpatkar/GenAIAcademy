...
Deployments use the newest `stable` version by default and are automatically updated to the newest `stable` version on each new revision. To pin to a specific version, set [`api_version`](lc:langsmith/cli#pinning-api-version) to the desired version in langgraph.json.
...
Docs: [DeltaChannel reference](lc:oss/python/langgraph/pregel#deltachannel)
...
Docs:
- [Agent Server API reference](lc:langsmith/server-api-ref)
- [LangGraph event streaming reference](lc:oss/python/langgraph/event-streaming)
