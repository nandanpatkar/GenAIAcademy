This example gets you from a local checkout to a deployed LangChain deep agent with a working chat UI. The backend runs as a [LangSmith Deployment](lc:langsmith/deployment), and the frontend is a Vite + React app that streams from it.
...
- A [LangSmith API key](lc:langsmith/create-account-api-key) with deployment access.
...
#### Step: Deploy the agent to LangSmith
...
#### Step: Copy the deployment API URL
...
#### Step: Test the UI against the remote deployment
...
See the [Agent Server API reference](lc:langsmith/server-api-ref) for the underlying thread and streaming APIs.
...
- [Frameworks and platforms overview](lc:langsmith/deploy-frameworks-and-platforms)
- [LangSmith Deployment overview](lc:langsmith/deployment)
- [LangGraph CLI](lc:langsmith/cli)
- [Deep Agents going to production](lc:oss/python/deepagents/going-to-production)
