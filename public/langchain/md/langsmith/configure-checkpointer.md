[Agent Server](lc:langsmith/agent-server) persists graph state using a checkpointer backend. By default, LangSmith stores checkpoints in PostgreSQL alongside other server data. You can switch to MongoDB or provide a custom implementation.


> [!NOTE]
>
> Regardless of the checkpointer backend, LangSmith always requires PostgreSQL for threads, runs, assistants, crons, and the [memory store](lc:oss/python/langgraph/stores). The checkpointer backend only controls where checkpoint data is stored.
...
| `custom` | User-provided | `langgraph.json` | Custom storage backends (see [custom checkpointer](lc:langsmith/custom-checkpointer)) |

## Default (PostgreSQL)

PostgreSQL is the default checkpointer backend. No configuration is needed. To use a custom PostgreSQL instance, set the [`POSTGRES_URI_CUSTOM`](lc:langsmith/env-var-self-hosted) environment variable.
...
### Deploy by environment
  ...
  **External MongoDB** (production):
  ...
  See the [standalone server guide](lc:langsmith/deploy-standalone-server) for a full Docker Compose example with MongoDB.
...
To use a storage backend other than PostgreSQL or MongoDB, implement a custom `BaseCheckpointSaver`. See [Add custom checkpointer](lc:langsmith/custom-checkpointer) for details.

## Related

- [Configure TTLs](lc:langsmith/configure-ttl) for checkpoint and store item expiration
- [Persistence concepts](lc:oss/python/langgraph/persistence) in LangGraph
- [Data plane](lc:langsmith/data-plane) architecture
- [Environment variables](lc:langsmith/env-var-cloud) reference
