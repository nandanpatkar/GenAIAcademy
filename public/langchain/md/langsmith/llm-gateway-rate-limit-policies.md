...
A rate limit policy restricts how many **requests** or **tokens** a subject can consume through the [LLM Gateway](lc:langsmith/llm-gateway) in a short rolling time window. The gateway enforces the limit in real time and blocks any request that would push the subject past it, returning a `429` response with a `Retry-After` header:
...
Rate limit policies and [spend cap policies](lc:langsmith/llm-gateway-spend-policies) are complementary and can be applied together—spend caps for cost control, rate limits for throughput and traffic control.

## Policy dimensions

Rate limit policies are evaluated for every incoming request. You can set a policy as a default (applying a blanket rate limit to all users, [workspaces](lc:langsmith/administration-overview#workspaces), or [API keys](lc:langsmith/create-account-api-key) or as a granular policy (an individual limit or a limit on a group of subjects).
...
> [!WARNING]
>
> Creating and managing policies requires `organization:manage` permission. For the full permissions breakdown, refer to [Traces, Engine, and access control](lc:langsmith/llm-gateway-access).
...
A rate limit policy can also carry a condition on a custom request header, so traffic from a single subject splits into separate limits by header value. Use this to give each of your own end customers its own throughput allowance under one API key. For more information, see [Per-customer policies](lc:langsmith/llm-gateway-header-policies).

## Next steps

- [Spend policies](lc:langsmith/llm-gateway-spend-policies): set cost caps alongside rate limits.
- [Per-customer policies](lc:langsmith/llm-gateway-header-policies): split a limit by a custom request header so each end customer gets its own allowance.
- [Data protection](lc:langsmith/llm-gateway-data-protection): add data protection policies.
