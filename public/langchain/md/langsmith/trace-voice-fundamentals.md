[Tracing](lc:langsmith/observability-concepts#traces) a voice agent is different from tracing a text agent. A conversation is continuous, bidirectional, and interruptible: users talk over the agent, change topics mid-sentence, and expect sub-second responses. To debug and evaluate these systems, your traces need to capture the conversation as a single, audio-aware unit rather than a series of disconnected text exchanges.

This page covers the core conventions for tracing voice applications in LangSmith. Follow these patterns regardless of which framework or model provider you use ([OpenAI Realtime](lc:langsmith/trace-openai-realtime), [Gemini Live](lc:langsmith/trace-gemini-live), [LiveKit](lc:langsmith/trace-with-livekit), [Pipecat](lc:langsmith/trace-with-pipecat), or your own).


> [!NOTE]
>
> These conventions assume you are exporting traces to LangSmith through one of the supported [tracing setups](lc:langsmith/observability). For audio rendering and playback in the UI, see [Log multimodal traces](lc:langsmith/log-multimodal-traces) and [Upload files with traces](lc:langsmith/upload-files-with-traces).
...
The two most common frameworks for building cascade voice agents are [LiveKit](lc:langsmith/trace-with-livekit) and [Pipecat](lc:langsmith/trace-with-pipecat).

### Speech-to-speech (S2S)

A speech-to-speech model (such as the [OpenAI Realtime API](lc:langsmith/trace-openai-realtime) or [Gemini Live](lc:langsmith/trace-gemini-live)) processes audio natively and replies with audio over a single persistent connection, typically a WebSocket. There is no STT/LLM/TTS decomposition to trace.
...
The rest of this page describes conventions that apply to both architectures. The provider guides cover the event-stream specifics for [OpenAI Realtime](lc:langsmith/trace-openai-realtime) and [Gemini Live](lc:langsmith/trace-gemini-live).
...
For background on grouping related runs, see [Nest traces](lc:langsmith/nest-traces). To group several separate sessions for one user, use [Threads](lc:langsmith/threads).
...
Attach the file using the [attachments API](lc:langsmith/upload-files-with-traces):
...
Set the `ls_modality` metadata field to `"audio"` on the root run. This flags the trace as a voice trace so LangSmith can render it appropriately and so you can [filter](lc:langsmith/filter-traces-in-application) for voice traces in your project.
...
> [!NOTE]
>
> For other `ls_` metadata fields, refer to [Metadata parameters reference](lc:langsmith/ls-metadata-parameters).
