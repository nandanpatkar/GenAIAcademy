...
The A2A endpoint is available in [Agent Server](lc:langsmith/agent-server) at `/a2a/{assistant_id}`.
...
Once your agents are running locally via `langgraph dev` or [deployed to production](lc:langsmith/deployment), you can facilitate communication between them using the A2A protocol.
...
When multiple agents communicate over A2A, LangSmith can group all their [traces](lc:langsmith/observability-concepts#traces) into a single [thread](lc:langsmith/observability-concepts#threads), which gives you a unified view of the entire multi-agent conversation.
...
1. Agent Server maps the `contextId` to `thread_id` in LangSmith [metadata](lc:langsmith/add-metadata-tags), so all turns appear in the same thread.
