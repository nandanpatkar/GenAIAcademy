The `mda` CLI tests and deploys code-first [Managed Deep Agents](lc:langsmith/managed-deep-agents-overview). It is included with the `managed-deepagents` npm and Python packages.
...
For the fastest end-to-end path, see the [quickstart](lc:langsmith/managed-deep-agents-quickstart). For workflow guidance, see [Identity](lc:langsmith/managed-deep-agents-identity), [Evals](lc:langsmith/managed-deep-agents-evals), [Custom tools](lc:langsmith/managed-deep-agents-tools), [Custom middleware](lc:langsmith/managed-deep-agents-middleware), [Connectors](lc:langsmith/managed-deep-agents-connectors/index), [Schedules](lc:langsmith/managed-deep-agents-schedules), and [Deploy an agent](lc:langsmith/managed-deep-agents-deploy).
...
For task layout, verifiers, identity fixtures, and running Harbor, see [Evals](lc:langsmith/managed-deep-agents-evals).
...
On success, the CLI prints the LangSmith deployment dashboard URL. For secrets routing and deploy tips, see [Deploy an agent](lc:langsmith/managed-deep-agents-deploy).
...
The agent entry must export a named `agent` definition created with `define_deep_agent` or `defineDeepAgent`. For a minimal example, see the [quickstart](lc:langsmith/managed-deep-agents-quickstart#edit-the-agent).
...
For examples, see [Custom tools](lc:langsmith/managed-deep-agents-tools) and [Custom middleware](lc:langsmith/managed-deep-agents-middleware).
...
When present, `mda` generates the custom auth handler, injects it into the compiled app, and scopes threads, memory, and store access from the declaration. Projects without identity keep the previous compile output. For presets, ingress modes, guest tokens, and `runtime.identity`, see [Identity](lc:langsmith/managed-deep-agents-identity).
...
Managed memory lives in the same Context Hub repo as the deployed instructions and skills. The runtime remounts a scoped tree as `/memories/user/` (hot `/memories/user/AGENTS.md` plus optional cold files) and optional org facts as `/memories/org/` (read-only). Deploy seeds agent memory when needed and syncs `instructions.md` and `skills/**`, but does not overwrite existing Context Hub `memories/**` files. For hot/cold tiers, identity remounts, org memory, and `disableMemory`, see [Memory](lc:langsmith/managed-deep-agents-memory).
...
- **LangSmith:** `connectors/langsmith.ts` or `connectors/langsmith.py` declares constrained LangSmith capabilities for untrusted callers. Requires [identity](lc:langsmith/managed-deep-agents-identity). The browser never receives `LANGSMITH_API_KEY`.

For examples and defaults, see [Connectors](lc:langsmith/managed-deep-agents-connectors/index).

### Channels

Declare messaging channels as modules directly under `channels/`. Each file exports a named `channel` (for example `defineSlackChannel` / `defineGitHubChannel`). The file stem becomes the channel name and mounts `POST /channels/{name}/events` on the Agent Server. Channels require a root [identity](lc:langsmith/managed-deep-agents-identity) declaration.
...
For handlers, triggers, and provider setup, see [Channels](lc:langsmith/managed-deep-agents-channels/index), [Slack](lc:langsmith/managed-deep-agents-channels/slack), and [GitHub](lc:langsmith/managed-deep-agents-channels/github).
...
Deploy extracts schedule declarations from static literals, arrays, objects, and top-level literal constants. A schedule can deliver its final response to a configured Slack channel with `deliver_to` / `deliverTo`. After the deployment reaches `DEPLOYED`, `mda deploy` replaces the existing managed LangSmith cron jobs with the current local schedule declarations. For examples and constraints, see [Schedules](lc:langsmith/managed-deep-agents-schedules).
...
For configuration examples and lifecycle behavior, see [Configure a sandbox](lc:langsmith/managed-deep-agents-deploy#configure-a-sandbox).

### Evals

Put Harbor-style eval tasks under `evals/`. Each task directory includes `instruction.md`, `task.toml`, an `environment/` image, and a `tests/test.sh` verifier. When the project declares [identity](lc:langsmith/managed-deep-agents-identity), each task also needs `identity.json`.

`mda init` scaffolds starter evals under `evals/`. For existing projects, use `mda evals init`. Compile a Harbor handoff with `mda evals compile`, then run trials with Harbor. Artifacts land under `.mda/evals/` and are not part of the deploy archive. For the full workflow, see [Evals](lc:langsmith/managed-deep-agents-evals).
