[Online evaluations](lc:langsmith/evaluation-concepts#online-evaluations) provide real-time feedback on your production [traces](lc:langsmith/observability-concepts#traces). This is useful to continuously monitor the performance of your application: identify issues, measure improvements, and ensure consistent quality over time.
...
Filters on evaluators work the same way as when you're filtering traces in a project. For more information on filters, you can refer to [Filter traces](lc:langsmith/filter-traces-in-application).
...
- A `Run` ([reference](lc:langsmith/run-data-format)). This represents the sampled run to evaluate.
