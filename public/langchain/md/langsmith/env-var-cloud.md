...
For advanced CORS configuration, see [how to add custom CORS configuration](lc:langsmith/cli#customizing-http-middleware-and-headers).
...
Sets the default [checkpointer backend](lc:langsmith/configure-checkpointer) for agent servers that don't specify one in `langgraph.json`. Accepted values: `"default"` (PostgreSQL), `"mongo"`, `"custom"`.
...
> [!NOTE]
>
> For selective tracing control based on runtime conditions (such as per-client requirements or data sensitivity), see [Conditional tracing](lc:langsmith/conditional-tracing).
...
This limits concurrent run execution, not the number of API requests your deployment can serve. Request-serving capacity is handled by API servers and scales independently of this value. For tuning guidance, see [Configure Agent Server for scale](lc:langsmith/agent-server-scale).
...
See [Configure checkpointer backend](lc:langsmith/configure-checkpointer) for details.
...
{/* Placeholder. No Cloud-exclusive Agent Server environment variables today. Add new sections here as they appear. */}


The Agent Server supports the following environment variables when deployed on [Cloud](lc:langsmith/deploy-to-cloud-overview). For variables specific to self-hosted deployments, see [Self-hosted Agent Server environment variables](lc:langsmith/env-var-self-hosted).
