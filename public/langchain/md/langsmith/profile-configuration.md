LangSmith SDK profiles let you keep [API keys](lc:langsmith/create-account-api-key), endpoints, and workspace IDs in a reusable JSON file instead of setting the same environment variables in every shell session.

Use profiles when you switch between [LangSmith Cloud regions](lc:langsmith/cloud#regional-storage), self-hosted instances, or [workspaces](lc:langsmith/administration-overview#workspaces) often, or when you want a remote runtime to load the same SDK configuration from a mounted file.
...
By default, [SDKs](lc:langsmith/reference) look for a profile file at:
...
| `api_key` | LangSmith API key. See [Create an account and API key](lc:langsmith/create-account-api-key). |
...
The [LangSmith CLI](lc:langsmith/langsmith-cli) also accepts a global `--profile` flag, which takes precedence over `LANGSMITH_PROFILE` for that command:
...
Use the [LangSmith CLI](lc:langsmith/langsmith-cli) to create, inspect, switch, and delete profiles without editing the JSON file by hand.
...
For hosted [LangSmith Cloud](lc:langsmith/cloud), configure these values as environment variables or [workspace secrets](lc:langsmith/set-up-hierarchy#configure-workspace-settings) unless the platform explicitly supports mounting secret files.
