...
> [!NOTE]
>
> To cap LLM cost on evaluator runs, refer to [Track and limit evaluator spend](lc:langsmith/evaluator-spend). Evaluator spend tracking and limits use the per-model pricing configured under [Model pricing](#create-a-new-or-modify-an-existing-model-price-entry).


## View costs in the LangSmith UI

In the [LangSmith UI](https://smith.langchain.com), you can explore usage and spend three ways: as a breakdown within individual traces, as aggregated metrics in project stats, and in dashboards.
...
![Cost tooltip](/langchain/images/langsmith/images/cost-tooltip-light.png)


![Cost tooltip](/langchain/images/langsmith/images/cost-tooltip-dark.png)
...
Dashboards help you explore cost and token usage trends over time. The [prebuilt dashboard](lc:langsmith/dashboards#prebuilt-dashboards) for a tracing project shows total costs and a cost breakdown by input and output tokens.
...
|--------|---------------|-----------------|
| **Automatically** | <ul><li>Calling LLMs with [LangChain](lc:oss/python/langchain/overview)</li><li>Tracing LLM calls to OpenAI, Anthropic or models that follow an OpenAI-compliant format with `@traceable`</li><li> Using LangSmith wrappers for [OpenAI](lc:langsmith/trace-openai) or [Anthropic](lc:langsmith/trace-anthropic)</li><li>For other model providers, read the [token and cost information guide](lc:langsmith/log-llm-trace#provide-token-and-cost-information)</li></ul> | Not applicable. |
...
> [!NOTE]
>
> Skip this section if you are calling LLMs with [LangChain](lc:oss/python/langchain/overview), using `@traceable` with OpenAI or Anthropic (or an OpenAI-compatible model), or using a LangSmith wrapper for [OpenAI](lc:langsmith/trace-openai) or [Anthropic](lc:langsmith/trace-anthropic).
...
2. Specify model name. When using a custom model, the following fields need to be specified in a [run's metadata](lc:langsmith/add-metadata-tags) in order to associate token counts with costs. It's also helpful to provide these metadata fields to identify the model when viewing traces and when filtering.
