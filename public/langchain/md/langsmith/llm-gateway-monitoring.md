The LLM Gateway **Spend Monitoring** dashboard shows how much LLM cost a [workspace](lc:langsmith/administration-overview#workspaces) has accrued through the gateway. Use it to compare spend over time and identify the users, [API keys](lc:langsmith/create-account-api-key), and models that account for that spend. The dashboard covers one workspace at a time; switch workspaces to compare them.

Viewing the dashboard requires the [Organization Admin](lc:langsmith/rbac#organization-admin) role and a Plus or Enterprise [plan](lc:langsmith/pricing-plans). Without both, the **Usage** tab does not appear.
...
- [LLM Gateway overview](lc:langsmith/llm-gateway)
- [Configure spend policies](lc:langsmith/llm-gateway-spend-policies)
