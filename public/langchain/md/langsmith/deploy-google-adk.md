This guide shows you how to deploy a [Google Agent Development Kit (ADK)](https://github.com/google/adk-python) agent on [LangSmith Agent Server](lc:langsmith/agent-server) using the [`deployments-wrap-sdk`](https://pypi.org/project/deployments-wrap-sdk/) package.

`deployments-wrap-sdk` provides a thin wrapper that turns a configured ADK `Runner` into a LangGraph-compatible graph, so you can deploy ADK agents without writing the [Functional API](lc:oss/python/langgraph/functional-api) glue yourself. The wrapper:

- Bridges ADK sessions to Agent Server's [checkpoint persistence](lc:langsmith/agent-server#persistence), so session state survives restarts and resumes across runs.
- Forwards ADK token events through LangGraph's streaming pipeline, so partial tokens show up in [`stream_mode="messages"`](lc:langsmith/streaming) and in [LangSmith Studio](lc:langsmith/studio).
- Automatically enables [LangSmith tracing](lc:langsmith/trace-with-google-adk) for ADK when `LANGSMITH_TRACING` is set.

## Prerequisites

- Python 3.11+
- [LangGraph CLI](lc:langsmith/cli) for local dev and deployment
...
- **Authentication**: if Agent Server [authentication](lc:langsmith/auth) is enabled, the authenticated user id becomes ADK's `user_id`. Otherwise the user id is `"anonymous"`.
...
- **Intermediate events as messages**: the response is emitted as one `AIMessage` containing the concatenated text. Tool calls, tool results, and intermediate sub-agent turns are not exposed as separate items in the graph's `messages` field. Inspect them in [LangSmith traces](lc:langsmith/observability) instead.
...
This serves the agent at `http://127.0.0.1:2024` and opens [LangSmith Studio](lc:langsmith/studio) so you can chat with the agent. Send a request directly with `curl`:
...
For environment configuration, deployment types, and revision management, refer to [Deploy to cloud](lc:langsmith/deploy-to-cloud). For self-hosted setups, refer to [Self-hosted deployments](lc:langsmith/self-hosted).
...
[Traces](lc:langsmith/observability) show agent invocations, tool calls, and LLM interactions in the [LangSmith UI](https://smith.langchain.com). For more on the underlying tracing integration, see [Trace Google ADK applications](lc:langsmith/trace-with-google-adk).
...
1. The wrapped graph reads `thread_id` from the run config and uses it as the ADK `session_id`. If [authentication](lc:langsmith/auth) is enabled, the authenticated user's id becomes the ADK `user_id`; otherwise the user id is `"anonymous"`.
