This guide covers two approaches to tracing with the [REST API](lc:langsmith/smith-api-ref): basic tracing using the `POST /runs` and `PATCH /runs` endpoints, and batch ingestion using `POST /runs/multipart` for higher throughput.

For a full list of endpoints and request/response schemas, refer to the [API reference](lc:langsmith/smith-api-ref).


> [!WARNING]
>
> We strongly recommend using the [Python](lc:langsmith/smith-python-sdk) or [TypeScript](lc:langsmith/smith-js-ts-sdk) SDK to send traces to LangSmith instead of the REST API directly. The SDKs include batching and background sending optimizations that prevent tracing from affecting your application's performance.
>
...
> [!NOTE]
>
> We recommend using **UUID v7** for run IDs. UUIDv7 embeds a timestamp, which preserves correct time-ordering of runs in a trace. Use `uuid7()` from the LangSmith SDK to generate them, or see [Specify a custom run ID](lc:langsmith/annotate-code#specify-a-custom-run-id) for more details.
...
> [!NOTE]
>
> When using the LangSmith REST API, provide your [API key](lc:langsmith/create-account-api-key) in the request headers as `"x-api-key"`.
>
...
The following example traces a chat completion with a parent chain run and a child LLM run. Set [`parent_run_id`](lc:langsmith/run-data-format) on a child run to attach it to its parent:
...
For more information, refer to [Run (span) data format](lc:langsmith/run-data-format).
...
Unlike basic tracing, this endpoint requires you to compute and set [`dotted_order`](lc:langsmith/run-data-format#what-is-dotted_order) and [`trace_id`](lc:langsmith/run-data-format) yourself. `dotted_order` encodes each run's timestamp and UUID with parent and child entries joined by dots (e.g., `20240101T000000Z<parent-uuid>.20240101T000001Z<child-uuid>`), telling LangSmith how runs relate and in what order they occurred. `trace_id` is the UUID of the root run.
...
- [Run (span) data format](lc:langsmith/run-data-format)
- [Specify a custom run ID](lc:langsmith/annotate-code#specify-a-custom-run-id)
- [Custom instrumentation](lc:langsmith/annotate-code)
