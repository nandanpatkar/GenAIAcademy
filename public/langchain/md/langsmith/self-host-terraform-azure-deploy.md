...
> [!TIP]
>
> For a condensed cheat sheet of `make` targets, required variables, and common constraints, see the [Azure quick reference](lc:langsmith/self-host-terraform-azure-quick-reference).
...
> [!WARNING]
>
> In-cluster ClickHouse runs as a single pod with no replication or backups, dev/POC only. For production, use [LangChain Managed ClickHouse](lc:langsmith/langsmith-managed-clickhouse).
...
For all variables, see the [Azure variables reference](lc:langsmith/self-host-terraform-azure-variables).
...
Enables [LangSmith Deployment](lc:langsmith/deploy-self-hosted-full-platform), which lets you deploy and manage agents as API servers directly from the [LangSmith UI](https://smith.langchain.com). This adds three new pods.
...
- Reference the [Azure variables](lc:langsmith/self-host-terraform-azure-variables) and the [quick reference](lc:langsmith/self-host-terraform-azure-quick-reference).
- Review the [Azure architecture](lc:langsmith/self-host-terraform-azure-architecture) for module structure, traffic flow, and Workload Identity.
- When something breaks, check the [Azure troubleshooting guide](lc:langsmith/self-host-terraform-azure-troubleshooting).
