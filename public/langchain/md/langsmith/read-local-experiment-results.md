When running [evaluations](lc:langsmith/evaluation-concepts), you may want to process results programmatically in your script rather than viewing them in the [LangSmith UI](https://smith.langchain.com). This is useful for scenarios like:
...
This guide shows you how to iterate over and process [experiment](lc:langsmith/evaluation-concepts#experiment) results from the `ExperimentResults`[ExperimentResults] object returned by `Client.evaluate()`[Client.evaluate].
...
>
> If you want to run evaluations locally **without** recording anything to LangSmith (for quick testing or validation), refer to [Run an evaluation locally](lc:langsmith/local) which uses `upload_results=False`.
...
- `result["run"]`: The execution of your target function.
  - `result["run"].inputs`: The inputs from your [dataset](lc:langsmith/evaluation-concepts#datasets) example.
...
For more information on running evaluations without uploading results, refer to [Run an evaluation locally](lc:langsmith/local).

## Related

- [Evaluate your LLM application](lc:langsmith/evaluate-llm-application)
- [Run an evaluation locally](lc:langsmith/local)
- [Fetch performance metrics from an experiment](lc:langsmith/fetch-perf-metrics-experiment)
