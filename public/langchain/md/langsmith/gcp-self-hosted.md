When running LangSmith on [Google Cloud Platform (GCP)](https://cloud.google.com/), [self-hosted](lc:langsmith/self-hosted) mode deploys a complete LangSmith platform with observability functionality.
...
## Initial setup
  ...
  #### Step: Deploy to Kubernetes
    ...
    Follow the [Kubernetes installation guide](lc:langsmith/kubernetes). LangSmith is tested on Google Kubernetes Engine (GKE).
...
Analytics database
  ...
  #### Step: Set up authentication
    ...
    **Key pages:**
    - [GCS HMAC key authentication](lc:langsmith/self-host-blob-storage#google-cloud-storage)
    - [Cloud SQL IAM authentication](lc:langsmith/self-host-external-postgres#iam-authentication)
    - [Memorystore IAM authentication](lc:langsmith/self-host-external-redis#iam-authentication)
...
![Architecture diagram showing GCP relations to LangSmith services](/langchain/images/langsmith/images/gcp-architecture-self-hosted.png)
...
- Configure your LangSmith instance to [export telemetry data](lc:langsmith/export-backend) and continuously monitor via [Cloud Logging](https://cloud.google.com/logging).
- The preferred method to manage [LangSmith deployments](lc:langsmith/deployment) is to create a CI process that builds [Agent Server](lc:langsmith/agent-server) images and pushes them to [Artifact Registry](https://cloud.google.com/artifact-registry). Create a test deployment for pull requests before deploying a new revision to staging or production upon PR merge.
