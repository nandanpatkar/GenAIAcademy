A _run_ is an invocation of an [assistant](lc:langsmith/assistants). When you execute a run, you specify which assistant to use—either by graph ID for the default assistant, or by assistant ID for a specific configuration.
...
- Runs can be stateless (no thread) or stateful (executed on a [thread](lc:langsmith/use-threads) for conversation persistence).
...
The Agent Server API provides several endpoints for creating and managing runs. For more details, refer to the [API reference](lc:langsmith/server-api-ref).
