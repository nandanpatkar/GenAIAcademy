Dashboards give you high-level insights into your [trace](lc:langsmith/observability-concepts#traces) data, helping you spot trends and monitor the health of your applications. Dashboards are available in **Monitoring** in the [LangSmith UI](https://smith.langchain.com).

LangSmith offers two dashboard types:

- **Prebuilt dashboards**: Automatically generated for every tracing project.
- **Custom dashboards**: Collections of charts you can configure to your needs. Two experiences are available depending on your [platform setup](lc:langsmith/platform-setup):
...
| :------ | :------------ |
| Traces | Trace count, latency and error rates. A [trace](lc:langsmith/observability-concepts#traces) is a collection of [runs](lc:langsmith/observability-concepts#runs) related to a single operation. For example, if a user request triggers an agent, all runs for that agent invocation would be part of the same trace. |
| LLM Calls | LLM call count and latency. Includes all runs where run type is "llm". |
| Cost & Tokens | Total and per-trace token counts and costs, broken down by token type. Costs are measured using [LangSmith's cost tracking](lc:langsmith/log-llm-trace#provide-token-and-cost-information). |
...
> [!NOTE]
>
> Available for LangSmith [Cloud](lc:langsmith/cloud) US.
...
When you add a filter, it defaults to filtering at the [run](lc:langsmith/observability-concepts#runs) level. To broaden the scope, open the filter picker, then the **Advanced** submenu at the bottom, and choose:

- **Trace filter**: filters at the [trace](lc:langsmith/observability-concepts#traces) (root-run) level.
...
Dataset sources do not expose run/trace/tree filters. Data is scoped by the selected dataset. For filter syntax, refer to [filtering traces in application](lc:langsmith/filter-traces-in-application).

**Grouping** creates multiple series on the same chart in one of two ways:

1. **Group by**: Automatically splits data into series based on one attribute. Available attributes: Run Name, Run Type, Tag, Project, [Metadata](lc:langsmith/add-metadata-tags) (with a path such as `metadata.ls_model_name`), and Feedback Label. Groups are ranked by frequency and capped at the top 20.
...
> [!NOTE]
>
> Available for LangSmith [Self-hosted](lc:langsmith/self-hosted) and LangSmith [Cloud](lc:langsmith/cloud) EU/APAC customers.
...
- Use the **Chart filters** section to refine the matching runs. This filter applies to all data series in the chart. For more information, view the guide on [filtering traces in application](lc:langsmith/filter-traces-in-application).
...
1. **Group by**: Group runs by [run tag or metadata](lc:langsmith/add-metadata-tags), run name, or run type. Group by automatically splits the data into multiple series based on the field selected. Group by defaults to the top 5 elements by frequency, configurable up to 20.
...
1. **Data Series**: Create a data series for each decision made at the `triage_input` node. The output of the decision is stored in the `triage.response` field of the output object, and the value of the decision is either `no`, `email`, or `notify`. Each of these decisions generates a separate data series in the chart.
   ![Decision at node](/langchain/images/langsmith/images/decision-at-node.png)
