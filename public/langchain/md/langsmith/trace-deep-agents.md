[`deepagents`](lc:oss/python/deepagents/overview) is an open-source agent framework built on top of LangGraph, designed for complex, multi-step tasks that require planning, tool usage, and sub-agent delegation. Deep Agents supports native LangSmith tracing.
...
- For tracing, a [LangSmith account and API key](lc:langsmith/create-account-api-key) (free to sign up).
...
Save the filter as a named view for quick reuse. For a full reference on filter options, see [Filter traces](lc:langsmith/filter-traces-in-application).
...
- `project_name="deepagents-demo"` routes traces from this block to the specified [LangSmith project](lc:langsmith/log-traces-to-project). This overrides `LANGSMITH_PROJECT` for runs created within the context.
- `tags=[...]` attaches tags to the traced runs. [Tags](lc:langsmith/add-metadata-tags) appear in the [LangSmith UI](https://smith.langchain.com), which you can use to filter and group traces.
