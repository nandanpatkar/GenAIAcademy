LangSmith provides flexible controls for managing how users join your [organization](lc:langsmith/administration-overview#organizations) when using [Single Sign-On (SSO) authentication](lc:langsmith/authentication-methods). You can independently enable or disable both Just-In-Time (JIT) provisioning and user invites to match your organization's security and onboarding requirements.
...
The `jit_provisioning_enabled` setting controls automatic user provisioning. When enabled, users who authenticate via your SSO provider are automatically added to your [organization](lc:langsmith/administration-overview#organizations) and assigned to default [workspaces](lc:langsmith/administration-overview#workspaces) with a default [role](lc:langsmith/rbac). For more details, refer to [Configure default SSO settings](#configure-default-sso-settings). When disabled, users must be explicitly invited or added via [SCIM](#scim-integration) before they can access the organization.

### Invites

The `invites_enabled` setting controls manual user invitations. When enabled, [organization administrators](lc:langsmith/administration-overview#organization-roles) can send invitations to users before they sign in. Invited users can claim their invite when signing in via SSO. When disabled, manual invitations are not allowed and users can only join via JIT provisioning or [SCIM](#scim-integration).
...
> [!NOTE]
>
> Consider the following if you are using [LangSmith self-hosted](lc:langsmith/self-hosted):
>
...
> - For additional self-hosted user management customizations, refer to [Customize user management](lc:langsmith/self-host-user-management).
...
1. Default workspace role. Choose the [workspace role](lc:langsmith/rbac#workspace-roles) that users receive when automatically provisioned. For details on what each role can do, refer to [Organization and workspace operations](lc:langsmith/organization-workspace-operations). Options include:

    - **[Viewer](lc:langsmith/rbac#workspace-viewer)**: Read-only access
    - **[User](lc:langsmith/rbac#organization-user)**: Standard access
    - **[Editor](lc:langsmith/rbac#workspace-editor)**: Can modify resources
    - **[Admin](lc:langsmith/rbac#workspace-admin)**: Full workspace control
...
If your organization uses [SCIM](lc:langsmith/user-management#set-up-scim-for-your-organization) (System for Cross-domain Identity Management), users can be automatically provisioned and managed through your identity provider. SCIM provides an additional mechanism for user management that works alongside JIT and invite settings.
...
[SSO Groups Sync](lc:langsmith/user-management#sso-groups-sync-alternative) is an alternative to SCIM that reads group memberships from the SSO token at login time and assigns org and workspace roles using the SCIM naming convention. The sync runs after JIT and invite resolution on each login, and owns only the memberships it created.
...
We recommend choosing one of SCIM or SSO Groups Sync per organization, not both, to avoid confusing precedence behavior. For configuration and tradeoffs, refer to [SSO Groups Sync](lc:langsmith/user-management#sso-groups-sync-alternative).

## Related documentation

- [Set up SSO with OAuth2.0 and OIDC](lc:langsmith/self-host-sso) (Self-hosted)
- [Set up SAML SSO](lc:langsmith/user-management#set-up-saml-sso-for-your-organization) (Cloud)
- [Set up SCIM](lc:langsmith/user-management#set-up-scim-for-your-organization)
- [User management](lc:langsmith/user-management)
- [Role-based access control](lc:langsmith/rbac)
