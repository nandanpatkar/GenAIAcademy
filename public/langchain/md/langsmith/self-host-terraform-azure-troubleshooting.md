...
> [!TIP]
>
> Before upgrading, review the [LangSmith self-hosted changelog](lc:langsmith/self-hosted-changelog) for breaking changes and required variable updates. Run `az aks get-credentials --name <cluster> --resource-group <rg>` before running any `kubectl` commands.
...
See the [architecture page](lc:langsmith/self-host-terraform-azure-architecture#workload-identity) for the full pod-to-WI mapping.
