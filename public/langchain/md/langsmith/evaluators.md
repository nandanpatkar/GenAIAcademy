[Evaluators](lc:langsmith/evaluation-concepts#evaluators) in LangSmith are [workspace-level](lc:langsmith/administration-overview#workspaces) resources. You can attach a single evaluator to multiple [tracing projects](lc:langsmith/observability-concepts#projects) and [datasets](lc:langsmith/evaluation-concepts#datasets), so you can apply consistent evaluation logic across your work without recreating it each time.


> [!TIP]
>
> The [LangSmith Engine](lc:langsmith/engine) suggests custom evaluators for detected issues and can deploy them with one click.
...
You can create an evaluator in the [LangSmith UI](https://smith.langchain.com) or programmatically with the [SDK](#create-an-evaluator-with-the-sdk). Evaluators created either way are workspace-level resources that appear in the **Evaluators** table.
...
1. The panel lets you:
   - **Create from scratch**: Build a new [LLM-as-a-Judge](lc:langsmith/llm-as-judge) or [Code](lc:langsmith/online-evaluations-code) evaluator.
...
You can also add an evaluator directly from a [tracing project](lc:langsmith/observability-concepts#projects) or [dataset](lc:langsmith/evaluation-concepts#datasets). In that flow, you can additionally **attach an existing evaluator** from your workspace, or create a [Composite](lc:langsmith/composite-evaluators-ui) evaluator. Refer to [Set up LLM-as-a-judge online evaluators](lc:langsmith/online-evaluations-llm-as-judge) and [Automatically run evaluators on experiments](lc:langsmith/bind-evaluator-to-dataset).

### Create an evaluator with the SDK

Use the LangSmith SDK to create evaluators programmatically. The SDK is available for [Python](lc:langsmith/smith-python-sdk) and [TypeScript](lc:langsmith/smith-js-ts-sdk). Evaluators created through the SDK appear in the **Evaluators** table alongside those created in the UI.
...
To create an LLM-as-a-judge evaluator and to retrieve, update, list, or delete evaluators, refer to [Manage evaluators with the SDK](lc:langsmith/manage-evaluators-sdk).
...
- **Projects & Datasets**: The tracing projects and datasets this evaluator is attached to, with each attachment's [weekly spend and limit](lc:langsmith/evaluator-spend).
...
When an online evaluator scores a trace, it attaches feedback to the trace. This can auto-upgrade the trace to [extended retention](lc:langsmith/usage-and-billing#data-retention-auto-upgrades), depending on the evaluator's retention setting. Extended retention keeps the trace longer but costs more. When you set up an online evaluator on a [tracing project](lc:langsmith/observability-concepts#projects), you can opt out of this upgrade so that scored traces stay at the project's base retention.

This control is available only when the project's [default retention](lc:langsmith/billing#change-project-level-default-retention) is the [base tier](lc:langsmith/usage-and-billing#how-it-works). If the project defaults to extended retention ([set at the project or workspace level](lc:langsmith/data-purging-compliance#data-retention)), traces scored by the evaluator follow that default and the option is locked.

To opt out of extending retention for scored traces:

1. When you [create](#create-an-evaluator) or [edit](#edit-an-evaluator) an online evaluator, set the source to a [tracing project](lc:langsmith/observability-concepts#projects), rather than a [dataset](lc:langsmith/evaluation-concepts#datasets).
...
The **Extend trace retention** toggle described above applies to both trace-level and thread-level (multi-turn) online evaluators. For more information on multi-turn evaluators, see [Set up multi-turn online evaluators](lc:langsmith/online-evaluations-multi-turn).
