...
For full details on bulk export configuration options, refer to [Bulk export trace data](lc:langsmith/data-export) and [Manage bulk export destinations](lc:langsmith/data-export-destinations).
...
You will need your [LangSmith API key](lc:langsmith/create-account-api-key) and [workspace ID](lc:langsmith/set-up-hierarchy#set-up-a-workspace).
...
LangSmith validates the credentials by performing a test write before saving the destination. If the request returns a `400` error, refer to [Debug destination errors](lc:langsmith/data-export-destinations#debug-destination-errors).
...
Bulk exports default to `zstandard` compression. This example sets `snappy` because Snappy is fast and widely supported by BigQuery. For all available options, refer to [Bulk export trace data](lc:langsmith/data-export#2-create-an-export-job), including field filtering and filter expressions.
...
### Create the table
  ...
  #### Tab: Native table
    ...
    BigQuery runs a load job to copy the data. The Hive partition columns appear as regular columns in the table. For the full list of available data columns, see [Exportable fields](lc:langsmith/data-export#exportable-fields).
...
For full details, see [Rotate destination credentials](lc:langsmith/data-export-destinations#rotate-destination-credentials).
...
For additional error codes and export status details, see [Monitor and troubleshoot bulk exports](lc:langsmith/data-export-monitor).
