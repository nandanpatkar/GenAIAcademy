...
Online evaluations target [_runs_](#runs) and [_threads_](#threads) from [tracing](lc:langsmith/observability-quickstart): real production traces without reference outputs.
...
Follow the [quickstart](lc:langsmith/evaluation-quickstart) to run your first offline evaluation.
...
Learn how to [configure online evaluations](lc:langsmith/online-evaluations-llm-as-judge) for production monitoring.
...
![List of datasets on the Examples tab in the LangSmith UI.](/langchain/images/langsmith/images/datasets-light.png)


![List of datasets on the Examples tab in the LangSmith UI.](/langchain/images/langsmith/images/datasets-dark.png)
...
![Example in the LangSmith UI.](/langchain/images/langsmith/images/example-light.png)


![Example in the LangSmith UI.](/langchain/images/langsmith/images/example-dark.png)


Learn more about [managing datasets](lc:langsmith/manage-datasets).
...
![Experiment view](/langchain/images/langsmith/images/experiment-view.png)

Multiple experiments typically run on a given dataset to test different application configurations (e.g., different prompts or LLMs). LangSmith displays all experiments associated with a dataset and supports [comparing multiple experiments](lc:langsmith/compare-experiment-results) side-by-side.

Learn [how to analyze experiment results](lc:langsmith/analyze-an-experiment).
...
A _run_ is a single execution trace from your [deployed application](lc:langsmith/deployment). Each run contains:
...
Learn more about [runs and traces in the Observability concepts](lc:langsmith/observability-concepts#runs).
...
- The [Evaluators](lc:langsmith/evaluators) page, to attach them to tracing projects or datasets
- The [Playground](lc:langsmith/prompt-engineering-concepts#playground)
...
- [Rules](lc:langsmith/rules), to run them automatically on tracing projects or datasets

### Attaching an evaluator to a tracing project or dataset

A single evaluator can be attached to many tracing projects and datasets. Configuration like sampling rate, filters, and [spend limits](lc:langsmith/evaluator-spend) is set per attached project or dataset, not per evaluator. View an evaluator's attached projects and datasets under its **Projects & Datasets** tab.
...
- [Run](lc:langsmith/observability-concepts#runs): The actual outputs and intermediate steps from running the application on the example inputs.

**Online evaluators** receive:
- [Run](lc:langsmith/observability-concepts#runs): The production trace containing inputs, outputs, and intermediate steps (no reference outputs available).
...
[Annotation queues](lc:langsmith/annotation-queues) streamline structured collection of human feedback on runs. They complement [inline annotation](lc:langsmith/annotate-traces-inline) by providing organized workflows with prescribed rubrics, team collaboration features, and progress tracking.

LangSmith supports two queue types:

- **Single-run queues**: Review one run at a time against custom rubric items. Useful for triaging issues or building datasets from production traces. Single-run queues also support [assertions](lc:langsmith/assertions), free-form acceptance criteria that an offline evaluator can grade future runs against.
...
Learn about [how to define an LLM-as-a-judge evaluator](lc:langsmith/llm-as-judge).
...
Learn [how run pairwise evaluations](lc:langsmith/evaluate-pairwise).
...
Learn more about [evaluation types and when to use each](lc:langsmith/evaluation-types).
...
Learn how to [create and manage dataset splits](lc:langsmith/manage-datasets-in-application#create-and-manage-dataset-splits).

**Versions**

LangSmith automatically creates dataset [versions](lc:langsmith/manage-datasets#version-a-dataset) when examples change. [Tag versions](lc:langsmith/manage-datasets#tag-a-version) to mark important milestones. Target specific versions in CI pipelines to ensure dataset updates don't break workflows.
...
[Annotation queues](lc:langsmith/annotation-queues) enable structured collection of human feedback. Flag specific runs for review, collect annotations in a streamlined interface, and transfer annotated runs to datasets for future evaluations.

Annotation queues complement [inline annotation](lc:langsmith/annotate-traces-inline) by offering additional capabilities: grouping runs, specifying criteria, and configuring reviewer permissions.
...
Evaluations can be written using standard testing tools like [pytest](lc:langsmith/pytest) or [Vitest/Jest](lc:langsmith/vitest-jest).
...
| **Setup location** | Evaluation tab (SDK, UI, Playground) | [Observability tab](lc:langsmith/online-evaluations-llm-as-judge) (automated rules) |
