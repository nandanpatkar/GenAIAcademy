This page provides a comprehensive reference table of [workspace](lc:langsmith/administration-overview#workspaces) and [organization](lc:langsmith/administration-overview#organizations) operations and which roles can perform them.
...
> [!NOTE]
>
> For an overview of LangSmith's RBAC system, role definitions, and permission concepts, refer to [Role-based access control](lc:langsmith/rbac).
...
> [!NOTE]
>
> Organization-level operations are controlled by organization roles, which are separate from the RBAC feature. Learn more in the [Role-based access control](lc:langsmith/rbac#organization-roles) guide.
...
System for Cross-domain Identity Management for user provisioning. For setup instructions, refer to the [SCIM setup guide](lc:langsmith/user-management#set-up-scim-for-your-organization).
...
| [View granular billable usage](lc:langsmith/granular-usage) | ✓ | ✓ | ✓ | `organization:read` |
| [Export granular usage as CSV](lc:langsmith/granular-usage#csv-export) | ✓ | ✓ | ✓ | `organization:read` |
...
These operations are controlled by [workspace-level roles and permissions](lc:langsmith/rbac#workspace-roles).


> [!TIP]
>
> To understand what each role means and their overall capabilities, refer to the [Role-based access control](lc:langsmith/rbac) guide.
...
> [!NOTE]
>
> The feedback formula operations are deprecated in favor of [composite evaluators](lc:langsmith/composite-evaluators-ui) and are scheduled for removal on 2026-08-20.
...
[LangSmith Deployment](lc:langsmith/deployment) configurations.
...
> [!TIP]
>
> `bulk-exports:read` and `bulk-exports:manage` are dedicated permissions that allow you to grant export access via a [custom role](lc:langsmith/rbac#custom-roles) without granting the broader `workspaces:manage` scope. This is useful for security-team service keys that need to export traces but should not be able to manage workspaces, members, or secrets.
...
[Fleet](lc:langsmith/fleet/index) workspace administration operations.
...
- [Organization Admin](lc:langsmith/rbac#organization-admin) automatically has full permissions in all workspaces.
- [Organization Operator](lc:langsmith/rbac#organization-operator) only gets workspace access when explicitly added to workspaces with workspace-level roles (or to workspaces they create).
- [Organization User](lc:langsmith/rbac#organization-user) and [Organization Viewer](lc:langsmith/rbac#organization-viewer) only get workspace access when explicitly added to workspaces with workspace-level roles.

For detailed role definitions, refer to [Organization roles](lc:langsmith/rbac#organization-roles) and [Workspace roles](lc:langsmith/rbac#workspace-roles).

### Workspace role independence

- Users can have different workspace roles in different workspaces.
- A user might be a [Workspace Admin](lc:langsmith/rbac#workspace-admin) in one workspace and a [Workspace Viewer](lc:langsmith/rbac#workspace-viewer) in another.
