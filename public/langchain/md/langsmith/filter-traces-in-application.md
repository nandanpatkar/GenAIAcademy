Tracing projects can accumulate large amounts of data across [threads](lc:langsmith/observability-concepts#threads), [traces](lc:langsmith/observability-concepts#traces), and [runs](lc:langsmith/observability-concepts#runs). LangSmith's filtering tools let you navigate and analyze that data precisely.
...
If you are programmatically exporting data for analysis via the [API](https://docs.langchain.com/langsmith/smith-api/run/query-runs) or [SDK](https://docs.smith.langchain.com/reference/python/client/langsmith.client.Client#langsmith.client.Client.list_runs), refer to the [exporting traces guide](lc:langsmith/export-traces) instead.
...
Run metadata and tags are also useful to filter on. These rely on good tagging across all parts of your pipeline. To learn more, refer to [Add metadata and tags to traces](lc:langsmith/add-metadata-tags).
...
> [!NOTE]
>
> For performance, LangSmith indexes up to 250 characters of data for full-text search. If your search query exceeds this limit, we recommend using [Input/Output key-value search](lc:langsmith/filter-traces-in-application#filter-based-on-input-%2F-output-key-value-pairs) instead.
...
![LangSmith filter bar showing full-text search and input/output filters with example search terms for python, tensorflow, embedding, fine, and tune](/langchain/images/langsmith/images/filter-bar-search-light.png)


![LangSmith filter bar showing full-text search and input/output filters with example search terms for python, tensorflow, embedding, fine, and tune](/langchain/images/langsmith/images/filter-bar-search-dark.png)
...
![LangSmith filter shortcuts panel showing quick access to common key-value pair filters](/langchain/images/langsmith/images/filter-shortcut-pane-light.png)


![LangSmith filter shortcuts panel showing quick access to common key-value pair filters](/langchain/images/langsmith/images/filter-shortcut-pane-dark.png)
...
1. Click the  icon in the filter bar. If you have constructed tree or trace filters, you can also copy those.
...
For more information on the query language syntax, refer to the [Trace query syntax](lc:langsmith/trace-query-syntax#filter-query-language).

## Filter runs in the Details view

You can also apply filters directly in the [Details view](lc:langsmith/view-traces#details-view), which is useful for sifting through traces with a large number of runs. The same filters available in the main runs table view can be applied here.
...
![LangSmith trace view showing filter options with ](/langchain/images/langsmith/images/trace-view-filter-light.png)


![LangSmith trace view showing filter options with ](/langchain/images/langsmith/images/trace-view-filter-dark.png)
