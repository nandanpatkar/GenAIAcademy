...
Trace your [LiveKit Agents](https://docs.livekit.io/agents/) voice agents to LangSmith with the LangSmith LiveKit integration. For high-level conventions, see [Voice tracing fundamentals](lc:langsmith/trace-voice-fundamentals).
...
[
 {
  "label": "pip",
  "lang": "bash",
  "code": "pip install \"langsmith[livekit]\" \"livekit-agents[openai,silero,turn-detector]\""
 },
 {
  "label": "uv",
  "lang": "bash",
  "code": "uv add \"langsmith[livekit]\" \"livekit-agents[openai,silero,turn-detector]\""
 }
]
```
...
To group a conversation's runs into a LangSmith [thread](lc:langsmith/threads), for thread-level views and token and cost aggregation, call `set_thread_id` once per conversation, inside its `@server.rtc_session()` handler before its spans are emitted:
