The [LangGraph SDK](lc:langsmith/langgraph-python-sdk) lets you stream outputs from the [LangSmith Deployment API](lc:langsmith/server-api-ref) in multiple modes, from full state snapshots after each step to token-by-token LLM output. Thread streaming also supports resumability: if a connection drops, reconnect with the last event ID to pick up where you left off.


> [!NOTE]
>
> LangGraph SDK and Agent Server are a part of [LangSmith](lc:langsmith/observability).
...
### Extended example: streaming updates
  ...
  See [LangSmith quickstart](lc:langsmith/deployment-quickstart) for more details.
  ...
  [LangGraph SDK](lc:langsmith/langgraph-python-sdk)
...
|----------------------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------|
| [`values`](#stream-graph-state)  | Stream the full graph state after each [super-step](lc:langsmith/graph-rebuild#define-graphs).                                                                                            | `.stream()` / `.astream()` with [`stream_mode="values"`](lc:oss/python/langgraph/streaming#graph-state)  |
| [`updates`](#stream-graph-state) | Streams the updates to the state after each step of the graph. If multiple updates are made in the same step (e.g., multiple nodes are run), those updates are streamed separately. | `.stream()` / `.astream()` with [`stream_mode="updates"`](lc:oss/python/langgraph/streaming#graph-state) |
| [`messages-tuple`](#messages)    | Streams LLM tokens and metadata for the graph node where the LLM is invoked (useful for chat apps).                                                                                 | `.stream()` / `.astream()` with [`stream_mode="messages"`](lc:oss/python/langgraph/streaming#messages)          |
| [`debug`](#debug)                | Streams as much information as possible throughout the execution of the graph.                                                                                                      | `.stream()` / `.astream()` with [`stream_mode="debug"`](lc:oss/python/langgraph/streaming#graph-state)   |
| [`custom`](#stream-custom-data)  | Streams custom data from inside your graph                                                                                                                                          | `.stream()` / `.astream()` with [`stream_mode="custom"`](lc:oss/python/langgraph/streaming#custom-data)  |
...
>
> **Stateful runs**
> Examples below assume that you want to **persist the outputs** of a streaming run in the [checkpointer](lc:oss/python/langgraph/persistence) DB and have created a thread. To create a thread:
>
> #### Tab: Python
>
> ```python
...
To include outputs from [subgraphs](lc:oss/python/langgraph/use-subgraphs) in the streamed outputs, you can set `subgraphs=True` in the `.stream()` method of the parent graph. This will stream outputs from both the parent graph and any subgraphs.
...
* To filter the streamed tokens by LLM invocation, you can [associate `tags` with LLM invocations](lc:oss/python/langgraph/streaming#filter-by-llm-invocation).
* To stream tokens only from specific nodes, use `stream_mode="messages"` and [filter the outputs by the `langgraph_node` field](lc:oss/python/langgraph/streaming#filter-by-node) in the streamed metadata.
...
If you don't want to **persist the outputs** of a streaming run in the [checkpointer](lc:oss/python/langgraph/persistence) DB, you can create a stateless run without creating a thread:
...
LangSmith allows you to join an active [background run](lc:langsmith/background-run) and stream outputs from it. To do so, you can use [LangGraph SDK's](lc:langsmith/langgraph-python-sdk) `client.runs.join_stream` method:
...
Thread streaming opens a long-lived connection for a thread and streams output from **every run** executed on that thread. This lets you monitor all activity on a thread from a single connection, for example, in a chat UI where multiple runs may be triggered over time through follow-up messages, [human-in-the-loop](lc:langsmith/add-human-in-the-loop) resumptions, or [background runs](lc:langsmith/background-run). To join a specific existing run by ID, see [Join and stream](#join-and-stream).
...
For API usage and implementation, refer to the [API reference](lc:langsmith/server-api-ref).
