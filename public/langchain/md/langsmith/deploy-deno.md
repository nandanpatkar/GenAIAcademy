...
#### Step: Create a Deno Deploy project
...
Deno Deploy runs each isolate with its own in-memory `MemorySaver` checkpointer. For production persistence across isolates, swap in a [durable checkpointer](lc:oss/python/langgraph/checkpointers#checkpointer-libraries) (Postgres, Redis, …). The route handlers and `server/threads.ts` helpers stay the same.
...
- [Deploy with Next.js](lc:langsmith/deploy-nextjs)
