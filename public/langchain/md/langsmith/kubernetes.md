...
- **LangSmith UI and APIs**: for [observability](lc:langsmith/observability), tracing, and [evaluation](lc:langsmith/evaluation).
- **Backend services**: (queue, playground, ACE).
- **Datastores**: (PostgreSQL, Redis, ClickHouse, optional blob storage).

For [agent deployment](lc:langsmith/deployment): To add deployment capabilities, complete this guide first, then follow [Enable LangSmith Deployment](lc:langsmith/deploy-self-hosted-full-platform#enable-langsmith-deployment).

LangChain has successfully tested LangSmith on the following Kubernetes distributions:

- Google Kubernetes Engine (GKE)
- Amazon Elastic Kubernetes Service (EKS): For architecture patterns and best practices, refer to [self-hosting on AWS](lc:langsmith/aws-self-hosted).
- Azure Kubernetes Service (AKS): For architecture patterns and best practices, refer to [self-hosting on AKS](lc:langsmith/azure-self-hosted).
- OpenShift (4.14+)
...
> [!TIP]
>
> **Prefer infrastructure as code?** [Deploy with Terraform](lc:langsmith/self-host-terraform) bundles cluster provisioning, secrets wiring, and the Helm release for AWS, Azure, and GCP into one workflow. The page below covers the Helm-only path against any conformant cluster you already manage.
...
- [PostgreSQL](lc:langsmith/self-host-external-postgres)
- [Redis](lc:langsmith/self-host-external-redis)
- [ClickHouse](lc:langsmith/self-host-external-clickhouse)

For the minimum supported version of each datastore, refer to [Minimum versions for self-hosting dependencies](lc:langsmith/self-host-dependency-versions).
...
1. You will need a working Kubernetes cluster that you can access via `kubectl`. Your cluster should have the following minimum requirements:
   ...
   1. Recommended: At least 16 vCPUs, 64GB Memory available
      ...
      * You may need to tune resource requests/limits for all of our different services based off of organization size/usage. You can find our recommendations in the [self-host scale guide](lc:langsmith/self-host-scale).
...
3. Egress to `https://beacon.langchain.com` (if not running in offline mode)
   ...
   1. LangSmith requires egress to `https://beacon.langchain.com` for license verification and usage reporting. This is required for LangSmith to function properly. You can find more information on egress requirements in the [Egress](lc:langsmith/self-host-egress) section.

## Configure your Helm charts:

1. Create a new file called `langsmith_config.yaml` with the configuration options from the previous step.
   1. There are several configuration options that you can set in the `langsmith_config.yaml` file. You can find more information on specific configuration options in the [Configuration](lc:langsmith/self-hosted) section.
...
3. Visit the external ip for the `langsmith-frontend` service on your browser
   ...
   ![Langsmith ui](/langchain/images/langsmith/images/langsmith-ui.png)

## Using LangSmith

Now that LangSmith is running, you can start using it to trace your code. You can find more information on how to use self-hosted LangSmith in the [self-hosted usage guide](lc:langsmith/self-hosted).
...
* Configure LangSmith with [Single Sign-On](lc:langsmith/self-host-sso) to secure your LangSmith instance
* Connect LangSmith to external Postgres and Redis instances
* Set up [Blob Storage](lc:langsmith/self-host-blob-storage) for storing large files

Review our [configuration section](lc:langsmith/self-hosted) for more information on how to configure these options.
...
- **[LangSmith Deployment](lc:langsmith/deployment)**: deploy, scale, and manage agents through the LangSmith UI.
- **[Fleet](lc:langsmith/fleet/index)**: create and manage AI agents without writing code.
- **[Insights](lc:langsmith/insights)**: get AI-powered analysis of your traces and application data.
- **[Chat](lc:langsmith/chat)**: an in-workspace chat experience across LangSmith to help you analyze traces, threads, prompts, and experiment results.
- **[Sandboxes](lc:langsmith/sandboxes)**: run code, expose temporary services, and create memory snapshots from LangSmith.

Follow the [Enable additional features](lc:langsmith/deploy-self-hosted-full-platform) guide to set up these components.
