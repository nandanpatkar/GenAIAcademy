...
> [!NOTE]
>
> This policy applies only to public endpoints documented in the [LangSmith API reference](lc:langsmith/smith-api-ref) and the [Agent Server API reference](lc:langsmith/server-api-ref). Internal, undocumented endpoints are not covered and can change, including with breaking changes, at any time.
...
1. **Announced**: the deprecation is published in the [changelog](lc:langsmith/changelog) with the removal date, once known, and, where call-site changes are needed, in a migration guide that documents the replacement.
...
Self-hosted major releases ship on a roughly six-week cadence. For details, see [Release policy](lc:langsmith/release-versions).
...
- [Changelog](lc:langsmith/changelog) for recent LangSmith updates
- [Release stages](lc:langsmith/release-stages) for how features move from alpha to GA
- [Release policy](lc:langsmith/release-versions) for self-hosted release channels, cadence, and version support
