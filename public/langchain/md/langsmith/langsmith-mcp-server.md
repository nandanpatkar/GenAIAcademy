...
> [!WARNING]
>
> **Deprecated—use the [LangSmith Remote MCP](lc:langsmith/langsmith-remote-mcp) instead.**
>
> LangSmith now hosts an OAuth-authenticated remote MCP server on LangSmith Cloud and on [self-hosted LangSmith](lc:langsmith/self-hosted) v0.15 or later. Cloud endpoints:
>
>
> {/* Pass `prefix` to change the hostname before ".langchain.com" (default: "api.smith").
>     Pass `suffix` to append a path (e.g. "/mcp") to each URL.
...
The LangSmith MCP Server is a [Model Context Protocol](https://modelcontextprotocol.io/introduction) (MCP) server that integrates with [LangSmith](https://smith.langchain.com). It lets MCP-compatible clients (for example, AI coding assistants) read [conversation history](lc:langsmith/observability-concepts#threads), [prompts](lc:langsmith/manage-prompts-programmatically), [runs and traces](lc:langsmith/observability-concepts#runs), [datasets](lc:langsmith/evaluation-concepts#datasets), [experiments](lc:langsmith/evaluation-concepts#experiment), and billing usage from your LangSmith workspace.
...
>
> - To connect and use remote MCP servers (including this one) in your Python application, see [MCP (Model Context Protocol)](lc:oss/python/langchain/mcp).
> - To connect and use this server in Fleet, see [Remote MCP servers](lc:langsmith/fleet/remote-mcp-servers).
...
- **Authentication:** Send your [LangSmith API key](lc:langsmith/create-account-api-key) in the `LANGSMITH-API-KEY` header.


> [!NOTE]
>
> The hosted instance is for [LangSmith Cloud](lc:langsmith/deploy-to-cloud). For a [self-hosted LangSmith](lc:langsmith/self-hosted) instance, run the server yourself and point it at your endpoint (see [Docker deployment](#docker-deployment-http-streamable)).
...
Use the **hosted** MCP server to connect to [LangSmith Cloud](lc:langsmith/cloud) (`smith.langchain.com`, `eu.smith.langchain.com`, `apac.smith.langchain.com`, or `aws.smith.langchain.com`). To connect to Cloud or [self-hosted LangSmith](lc:langsmith/self-hosted), run the server [locally](#installation-run-locally) and set `LANGSMITH_ENDPOINT`. For self-hosted deployments, you can also run the server via the [Docker image](#docker-deployment-http-streamable) inside your VPC.
...
|----------|----------|-------------|
| `LANGSMITH_API_KEY` | Yes | Your [LangSmith API key](lc:langsmith/create-account-api-key) for authentication. |
| `LANGSMITH_WORKSPACE_ID` | No | Workspace ID when your API key has access to multiple workspaces. |
| `LANGSMITH_ENDPOINT` | No | API endpoint URL (for [self-hosted](lc:langsmith/self-hosted) or custom regions). Default: `https://api.smith.langchain.com`. |
