This guide shows you how to deploy standalone [Agent Servers](lc:langsmith/agent-server) directly, without a [control plane](lc:langsmith/control-plane). You can deploy the server independently and still send traces to LangSmith ([self-hosted](lc:langsmith/self-hosted) or [Cloud](lc:langsmith/cloud)) for [observability](lc:langsmith/observability) and [evaluation](lc:langsmith/evaluation). Standalone servers are production-ready and provide the most lightweight option for running agents.
...
![Standalone server architecture](/langchain/images/langsmith/images/standalone-server-light.png)


![Standalone server architecture](/langchain/images/langsmith/images/standalone-server-dark.png)


### Workflow

1. Define and test your graph locally using the `langgraph-cli` or [Studio](lc:langsmith/studio).
...
1. Use the [LangGraph CLI](lc:langsmith/cli) to [test your application locally](lc:langsmith/local-dev-testing).
2. Use the [LangGraph CLI](lc:langsmith/cli) to build a Docker image (i.e. `langgraph build`).
...
> [!TIP]
>
> You can optionally store checkpoint data in MongoDB instead of PostgreSQL. PostgreSQL is still required for all other server data. See [Configure checkpointer backend](lc:langsmith/configure-checkpointer) for details.
  ...
  5. `LANGSMITH_ENDPOINT`: To send traces to a [self-hosted LangSmith](lc:langsmith/self-hosted) instance, set `LANGSMITH_ENDPOINT` to the hostname of the self-hosted LangSmith instance. Do not add a trailing slash to the URL, as this can cause authentication errors.
4. Egress to `https://beacon.langchain.com` from your network. This is required for license verification and usage reporting if not running in air-gapped mode. See the [Egress documentation](lc:langsmith/self-host-egress) for more details.
...
The Helm chart (v0.2.6+) supports MongoDB checkpointing with a bundled instance (dev/testing) or an external deployment (production). Set `mongo.enabled: true` in your values file. See [Configure checkpointer backend](lc:langsmith/configure-checkpointer#deploy-by-environment) for full configuration details.
...
See [Configure checkpointer backend](lc:langsmith/configure-checkpointer) for more details on MongoDB configuration options.
