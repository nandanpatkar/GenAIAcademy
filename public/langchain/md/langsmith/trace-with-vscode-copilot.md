...
| `OTEL_EXPORTER_OTLP_HEADERS` | Authentication headers for the OTLP exporter. Use `x-api-key=<your_langsmith_api_key>` and optionally `Langsmith-Project=<project>` to route traces to a specific [LangSmith project](lc:langsmith/log-traces-to-project). |
...
Start a Copilot Chat session and send a request. Open your [LangSmith project](https://smith.langchain.com) to view the resulting traces. Each agent interaction produces a hierarchical span tree following the [OTel GenAI Semantic Conventions](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/gen-ai/):
...
- [Trace with OpenTelemetry](lc:langsmith/trace-with-opentelemetry)
