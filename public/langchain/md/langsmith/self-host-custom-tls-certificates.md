Use this guide to configure TLS in LangSmith. Start by mounting internal certificate authorities (CAs) so your deployment trusts the right roots system‑wide, for database or external service calls. You can then configure [Playground](lc:langsmith/prompt-engineering-concepts#playground)-specific mTLS for communicating securely with supported model providers.
...
Use this approach to make internal/public CAs trusted system‑wide by LangSmith (Playground model calls and [database/external service connections](lc:langsmith/self-hosted#storage-services)).
...
> * Azure OpenAI
> * OpenAI
> * Custom (our custom model server). Refer to the [custom model server documentation](lc:langsmith/custom-endpoint) for more information.
>
...
To use custom TLS certificates, set the following environment variables. See the [self-hosted overview](lc:langsmith/self-hosted) for more information on how to configure application settings.
