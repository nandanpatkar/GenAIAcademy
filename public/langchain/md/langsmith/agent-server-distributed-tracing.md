When you call a deployed [Agent Server](lc:langsmith/agent-server) from another service, you can propagate trace context so that the entire request appears as a single unified trace in LangSmith. This uses LangSmith's [distributed tracing](lc:langsmith/distributed-tracing) capabilities, which propagate context via HTTP headers.
...
If you're using the [LangGraph SDK](lc:langsmith/reference) directly, propagate trace headers manually using `run_tree.to_headers()`:
...
- [Distributed tracing](lc:langsmith/distributed-tracing): General distributed tracing concepts and patterns
- [RemoteGraph](lc:langsmith/use-remote-graph): Full guide to interacting with deployments using RemoteGraph
