...
For the access and retention model of the managed app, see [Engine security](lc:langsmith/engine-security#github-integration).
...
### Create a GitHub App
  ...
  #### Step: Set the webhook URL and secret
  ...
  #### Step: Create the app and gather its values
  ...
  #### Step: Create a Kubernetes Secret
    ...
    Do not put these values in Helm values or command-line arguments. For production deployments, use your existing secrets workflow, such as [Sealed Secrets](https://github.com/bitnami-labs/sealed-secrets) or [External Secrets Operator](https://external-secrets.io/). See [Use an existing secret](lc:langsmith/self-host-using-an-existing-secret) for more.
  

  #### Step: Add the configuration to your langsmith_config.yaml

    Add the following to `hostBackend.deployment.extraEnv` in your [`langsmith_config.yaml`](lc:langsmith/kubernetes#configure-your-helm-charts). Reference the sensitive values with `secretKeyRef`; never set them through `commonEnv` or as inline values:
  ...
  #### Step: Install the app on repositories
...
- [Find and fix your agent's issues](lc:langsmith/engine): Engine setup, costs, and the issue workflow.
- [Engine on self-hosted](lc:langsmith/engine-self-hosted): Self-hosted architecture and data handling.
- [Engine security](lc:langsmith/engine-security): How Engine handles your data and GitHub access.
- [Enable Engine](lc:langsmith/deploy-self-hosted-full-platform#enable-engine): Enable Engine in the LangSmith Helm chart.
