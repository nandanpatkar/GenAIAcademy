...
Each bulk export is divided into multiple _runs_, where each run processes data for a [specific date partition](lc:langsmith/data-export#partitioning-scheme) (typically organized by day). Runs are processed independently, which enables:
...
|--------------|-------|------------------|-----------------|
| **Infrastructure interruption** | [Deployments](lc:langsmith/deployment), server restarts, worker crashes | Yes, automatically requeued with remaining retries. | None, jobs resume automatically. |
| **Run timeout** | Single run exceeds 4-hour limit | Yes, retried up to 20 times (subject to change). | If persistent, narrow date range, add filters, or [limit the exported fields](lc:langsmith/data-export#limit-exported-fields). |
| **Workflow timeout** | Entire export exceeds 72 hours | No | Reduce export scope (date range, filters) or break into smaller exports. |
| **Storage/destination errors** | [Invalid credentials](lc:langsmith/data-export-destinations#credentials-configuration), [missing bucket](lc:langsmith/data-export-destinations#configuration-fields), [permission issues](lc:langsmith/data-export-destinations#permissions-required) | No | Fix destination configuration and create new export. |
...
On [LangSmith Self-hosted](lc:langsmith/self-hosted), the concurrency limits are the defaults. To tune pod memory usage during bulk exports, configure the following environment variables on the `langsmith-backend` service:
...
3. **Verify destination access**: Ensure your [destination bucket](lc:langsmith/data-export-destinations#configuration-fields) still exists and [credentials](lc:langsmith/data-export-destinations#credentials-configuration) are valid.
4. **Check run size**: If you see timeout errors, your date partitions may contain too much data. It may be helpful to [limit the exported fields](lc:langsmith/data-export#limit-exported-fields).
