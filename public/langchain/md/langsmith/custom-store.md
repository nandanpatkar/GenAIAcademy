...
Add the `store` key to your [`langgraph.json` configuration file](lc:langsmith/application-structure#configuration-file-concepts). The `path` points to the async context manager you [defined earlier](#define-the-store).
...
- [Use a custom checkpointer](lc:langsmith/custom-checkpointer) to replace the built-in checkpoint storage.
