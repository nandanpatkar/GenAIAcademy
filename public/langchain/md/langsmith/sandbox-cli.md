The [LangSmith CLI](lc:langsmith/langsmith-cli) includes sandbox commands for creating snapshots, booting sandboxes, running commands, opening interactive shells, and tunneling TCP connections into a sandbox.
...
For private images, create a registry first (see [Private registries](lc:langsmith/sandbox-snapshots#private-registries)), then pass its id with `--registry-id`:
...
For more on proxy rules, see [Sandbox auth proxy](lc:langsmith/sandbox-auth-proxy).
...
> [!TIP]
>
> For HTTP applications you want to open in a browser or share with teammates, use [Sandbox service URLs](lc:langsmith/sandbox-service-urls). Use tunnels for raw TCP protocols or local development tools.
