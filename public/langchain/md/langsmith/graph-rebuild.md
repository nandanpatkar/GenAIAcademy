...
- Make sure to check out [this how-to guide](lc:langsmith/setup-app-requirements-txt) on setting up your app for deployment first.
...
> [!NOTE]
>
> `ServerRuntime` is in [beta](lc:langsmith/release-stages) and may change in future releases.
...
| `user` | `BaseUser \| None` | The authenticated user, or `None` if no [custom auth](lc:langsmith/custom-auth) is configured. |
...
You can use the factory function to customize or disable tracing for a specific graph. See [Conditional tracing: Customize tracing in deployed agents](lc:langsmith/conditional-tracing#customize-tracing-in-deployed-agents) for examples.

See more info on the [LangGraph API configuration file](lc:langsmith/cli#configuration-file).
