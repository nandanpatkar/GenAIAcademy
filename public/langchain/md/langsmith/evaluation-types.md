...
Run offline evaluations client-side using the LangSmith SDK (`Python`[langsmith-python] or `TypeScript`[langsmith-js]) or server-side via the [Playground](lc:langsmith/prompt-engineering-concepts#playground) or by [binding evaluators to a dataset](lc:langsmith/bind-evaluator-to-dataset).

![Offline](/langchain/images/langsmith/images/offline.png)
...
For more details, refer to the [Pytest](lc:langsmith/pytest) and [Vitest/Jest](lc:langsmith/vitest-jest) pages.
...
![Comparison view](/langchain/images/langsmith/images/comparison-diff-view-light.png)
...
Online evaluators typically run server-side. LangSmith provides built-in [LLM-as-judge evaluators](lc:langsmith/llm-as-judge) for configuration, and supports custom code evaluators that run within LangSmith.

![Online](/langchain/images/langsmith/images/online.png)
...
- Programmatic offline evaluation: [With the SDK](lc:langsmith/llm-as-judge-sdk)
- Offline evaluation on datasets: [In the UI](lc:langsmith/llm-as-judge)
- Online evaluation on production traces: [In the UI](lc:langsmith/online-evaluations-llm-as-judge)
...
- Offline evaluation on datasets: [In the UI](lc:langsmith/code-evaluator-ui)
- Programmatic offline evaluation: [With the SDK](lc:langsmith/code-evaluator-sdk)
- Online evaluation on production traces: [In the UI](lc:langsmith/online-evaluations-code)
...
- Offline evaluation with predefined aggregation: [In the UI](lc:langsmith/composite-evaluators-ui)
- Offline evaluation with custom aggregation logic: [With the SDK](lc:langsmith/composite-evaluators-sdk)
- Online evaluation on production traces: [In the UI](lc:langsmith/online-evaluations-composite)
...
- Custom aggregation functions for offline evaluation: [With the SDK](lc:langsmith/summary)
...
- Compare existing experiments: [With the SDK](lc:langsmith/evaluate-pairwise)
