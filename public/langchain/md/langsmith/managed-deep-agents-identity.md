...
Managed Deep Agents answers that question before every run. You declare a small contract once, and the runtime partitions threads, [memory](lc:langsmith/managed-deep-agents-memory), and credentials so callers cannot see or affect each other.
...
This page assumes you have an existing Managed Deep Agents project and the `mda` CLI installed. If you are new to Managed Deep Agents, start with the [overview](lc:langsmith/managed-deep-agents-overview) and [quickstart](lc:langsmith/managed-deep-agents-quickstart) first.
...
- **Memory**: which durable [Context Hub](lc:langsmith/managed-deep-agents-memory) slice the run can see
...
This is the default ingress for all presets, and the recommended choice when you already have a backend in front of the agent. For the broader LangGraph auth model, see [Add auth to your server](lc:langsmith/add-auth-server).
...
For how memory paths remount under each scope, see [Scope memory with identity](lc:langsmith/managed-deep-agents-memory#scope-memory-with-identity).
...
To expose LangSmith capabilities to browsers or other untrusted callers, add a [LangSmith connector](lc:langsmith/managed-deep-agents-connectors/langsmith). It requires identity so capability routes can resolve the caller and prove ownership before calling LangSmith server-side.
