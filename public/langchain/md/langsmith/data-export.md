...
**Before you start:** exports may take some time depending on data volume, and LangSmith limits how many exports can run concurrently. Bulk exports have a 72-hour runtime timeout—refer to [Automatic retry behavior](lc:langsmith/data-export-monitor#automatic-retry-behavior) for details. Once launched, LangSmith handles orchestration and [resilience of the export process](lc:langsmith/data-export-monitor#failure-modes-and-retry-policy) automatically.

## 1. Create a destination

The destination tells LangSmith where to write your exported data. Before making this request, you will need:

- Your [LangSmith API key](lc:langsmith/create-account-api-key) and [workspace ID](lc:langsmith/set-up-hierarchy#set-up-a-workspace).
- An S3 or S3-compatible bucket with **write access** granted to LangSmith (refer to [Permissions required](lc:langsmith/data-export-destinations#permissions-required)).
...
Credentials are stored securely in encrypted form. The API will validate that the destination and credentials are valid before saving. If the request fails, refer to [Debug destination errors](lc:langsmith/data-export-destinations#debug-destination-errors).

Save the `id` from the response; you will need it when creating an export job.

Refer to [Manage bulk export destinations](lc:langsmith/data-export-destinations) for permissions setup, provider-specific configuration (AWS S3, GCS, MinIO), and credential options.
...
You can optionally add a `filter` expression to narrow the set of runs exported. Refer to our [filter query language](lc:langsmith/trace-query-syntax#filter-query-language) and [examples](lc:langsmith/export-traces#use-filter-query-language) for syntax. Not setting the `filter` field will export all runs.
...
>
> On [LangSmith cloud](lc:langsmith/cloud), each workspace can create at most 250 bulk exports per hour. This budget includes one-off exports and exports spawned by [scheduled bulk exports](#schedule-recurring-exports), so a workspace with many active schedules consumes part of the hourly budget automatically.
>
...
>
> [Self-hosted LangSmith](lc:langsmith/self-hosted) does not enforce this limit by default.
...
> [!NOTE]
>
> [Self-hosted](lc:langsmith/self-hosted): currently available only on the `v0.16.1rc1` [preview release](lc:langsmith/release-versions#preview). Wait for the `v0.16.1` stable release before running it in production.
...
>
> On [LangSmith cloud](lc:langsmith/cloud), each `all_experiments` export includes at most 250 experiments. To export more:
>
...
>
> [Self-hosted LangSmith](lc:langsmith/self-hosted) has no per-export limit.
...
- To stop a scheduled export, [cancel it](lc:langsmith/data-export-monitor#stop-an-export).
...
>
> On [LangSmith cloud](lc:langsmith/cloud), each workspace can have at most 200 active **scheduled** (recurring) bulk exports at a time. That is, exports configured with an `interval_hours` value. The limit caps the number of **schedules**, not the number of times they run: a schedule that has produced thousands of historical export runs still counts as one.
>
> One-off (non-recurring) bulk exports are not subject to this limit.
>
> If your workspace reaches the limit, new scheduled export requests will be rejected with a `429` until you [cancel](lc:langsmith/data-export-monitor#stop-an-export) an existing schedule. To raise the limit, contact support via [support.langchain.com](https://support.langchain.com).
>
> [Self-hosted LangSmith](lc:langsmith/self-hosted) does not enforce this limit by default.
...
Allowed values: `zstandard`, `gzip`, `snappy`, `none`. Use `snappy` when loading into BigQuery, see [Export trace data to BigQuery](lc:langsmith/big-query-bulk-export).


> [!NOTE]
>
> On [Self-hosted LangSmith](lc:langsmith/self-hosted), the default is `gzip`. Set the `FF_BULK_EXPORT_DEFAULT_COMPRESSION` environment variable to change the default.
...
Refer to [Monitor and troubleshoot bulk exports](lc:langsmith/data-export-monitor) for how to list runs, stop an export, and diagnose failures.
