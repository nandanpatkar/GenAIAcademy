In this tutorial, you will extend [the chatbot created in the last tutorial](lc:langsmith/set-up-custom-auth) to give each user their own private conversations. You'll add [resource-level access control](lc:langsmith/auth#single-owner-resources) so users can only see their own threads.

![Authorization flow: after authentication, an authorization handler tags each resource with owner=user id and returns a filter so users only see their own threads.](/langchain/images/langsmith/images/authorization.png)

## Prerequisites

Before you start this tutorial, ensure you have the [bot from the first tutorial](lc:langsmith/set-up-custom-auth) running without errors.

## 1. Add resource authorization

Recall that in the last tutorial, the `Auth`[Auth] object lets you register an [authentication function](lc:langsmith/auth#authentication), which LangSmith uses to validate the bearer tokens in incoming requests. Now you'll use it to register an **authorization** handler.

Authorization handlers are functions that run **after** authentication succeeds. These handlers can add [metadata](lc:langsmith/auth#filter-operations) to resources (like who owns them) and filter what each user can see.
...
The broad `@auth.on` handler matches on all [authorization events](lc:langsmith/auth#supported-resources). This is concise, but it means the contents of the `value` dict are not well-scoped, and the same user-level access control is applied to every resource. If you want to be more fine-grained, you can also control specific actions on resources.
...
The first three of these match specific **actions** on each resource (see [resource actions](lc:langsmith/auth#resource-specific-handlers)), while the last two (`@auth.on.assistants` and `@auth.on.store`) matche _any_ action on the `assistants` and `store` resources. For each request, LangGraph will run the most specific handler that matches the resource and action being accessed. This means that the four handlers above will run rather than the broadly scoped "`@auth.on`" handler.

Store authorization works differently from threads and assistants. For store isolation patterns and tradeoffs, see [Isolate store per user](lc:langsmith/store-auth).
...
1. Move on to [Connect an authentication provider](lc:langsmith/add-auth-server) to add real user accounts.
2. Read more about [authorization patterns](lc:langsmith/auth#authorization).
