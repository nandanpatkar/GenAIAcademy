[_Datasets_](lc:langsmith/evaluation-concepts#datasets) enable you to perform repeatable evaluations over time using consistent data. Datasets are made up of [_examples_](lc:langsmith/evaluation-concepts#examples), which store inputs, outputs, and optionally, reference outputs.
...
A common pattern for constructing datasets is to convert notable traces from your application into dataset examples. This approach requires that you have [configured tracing to LangSmith](lc:langsmith/observability-concepts).


> [!TIP]
>
> A technique to build datasets is to filter the most interesting traces, such as traces that were tagged with poor user feedback, and add them to a dataset. For tips on how to filter traces, refer to the [Filter traces](lc:langsmith/filter-traces-in-application) guide.
...
1. On the **Runs** tab, select a run from the table. On the individual run details page, select  **Add to** -> **Dataset** in the top right corner.
    ...
    When you select a dataset from the run details page, a modal will pop up letting you know if any [transformations](lc:langsmith/dataset-transformations) were applied or if schema validation failed.

    You can then optionally edit the run before adding it to the dataset.

### Automatically from a tracing project

You can use [run rules](lc:langsmith/rules) to add traces automatically to a dataset based on certain conditions. For example, you could add all traces that are [tagged](lc:langsmith/observability-concepts#tags) with a specific use case or have a [low feedback score](lc:langsmith/observability-concepts#feedback).
...
> [!TIP]
>
> If you rely on subject matter experts to build meaningful datasets, use [annotation queues](lc:langsmith/annotation-queues) to provide a streamlined view for reviewers. Human reviewers can optionally modify the inputs/outputs/reference outputs from a trace before it is added to the dataset.
...
> [!NOTE]
>
> **Add to Dataset** is available for **run** queue items only. [Thread](lc:langsmith/observability-concepts#threads) items in an annotation queue support rubric feedback, but not dataset export.


> [!TIP]
>
> You can also set up rules to add runs that meet specific criteria to an annotation queue using [automation rules](lc:langsmith/rules).


### From the Playground

On the [**Playground**](lc:langsmith/prompt-engineering-concepts#playground) page:
...
> [!NOTE]
>
> Creating datasets inline in the Playground is not supported for datasets that have nested keys. In order to add/edit examples with nested keys, you must edit [from the datasets page](lc:langsmith/manage-datasets-in-application#create-a-new-dataset-from-the-datasets-%26-experiments-page).
...
If you have existing examples and a [schema](#create-a-dataset-schema) defined on your dataset, when you click **+ Example** there is an option to  **Add AI-Generated Examples**. This will use an LLM to create [synthetic](lc:langsmith/evaluation-concepts#building-datasets) examples.

In **Generate examples**, do the following:

1. Click **API Key** in the top right of the pane to set your OpenAI API key as a [workspace secret](lc:langsmith/administration-overview#workspaces). If your workspace already has an OpenAI API key set, you can skip this step.
1. Select few-shot examples: Toggle **Automatic** or **Manual** reference examples. You can select these examples manually from your dataset or use the automatic selection option.
1. Enter the number of synthetic examples you want to generate.
1. Click **Generate**.

    
![The AI-Generated Examples configuration window. Selections for manual and automatic and number of examples to generate.](/langchain/images/langsmith/images/generate-synthetic-light.png)


    
![The AI-Generated Examples configuration window. Selections for manual and automatic and number of examples to generate.](/langchain/images/langsmith/images/generate-synthetic-dark.png)
...
![Select generated examples page with generated examples selected and Save examples button.](/langchain/images/langsmith/images/select-generated-examples-light.png)


    
![Select generated examples page with generated examples selected and Save examples button.](/langchain/images/langsmith/images/select-generated-examples-dark.png)
...
LangSmith datasets store arbitrary JSON objects. We recommend (but do not require) that you define a schema for your dataset to ensure that they conform to a specific JSON schema. Dataset schemas are defined with standard [JSON schema](https://json-schema.org/), with the addition of a few [prebuilt types](lc:langsmith/dataset-json-types) that make it easier to type common primitives like messages and tools.
...
For the full list of available transformations, refer to the [Dataset transformations reference](lc:langsmith/dataset-transformations).


> [!NOTE]
>
> If you plan to collect production traces in your dataset from LangChain [ChatModels](lc:oss/python/langchain/models) or from OpenAI calls using the [LangSmith OpenAI wrapper](lc:langsmith/annotate-code), we offer a prebuilt Chat Model schema that converts messages and tools into industry standard openai formats that can be used downstream with any model for testing. You can also customize the template settings to match your use case.
>
> Please see the [dataset transformations reference](lc:langsmith/dataset-transformations) for more information.


### Create and manage dataset splits

For an overview of when and why to use splits, refer to [Dataset organization](lc:langsmith/evaluation-concepts#dataset-organization).
...
![Add to Split](/langchain/images/langsmith/images/add-to-split2.png)
...
You may use this to store information about your examples, such as tags or version info, which you can then [group by](lc:langsmith/analyze-an-experiment#group-results-by-metadata) when analyzing experiment results or [filter by](lc:langsmith/manage-datasets-programmatically#list-examples-by-metadata) when you call `list_examples` in the SDK.
...
![Filters Applied to Examples](/langchain/images/langsmith/images/filters-applied.png)
