...
The control plane API is part of [LangSmith Deployment](lc:langsmith/deployment). With the control plane API, you can programmatically create, manage, and automate your [Agent Server](lc:langsmith/agent-server) deployments—for example, as part of a custom CI/CD workflow.
...
**Note**: Self-hosted deployments of LangSmith will have a custom host for the control plane. The control plane APIs can be accessed at the path `/api-host`. For example, `http(s)://<host>/api-host/v2/deployments`. See [the self-host usage guide](lc:langsmith/self-host-usage#configuring-the-application-you-want-to-use-with-langsmith) for more details.
