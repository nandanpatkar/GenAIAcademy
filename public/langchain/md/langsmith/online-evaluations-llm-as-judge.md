...
- Runs where a [user left feedback](lc:langsmith/attach-user-feedback) indicating the response was unsatisfactory.
- Runs that invoke a specific tool call. See [filtering for tool calls](lc:langsmith/filter-traces-in-application#example-filtering-for-tool-calls) for more information.
- Runs that match a particular piece of metadata (e.g. if you log traces with a `plan_type` and only want to run evaluations on traces from your enterprise customers). See [adding metadata to your traces](lc:langsmith/add-metadata-tags) for more information.

[Filters on evaluators](lc:langsmith/filter-traces-in-application) work the same way as when you're filtering traces in a project.
...
For details, refer to [Track and limit evaluator spend](lc:langsmith/evaluator-spend).

## Configure the LLM-as-a-judge evaluator

View [LLM-as-a-judge evaluators](lc:langsmith/llm-as-judge#evaluator-templates) for more information.
...
- **Using attachments from traces**: Similar to [offline evaluations with attachments](lc:langsmith/evaluate-with-attachments), you can use attachments from your traces in online evaluations. Since your traces already include attachments logged via the SDK, you can reference them directly in your evaluator.
...
![Edit evaluator modal with an image attachment selected for the input.](/langchain/images/langsmith/images/variable-multimodal-content-light.png)


    
![Edit evaluator modal with an image attachment selected for the input.](/langchain/images/langsmith/images/variable-multimodal-content-dark.png)
