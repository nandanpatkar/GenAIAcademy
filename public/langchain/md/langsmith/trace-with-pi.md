...
Use `replicas` to send traces to multiple LangSmith destinations simultaneously. This is useful for forwarding traces to both a personal workspace and a shared team project, or to a [self-hosted LangSmith](lc:langsmith/self-hosted) instance alongside the [cloud](lc:langsmith/cloud).
