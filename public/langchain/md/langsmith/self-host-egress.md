...
> [!WARNING]
>
> **Egress to `https://beacon.langchain.com` is required.** Refer to the [allowlisting IP section](lc:langsmith/cloud#allowlisting-ip-addresses) for static IP addresses, if needed.


> [!NOTE]
>
> If you enable [Engine](lc:langsmith/deploy-self-hosted-full-platform#enable-engine), it requires a second egress destination, and Engine content leaves your environment through it. See [LangSmith Intelligence for Engine](#langsmith-intelligence-for-engine).
...
This section applies only if you enable [Engine](lc:langsmith/deploy-self-hosted-full-platform#enable-engine). LangSmith Intelligence is the LangChain-managed service that powers Engine. No other LangSmith feature depends on it, and none requires egress beyond what this page already describes.
...
> [!NOTE]
>
> Engine is currently available for self-hosted deployments in **AWS US** and **GCP US**. AWS EU and Azure are planned. See [Availability by cloud and region](lc:langsmith/engine-self-hosted#availability-by-cloud-and-region).
...
For the complete data flow and model-provider commitments, see [Engine on self-hosted](lc:langsmith/engine-self-hosted).
