...
If you're using a [tracing integration](lc:langsmith/integrations), pass `thread_id` in the run metadata:
...
| `filter` | `string` | — | Filter expression applied when fetching runs, using [LangSmith trace query syntax](lc:langsmith/trace-query-syntax). |
...
| `runs` | ``Run`[]` | Root runs in this thread, sorted chronologically (oldest first). |
...
Pass a filter expression to narrow results using [LangSmith trace query syntax](lc:langsmith/trace-query-syntax). For example, to surface only threads containing at least one failed run:
