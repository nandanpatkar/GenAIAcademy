> [!NOTE]
>
> This guide assumes you have read the [admin guide](lc:langsmith/administration-overview) and [organization setup guide](lc:langsmith/set-up-hierarchy#set-up-an-organization).
...
![Update SSO Member Settings](/langchain/images/langsmith/images/sso-member-settings-update.png)
...
[SSO Groups Sync](lc:langsmith/user-management#sso-groups-sync-alternative) reads group memberships from the OIDC ID token and assigns org and workspace roles using the [SCIM naming convention](lc:langsmith/user-management#group-naming-convention). It is a simpler alternative to [SCIM](lc:langsmith/user-management#set-up-scim-for-your-organization) for self-hosted organizations whose IdP can include groups in the OIDC token but cannot easily run SCIM provisioning.

For IdP-side configuration (claim, scope) refer to the [SSO Groups Sync section in the OIDC SSO setup guide](lc:langsmith/self-host-sso#sso-groups-sync). For settings reference and behavior, see the [main SSO Groups Sync documentation](lc:langsmith/user-management#sso-groups-sync-alternative).
...
> [!NOTE]
>
> The `userOrgCreationDisabled` feature flag is set to `true` by default for organizations using [basic auth](lc:langsmith/self-host-basic-auth) or [SSO](lc:langsmith/self-host-sso).
...
> [!NOTE]
>
> The `personalOrgsDisabled` feature flag is set to `true` by default for organizations using [basic auth](lc:langsmith/self-host-basic-auth) or [SSO](lc:langsmith/self-host-sso).
...
To disable PAT creation for a single organization instead, see the [per-organization API option](lc:langsmith/manage-organization-by-api#security-settings).
