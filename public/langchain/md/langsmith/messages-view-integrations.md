> [!NOTE]
>
> The [Messages view](lc:langsmith/view-traces#messages-view) is in **[beta](lc:langsmith/release-stages)**.


The [Messages view](lc:langsmith/view-traces#messages-view) renders an agent's trace as a chat-style conversation: user prompts, model responses, tool calls, and tool results, in order. The Messages view needs two pieces of run metadata to render the main conversation:
...
- [Claude Code](lc:langsmith/trace-claude-code)
- [Claude Agent SDK](lc:langsmith/trace-claude-agent-sdk)
- [OpenAI Codex](lc:langsmith/trace-with-codex)
- [Cursor](lc:langsmith/trace-with-cursor)
- [Pi](lc:langsmith/trace-with-pi)
- [OpenCode](lc:langsmith/trace-with-opencode)
- [GitHub Copilot](lc:langsmith/trace-with-vscode-copilot)
- [Deep Agents](lc:langsmith/trace-deep-agents)
- [LangChain](lc:langsmith/trace-with-langchain)
- [LangGraph](lc:langsmith/trace-with-langgraph)
...
For the full `ls_agent_type` schema and the other values (`subagent`, `middleware`, `compaction`) that official integrations set on non-root runs, see the [Coding agent metadata contract](lc:langsmith/coding-agent-metadata-contract). For the underlying thread-grouping mechanism, see [Configure threads](lc:langsmith/threads).
...
- [Configure threads](lc:langsmith/threads): how `thread_id` groups runs across LangSmith.
- [Coding agent metadata contract](lc:langsmith/coding-agent-metadata-contract): the full `ls_agent_type` schema.
- [View traces](lc:langsmith/view-traces#messages-view): what the Messages view shows and how to customize it.
