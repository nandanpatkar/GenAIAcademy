Code evaluators in the [LangSmith UI](https://smith.langchain.com) allow you to write custom evaluation logic using Python or TypeScript code directly in the interface. Unlike [LLM-as-a-judge](lc:langsmith/llm-as-judge) evaluators that use a model to evaluate outputs, code evaluators use deterministic logic you define.


> [!NOTE]
>
> To create a code evaluator that appears in the LangSmith UI programmatically, refer to [Manage evaluators with the SDK](lc:langsmith/manage-evaluators-sdk). To define a code evaluator function that you pass to `evaluate()`, refer to [How to define a code evaluator (SDK)](lc:langsmith/code-evaluator-sdk). To grade outputs against assertions saved on dataset examples, refer to [Use assertions](lc:langsmith/assertions).
...
- When running evaluations from the [Playground](lc:langsmith/prompt-engineering-concepts#playground)
- As part of a dataset to [automatically run evaluations on experiments](lc:langsmith/bind-evaluator-to-dataset)

## Related

- [LLM-as-a-judge evaluator (UI)](lc:langsmith/llm-as-judge): Use an LLM to evaluate outputs
- [Composite evaluators](lc:langsmith/composite-evaluators-ui): Combine multiple evaluator scores
