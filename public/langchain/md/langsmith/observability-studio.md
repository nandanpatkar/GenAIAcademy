LangSmith [Studio](lc:langsmith/studio) provides tools to inspect, debug, and improve your app beyond execution. By working with traces, datasets, and prompts, you can see how your application behaves in detail, measure its performance, and refine its outputs:
...
Define your [configuration](lc:oss/python/langgraph/use-graph-api#add-runtime-configuration) to specify prompt fields and their associated nodes using `langgraph_nodes` and `langgraph_type` keys.
...
The [Playground](lc:langsmith/create-a-prompt) interface allows testing individual LLM calls without running the full graph:
...
- **A LangSmith dataset**: Your dataset should contain the inputs you want to test and optionally, reference outputs for comparison. The schema for the inputs must match the required input schema for the assistant. For more information on schemas, see the [graph API schema documentation](lc:oss/python/langgraph/graph-api#schema). For more on creating datasets, refer to [How to Manage Datasets](lc:langsmith/manage-datasets-in-application#create-a-dataset-and-add-examples).
...
- **A running application**: The experiment can be run against:
  - An application deployed on [LangSmith](lc:langsmith/deployment).
  - A locally running application started via the [langgraph-cli](lc:langsmith/local-dev-testing).


> [!NOTE]
>
> Studio experiments follow the same [data retention](lc:langsmith/usage-and-billing#data-retention) rules as other experiments. By default, traces have base tier retention (14 days). However, traces will automatically upgrade to extended tier retention (400 days) if feedback is added to them. Feedback can be added in one of two ways:
>
> - The [dataset has evaluators configured](lc:langsmith/bind-evaluator-to-dataset).
> - [Feedback](lc:langsmith/observability-concepts#feedback) is manually added to a trace.
>
> This auto-upgrade increases both the retention period and the cost of the trace. For more details, refer to [Data retention auto-upgrades](lc:langsmith/usage-and-billing#how-it-works).
...
- A [locally running agent](lc:langsmith/local-dev-testing).
...
Add [examples](lc:langsmith/evaluation-concepts#examples) to [LangSmith datasets](lc:langsmith/manage-datasets) from nodes in the thread log. This is useful to evaluate individual steps of the agent.
...
For more details, refer to [How to evaluate an application's intermediate steps](lc:langsmith/evaluate-on-intermediate-steps).
