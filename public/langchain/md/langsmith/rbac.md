...
LangSmith's RBAC system manages user permissions within workspaces. RBAC allows you to control who can access your LangSmith [workspace](lc:langsmith/administration-overview#workspaces) and what they can do within it.
...
To learn how to set up RBAC and assign roles to users, refer to the [User Management guide](lc:langsmith/user-management#set-up-access-control). Your identity provider can also assign roles automatically via [SCIM groups](lc:langsmith/user-management#set-up-scim-for-your-organization) or [SSO Groups Sync](lc:langsmith/user-management#sso-groups-sync-alternative).
...
- `organization:pats:create` - Create organization-level [personal access tokens](lc:langsmith/administration-overview#personal-access-tokens-pats)


**Key Capabilities**:
- Manage [organization settings](lc:langsmith/set-up-hierarchy#set-up-an-organization) and branding
- Configure [SSO and authentication methods](lc:langsmith/user-management#set-up-saml-sso-for-your-organization)
- Manage [billing](lc:langsmith/billing) and subscription plans
- Create and delete [workspaces](lc:langsmith/set-up-hierarchy)
...
- View organization [usage](lc:langsmith/usage-and-billing#usage-limits) and analytics
- View [audit logs](lc:langsmith/audit-logs) (Enterprise)

For details on setting up and managing your organization, refer to the [Administration Overview](lc:langsmith/administration-overview#organizations).
...
**Key Capabilities:**
- Create and manage [workspaces](lc:langsmith/set-up-hierarchy#set-up-a-workspace)
...
- Cannot manage organization [billing](lc:langsmith/billing) or subscription plans
- Cannot configure [SSO or authentication methods](lc:langsmith/user-management#set-up-saml-sso-for-your-organization)
...
- Create [personal access tokens](lc:langsmith/administration-overview#personal-access-tokens-pats) for API access
...
- Cannot delete [runs](lc:langsmith/observability-concepts#runs)
...
> [!TIP]
>
> For step-by-step instructions on assigning workspace roles to users, refer to the [User Management guide](lc:langsmith/user-management#assign-a-role-to-a-user).
...
Custom roles are created at the [organization](lc:langsmith/administration-overview#organizations) level and can be assigned to users in any [workspace](lc:langsmith/administration-overview#workspaces) within that organization.
...
For details on which specific permissions are required for each operation, refer to the [Organization and workspace operations reference](lc:langsmith/organization-workspace-operations).
...
> [!NOTE]
>
> Role restriction is available for organizations on the [Enterprise](lc:langsmith/pricing-plans) plan.
...
To restrict a role, in the [LangSmith UI](https://smith.langchain.com):
...
> [!NOTE]
>
> Role restriction applies to both system roles (Workspace Admin, Workspace Editor, Workspace Viewer) and custom roles. It is available on LangSmith [Cloud](lc:langsmith/cloud) and [Self-hosted](lc:langsmith/self-hosted).
