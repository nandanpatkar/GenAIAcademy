...
Update `values.yaml` in your LangSmith Helm installation to point each LangChain image repository at its `-fips` counterpart, keeping your existing tag. Replace `0.16.21` with the [LangSmith version](lc:langsmith/self-hosted-changelog) you want to deploy:
...
Apply the change and upgrade following the [Upgrading LangSmith](lc:langsmith/self-host-upgrades) guide.
