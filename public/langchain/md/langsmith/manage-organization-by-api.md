...
> [!TIP]
>
> Prefer infrastructure-as-code? Use the [LangSmith Terraform provider](lc:langsmith/manage-with-terraform) to manage workspaces, roles, members, evaluators, and alerts declaratively.
...
> * [Organization setup how-to guild](lc:langsmith/set-up-hierarchy#set-up-an-organization)
...
> * Organization-scoped [service keys](lc:langsmith/administration-overview#service-keys) with Organization Admin permission may be used for these actions.
...
Some commonly-used endpoints and use cases are listed below. For a complete list of available endpoints, see the [API docs](lc:langsmith/smith-api-ref). **The `X-Organization-Id` header should be present on all requests, and `X-Tenant-Id` header should be present on requests that are scoped to a particular workspace.**
...
> [!NOTE]
>
> These params should be omitted: `read_only` (deprecated), `password` and `full_name` ([basic auth](lc:langsmith/authentication-methods) only)
...
> [!NOTE]
>
> "Shared resources" in this context refer to [public prompts](lc:langsmith/create-a-prompt#save-your-prompt), [shared runs](lc:langsmith/manage-trace#share-a-trace), and [shared datasets](lc:langsmith/manage-datasets#share-a-dataset).
...
* [Disable/enable PAT creation](https://docs.langchain.com/langsmith/smith-api/orgs/update-current-organization-info) (for self-hosted, available in Helm chart version 0.11.25+)
  ...
  * See the [admin guide](lc:langsmith/administration-overview#organization-roles) for information about the Organization Viewer role, which cannot create PATs.
  * For self-hosted deployments, you can also [globally disable PAT creation](lc:langsmith/self-host-user-management#disabling-personal-access-token-creation) across all organizations using an environment variable.
...
* `/sso/email-verification/send` (Cloud-only): this endpoint is related to [SAML SSO](lc:langsmith/user-management)
