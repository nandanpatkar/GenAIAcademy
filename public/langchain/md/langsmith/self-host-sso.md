LangSmith [Self-Hosted](lc:langsmith/self-hosted) provides SSO via OAuth2.0 and OIDC, which delegates authentication to your Identity Provider (IdP) to manage access to LangSmith.
...
- You may upgrade a [`basic` auth](lc:langsmith/self-host-basic-auth) installation to `oauth` mode, but not a [`none` auth](lc:langsmith/authentication-methods#none) installation.
    - Upgrade a `basic` auth installation by removing the [`basic` auth configuration parameters](lc:langsmith/self-host-basic-auth#configuration) and adding the [OAuth with client secret configuration parameters](#configuration). Users may then log in via OAuth **only**. To maintain access post-upgrade, **you must have access to log in via OAuth using an email address that previously logged in via basic auth.**
...
- [Self-hosted](lc:langsmith/self-hosted) and on an [Enterprise plan](lc:langsmith/pricing-plans).
...
* IdP-initiated SSO
* SP-initiated SSO
* Just-In-Time provisioning (see [Manage user access in SSO organizations](lc:langsmith/jit-invite-sso))
...
> [!NOTE]
>
> For details on SCIM setup, refer to [Set up SCIM for your organization](lc:langsmith/user-management#set-up-scim-for-your-organization).
...
1. (Optional, if planning to use [SCIM](lc:langsmith/user-management#set-up-scim-for-your-organization) as well) Fill in `LangSmithUrl`: The `<langsmith_url>` portion from above, e.g., `langsmith.yourdomain.com`.
...
[SSO Groups Sync](lc:langsmith/user-management#sso-groups-sync-alternative) lets LangSmith assign org and workspace roles from a group membership claim on the OIDC token, as a simpler alternative to [SCIM](lc:langsmith/user-management#set-up-scim-for-your-organization). On self-hosted, you must configure the IdP to include groups in the OIDC ID token before LangSmith can read them.
...
1. Group names must follow the [SCIM naming convention](lc:langsmith/user-management#group-naming-convention) (e.g., `LS:Organization Admin`, `LS:Organization User:prod:Editor`). The separator is shared with SCIM via [`scim_group_name_separator`](lc:langsmith/user-management#configure-custom-separator).
...
Per-provider SSO Groups Sync settings are stored on the SSO provider record and toggled via the [LangSmith UI](https://smith.langchain.com) or [API](lc:langsmith/reference) (not via Helm values).
...
Groups used for SSO Groups Sync must follow the [group naming convention](lc:langsmith/user-management#group-naming-convention) documented on the main user management page (for example, `LS:Organization Admins`, `LS:Organization User:Production:Editor`). Group names are case-sensitive and must match the convention exactly.
