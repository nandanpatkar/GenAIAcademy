...
- A [LangSmith account](https://smith.langchain.com) and an [API key](lc:langsmith/create-account-api-key).
...
Alternatively, select a [LangSmith SDK profile](lc:langsmith/profile-configuration) instead of exporting a key:
...
Open [Datasets & Experiments](lc:langsmith/manage-datasets), select the dataset Harbor synced, such as `terminal-bench@2.0`, then open the Experiments tab to view the run.
...
For more information on building Deep Agents, see the [Deep Agents documentation](lc:oss/python/deepagents/overview).
...
- **Existing snapshot**: Pass `environment.kwargs.snapshot_name` to boot from a [snapshot](lc:langsmith/sandbox-snapshots) you already created.
- **Dockerfile**: Include an `environment/Dockerfile`. Harbor builds a snapshot from it with the [build-from-Dockerfile flow](lc:langsmith/sandbox-snapshots#build-a-snapshot-from-a-dockerfile), using the task `environment/` directory as the build context.
...
- [Deep Agents documentation](lc:oss/python/deepagents/overview)
- [Datasets & Experiments](lc:langsmith/manage-datasets)
- [Analyze an experiment](lc:langsmith/analyze-an-experiment)
- [Sandbox snapshots](lc:langsmith/sandbox-snapshots)
