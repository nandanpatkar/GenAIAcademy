...
description: Write better prompts for coding assistants and AI systems when you want diagram output for Flow Design.
---
...
```text
Generate OpenFlow DSL for Flow Design.
...
> `"Read https://flow-design.com/llms.txt and then generate an architecture diagram showing our Next.js frontend connecting to a Supabase backend."`

### Example Prompt for ChatGPT
If you are using ChatGPT with web-browsing enabled:

> `"Go to https://flow-design.com/llms.txt to learn the Flow Design syntax. Then, write a flowchart detailing an OAuth2 login sequence. Output the result using the \`\`\`openflow code block."`
