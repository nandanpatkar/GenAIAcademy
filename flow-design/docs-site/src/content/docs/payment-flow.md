...
description: Model subscription, checkout, retry, and exception-heavy payment systems in Flow Design.
---

Payment systems are one of the best use cases for Flow Design because they mix clear business stages with asynchronous branches, retries, and exception handling.

They are also easy to get wrong if the diagram only shows the happy path. Flow Design is a good fit because it lets you start rough, then keep adding operational detail.
...
## Why this works well in Flow Design
