...
# Catalog resource: flow-design://icons (full) or flow-design://icons/{provider} (per pack)
`;
...
export function registerResources(server: McpServer): void {
  ...
  server.registerResource(
    'dsl-cheatsheet',
    'flow-design://docs/dsl-cheatsheet',
    {
  ...
  server.registerResource(
    'icons-by-provider',
    new ResourceTemplate('flow-design://icons/{provider}', {
      list: async () => {
        const providers = await getIconProviders();
        return {
          resources: providers.map((provider) => ({
            uri: `flow-design://icons/${provider}`,
            name: `icons-${provider}`,
  ...
  server.registerResource(
    'template',
    new ResourceTemplate('flow-design://templates/{name}', {
      list: async () => ({
        resources: STARTER_TEMPLATES.map((template) => ({
          uri: `flow-design://templates/${template.name}`,
          name: template.name,
