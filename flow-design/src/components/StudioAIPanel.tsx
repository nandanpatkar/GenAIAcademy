...
function getGenerationModeButtonClassName(isActive: boolean): string {
  if (isActive) {
    return 'bg-[var(--brand-surface)] text-emerald-600 border border-emerald-200 shadow-sm';
  }
...
function getInfoIconClassName(isActive: boolean): string {
  return `h-3.5 w-3.5 focus:outline-none ${isActive ? 'text-emerald-400' : 'text-[var(--brand-secondary)]'}`;
}
