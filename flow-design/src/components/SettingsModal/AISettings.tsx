...
export function AISettings(): React.ReactElement {
    ...
    const customModelError = currentProvider === 'custom' && readiness.blockingIssue?.detail.includes('model ID')
        ? readiness.blockingIssue.detail
        : undefined;
    const azureEndpointError = currentProvider === 'azure-openai' && readiness.blockingIssue?.detail.includes('endpoint')
        ? readiness.blockingIssue.detail
        : undefined;
    const azureDeploymentError = currentProvider === 'azure-openai' && readiness.blockingIssue?.detail.includes('deployment')
        ? readiness.blockingIssue.detail
        : undefined;
        ...
        <div className="space-y-8 pb-4 animate-in fade-in duration-200 w-full min-w-0">
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-3">
                    ...
                    {currentProvider === 'custom' || currentProvider === 'azure-openai' ? (
                        <div className="space-y-2">
                            <Input
                                value={currentModel === 'custom' ? '' : currentModel}
                                onChange={e => setAISettings({ model: e.target.value })}
                                placeholder={currentProvider === 'azure-openai' ? 'gpt-5.2' : 'e.g. llama3-70b-8192 or gpt-4o'}
                                error={currentProvider === 'azure-openai' ? azureDeploymentError : customModelError}
                                helperText={currentProvider === 'azure-openai'
                                    ? 'Enter the deployment name from Microsoft Foundry (often gpt-5.2).'
                ...
                {currentProvider === 'azure-openai' && (
                    <div className="space-y-3">
                        ...
                        <Input
                            value={aiSettings.customBaseUrl ?? ''}
                            onChange={e => setAISettings({ customBaseUrl: e.target.value })}
                            placeholder="https://aicxm-openai.cognitiveservices.azure.com/"
                            error={azureEndpointError}
                            helperText="Paste the endpoint from your Azure OpenAI resource. /openai/v1 is added automatically."
                        />
                    </div>
                )}
            </div>
