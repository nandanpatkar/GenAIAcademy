...
import { X, Keyboard, PenSquare, Plug, WandSparkles, ImagePlus } from 'lucide-react';
...
import { CustomIconsSettings } from './CustomIconsSettings';
import { SidebarItem } from '../ui/SidebarItem';

interface SettingsModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialTab?: 'canvas' | 'shortcuts' | 'ai' | 'mcp' | 'custom-icons';
}
...
function OpenSettingsModalContent({
  ...
  const [activeTab, setActiveTab] = useState<'canvas' | 'shortcuts' | 'ai' | 'mcp' | 'custom-icons'>(initialTab);
      ...
      >
        ...
        <div className="w-full md:w-64 bg-[var(--brand-background)]/50 border-b md:border-b-0 md:border-r border-[var(--color-brand-border)]/60 p-2 md:p-4 flex flex-row md:flex-col gap-1 overflow-x-auto md:overflow-visible items-center md:items-stretch custom-scrollbar shrink-0">
          ...
          <SidebarItem
            ...
            isActive={activeTab === 'custom-icons'}
        ...
        <div className="flex-1 flex flex-col min-w-0 min-h-0 bg-[var(--brand-surface)]/50">
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar">
            <div className="p-6">
              ...
              {activeTab === 'custom-icons' && <CustomIconsSettings />}
            </div>
          </div>
        </div>
      </div>
