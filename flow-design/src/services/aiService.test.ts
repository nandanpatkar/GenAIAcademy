...
import { buildAzureOpenAIChatUrl, parseClaudeContent, parseOpenAICompatibleContent } from './aiService';
import { parseClaudeStreamDelta, parseOpenAIStreamDelta } from './aiServiceSchemas';

describe('aiService response parsing', () => {
  it('builds Azure OpenAI v1 chat URLs from resource or API base endpoints', () => {
    expect(buildAzureOpenAIChatUrl('https://example.openai.azure.com/')).toBe(
      'https://example.openai.azure.com/openai/v1/chat/completions'
    );
    expect(buildAzureOpenAIChatUrl('https://example.openai.azure.com/openai/v1')).toBe(
      'https://example.openai.azure.com/openai/v1/chat/completions'
    );
  });
