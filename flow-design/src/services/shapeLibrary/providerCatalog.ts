...
import { listCloudIcons, isSupabaseAvailable } from '@/services/cloudIconService';
...
export async function loadProviderCatalog(
  ...
  const catalogPromise = (async () => {
    // 1. Build-time bundled SVG icons (fast, synchronous)
    const bundledItems = SVG_SOURCES.filter((source) => source.provider === normalizedProvider)
    ...
    // 2. Supabase cloud icons (runtime uploaded) — merged on top
    const cloudItems: DomainLibraryItem[] = [];
    if (isSupabaseAvailable) {
      try {
        const cloudIcons = await listCloudIcons(normalizedProvider);
        for (const ci of cloudIcons) {
          const packId = `cloud-icon:${ci.provider}`;
          const shapeId = `${slugify(ci.category)}-${ci.name}`;
          cloudItems.push({
            id: `${packId}:${shapeId}`,
            category: provider,
            label: ci.name.split('-').map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(' '),
            description: `${provider.toUpperCase()} ${ci.category} (custom)`,
            icon: 'Box',
            color: getProviderColor(normalizedProvider),
            nodeType: 'custom',
            assetPresentation: 'icon',
            providerShapeCategory: ci.category,
            archIconPackId: packId,
            archIconShapeId: shapeId,
            // Store publicUrl in a non-standard field for preview resolution
            cloudIconUrl: ci.publicUrl,
          } as DomainLibraryItem & { cloudIconUrl: string });
        }
      } catch {
        // Cloud fetch failed silently — bundled icons still show
      }
    }

    return [...bundledItems, ...cloudItems].sort((left, right) =>
      left.providerShapeCategory === right.providerShapeCategory
        ? left.label.localeCompare(right.label)
        : (left.providerShapeCategory || '').localeCompare(right.providerShapeCategory || '')
    );
...
/** Call this after uploading a new icon to force the catalog to re-fetch. */
export function invalidateProviderCatalogCache(provider?: string): void {
  if (provider) {
    providerCatalogPromiseCache.delete(normalizeProviderPathSegment(provider));
  } else {
    providerCatalogPromiseCache.clear();
  }
}
...
export async function loadProviderShapePreview(
  ...
  // packId format: "cloud-icon:{provider}"
  if (packId.startsWith('cloud-icon:')) {
    // The publicUrl is stored on the catalog item; look it up from all cached providers
    for (const [, promise] of providerCatalogPromiseCache) {
      const items = await promise;
      const match = items.find(
        (item) => item.archIconPackId === packId && item.archIconShapeId === shapeId
      ) as (DomainLibraryItem & { cloudIconUrl?: string }) | undefined;
      if (match?.cloudIconUrl) {
        const preview: ProviderShapePreview = {
          packId,
          shapeId,
          label: match.label,
          category: match.providerShapeCategory ?? '',
          previewUrl: match.cloudIconUrl,
        };
