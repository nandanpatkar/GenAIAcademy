...
export function getAIReadinessState(aiSettings: AISettings): AIReadinessState {
  ...
  if (provider === 'azure-openai') {
    if (!customBaseUrl || !isValidHttpUrl(customBaseUrl)) {
      return {
        ...
        blockingIssue: buildBlockingIssue(
          providerName,
          'Add a valid Azure resource endpoint, for example https://your-resource.openai.azure.com.',
        ),
    ...
    if (!apiKey) {
      return {
        canGenerate: false,
        blockingIssue: buildBlockingIssue(providerName, 'Add your Azure OpenAI API key in Settings before generating.'),
        advisory: getProviderAdvisory(provider, providerName),
      };
    }
    ...
    if (!model) {
      return {
        canGenerate: false,
        blockingIssue: buildBlockingIssue(providerName, 'Add the Azure deployment name before generating.'),
        advisory: getProviderAdvisory(provider, providerName),
      };
    }
