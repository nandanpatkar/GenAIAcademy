...
export function highlightGitCommands(text: string) {
    ...
    return parts.map((part, index) => {
        if (part.startsWith("`") && part.endsWith("`")) {
            ...
            return (
                <code key={index} className="rounded bg-emerald-800/40 px-1.5 py-0.5 font-mono text-sm text-zinc-300">
                    {command}
                </code>
            );
        }
                ...
                <span key={index}>
                    {boldParts.map((boldPart, boldIndex) => {
                        if (boldPart.startsWith("**") && boldPart.endsWith("**")) {
                            return (
                                <strong key={`${index}-${boldIndex}`} className="font-semibold text-zinc-100">
                                    {boldPart.slice(2, -2)}
                                </strong>
                            );
                        }
