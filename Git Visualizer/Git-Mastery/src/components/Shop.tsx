...
export function Shop({ isOpen, onClose }: ShopProps) {
    ...
    const getRarityColor = (rarity: ShopItem["rarity"]) => {
        switch (rarity) {
            ...
            case "epic":
                return "text-[#00ff88] border-emerald-600";
            case "legendary":
    ...
    return (
        <>
            <Dialog open={isOpen} onOpenChange={onClose}>
                <DialogContent className="mx-2 flex max-h-[90vh] w-[calc(100vw-1rem)] max-w-4xl flex-col overflow-hidden border-white/10 bg-[#0a0a0c] text-zinc-100 sm:mx-6 sm:w-[calc(100vw-3rem)] md:mx-0 md:w-full">
                    <DialogHeader className="flex-shrink-0">
                        <DialogTitle className="flex items-center text-xl text-white sm:text-2xl">
                            <ShoppingCart className="mr-2 h-5 w-5 text-[#00ff88] sm:h-6 sm:w-6" />
                            {t("shop.title")}
                        </DialogTitle>
                        <DialogDescription className="text-sm text-zinc-300 sm:text-base">
                            {t("shop.subtitle")}
                        </DialogDescription>
                                ...
                                return (
                                    <Card
                                        ...
                                        className={`border transition-all duration-300 ${getRarityColor(item.rarity)} ${getRarityBg(item.rarity)} ${
                                            isPurchased ? "opacity-60" : "hover:shadow-lg hover:shadow-emerald-500/20"
                                        } min-w-0`}>
                                        ...
                                        <CardContent className="space-y-3 overflow-hidden p-4 pt-0 sm:space-y-4 sm:p-6 sm:pt-0">
                                            <p className="break-words text-xs text-zinc-200 sm:text-sm">
                                                {item.description}
                                            </p>
                                                        ...
                                                        className={`w-full sm:w-auto ${
                                                            isPurchased
                                                                ...
                                                                : !canAfford
                                                                  ...
                                                                  : "bg-[#00cc66] text-white hover:bg-emerald-700"
                                                        }`}>
                                                        {isPurchased
                                                            ? t("shop.purchased")
                                                            : !canAfford
            ...
            <Dialog open={showProTip} onOpenChange={setShowProTip}>
                <DialogContent className="max-w-md border-blue-600 bg-gradient-to-br from-blue-950 to-emerald-950">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-zinc-200">{t("shop.proTip.subtitle")}</DialogDescription>
                    </DialogHeader>

                    <div className="space-y-4">
                        <div className="rounded-lg border border-blue-700 bg-blue-950/50 p-4">
                            <p className="text-lg leading-relaxed text-zinc-100">{currentTip}</p>
                        </div>
