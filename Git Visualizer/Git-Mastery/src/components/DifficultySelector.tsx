...
export function DifficultySelector({ isOpen, onClose, isInitialSelection = false }: DifficultySelectorProps) {
    ...
    const getDifficultyColorClasses = (difficulty: DifficultyLevel, isSelected: boolean) => {
        ...
        if (isSelected) {
            switch (difficulty) {
                ...
                case "pro":
                    return `${baseClasses} border-[#00ff88]/50 bg-white/10 ring-2 ring-[#00ff88]/30`;
            }
        }
        ...
        switch (difficulty) {
            ...
            case "pro":
                return `${baseClasses} border-emerald-800/30 bg-zinc-900/10 hover:border-emerald-600 hover:bg-white/10`;
            default:
                return baseClasses;
        }
    };
    ...
    return (
        <Dialog open={isOpen} onOpenChange={() => !isInitialSelection && onClose()}>
            <DialogContent
                className="z-50 mx-2 max-h-[90vh] w-[calc(100vw-1rem)] max-w-4xl overflow-y-auto border-white/10 bg-[#0a0a0c] text-zinc-100 sm:mx-6 sm:w-[calc(100vw-3rem)] md:mx-0 md:w-full"
                ...
                {isInitialSelection && (
                    <div className="absolute left-1/2 top-4 z-10 -translate-x-1/2 transform px-2 sm:top-2">
                        <div className="whitespace-nowrap rounded-full bg-gradient-to-r from-emerald-600 to-pink-600 px-3 py-1 text-xs font-semibold text-white sm:px-4">
                ...
                <DialogHeader className="mt-8 text-center sm:mt-3">
                    <DialogTitle className="flex items-center justify-center text-xl text-white sm:text-2xl">
                        <Settings className="mr-2 h-5 w-5 text-[#00ff88] sm:h-6 sm:w-6" />
                        {isInitialSelection ? "Welcome to Git Mastery! 🚀" : "Change Difficulty Level"}
                    </DialogTitle>
                    <DialogDescription className="px-2 text-sm text-zinc-300 sm:px-0 sm:text-base">
                        {isInitialSelection
                    ...
                    {difficulties.map(diff => (
                        <Card
                            ...
                            <CardContent className="flex flex-1 flex-col space-y-2 p-3 sm:space-y-3 sm:p-6">
                                <p className="text-center text-xs text-zinc-200 sm:text-sm">{diff.description}</p>
                                ...
                                <div className="pt-2 text-center">
                                    <div className="text-xs text-[#00ff88]">Max Points: {diff.maxPoints}</div>
                                </div>
                ...
                <div className="mt-4 flex flex-col justify-center space-y-2 sm:mt-6 sm:flex-row sm:space-x-4 sm:space-y-0">
                    {!isInitialSelection && (
                        <Button
                            variant="outline"
                            onClick={onClose}
                            className="w-full border-emerald-700 text-zinc-300 hover:bg-zinc-900/50 sm:w-auto">
                            Cancel
                        </Button>
                    )}
                    <Button
                        ...
                        className={`w-full sm:w-auto ${
                            selectedDifficulty === "beginner"
                                ...
                                : selectedDifficulty === "advanced"
                                  ...
                                  : "bg-[#00cc66] hover:bg-emerald-700"
                        } text-white`}>
