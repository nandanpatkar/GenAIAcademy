...
function LevelPageContent() {
    ...
    const FileTreeItem = ({
        ...
        if (item.isDirectory) {
            ...
            return (
                <div className="mb-1">
                    <div
                        className="flex cursor-pointer items-center rounded px-2 py-0.5 hover:bg-zinc-900/30"
                        onClick={() => setIsOpen(!isOpen)}>
                        <span className="mr-1 text-[#00ff88]">
                            {isOpen ? <ChevronDown className="h-3 w-3" /> : <ChevronRight className="h-3 w-3" />}
                        </span>
                        <Folder className="mr-1 h-3.5 w-3.5 text-[#00ff88]" />
                        <span className="font-medium text-zinc-300">{item.name === "/" ? "root" : item.name}</span>
                    </div>
            ...
            // File
            return (
                <div className="mb-1 flex items-center justify-between rounded px-2 py-0.5 hover:bg-zinc-900/30">
                    <div className="flex items-center truncate text-left text-zinc-300" title={item.path}>
                        <FileIcon className="mr-1 h-3 w-3 text-[#00ff88]" />
                        <span>{item.name}</span>
                    </div>
                    <div className="flex">
                        <Button
                            variant="ghost"
                            size="sm"
                            className="ml-1 h-6 w-6 p-0 text-zinc-300 hover:bg-white/20 hover:text-zinc-100"
                        ...
                        <Button
                            variant="ghost"
                            size="sm"
                            className="ml-1 h-6 w-6 p-0 text-zinc-300 hover:bg-red-900/30 hover:text-red-300"
    ...
    const renderEditableFiles = () => {
            ...
            return (
                <div className="mt-4">
                    <div className="mb-2 flex items-center justify-between">
                        <h3 className="font-medium text-zinc-200">{t("level.filesToEdit")}</h3>
                        {!isGitInitialized && (
                            <span className="text-xs text-yellow-400">⚠️ {t("level.gitNotInitialized")}</span>
                        )}
                    </div>
                    <p className="text-sm text-[#00ff88]">No editable files found.</p>
                </div>
            );
        }
        ...
        return (
            <div className="mt-4">
                ...
                <div className="rounded border border-emerald-800/30 bg-zinc-900/10 p-3">
    ...
    const renderLevelChallenge = () => {
        if (!levelData) {
            return <div className="text-zinc-300">{t("level.notFound")}</div>;
        }
            ...
            <ClientOnly>
                <div className="space-y-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <p className="text-sm text-zinc-200 sm:text-base">{levelData.description}</p>
                    </div>

                    {/* Objectives Section */}
                    <div className="space-y-3">
                        <h3 className="flex items-center text-sm font-medium text-zinc-200 sm:text-base">
                            {t("level.objectives")}
                        </h3>
                        <div className="space-y-2">
                            {levelData.objectives.map((objective, index) => {
                                ...
                                return (
                                    <div
                                        key={index}
                                        className="flex items-start space-x-3 rounded-lg bg-white/10 p-3 transition-all hover:bg-zinc-900/30">
                                        <div
                                            className={`mt-0.5 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full border-2 transition-all ${
                                                isCompleted
                                                    ? "border-green-500 bg-green-500/20"
                                                    : "border-[#00ff88]/50 bg-white/10"
                                            }`}>
                                            {isCompleted && (
                                                <svg
                                        ...
                                        <span
                                            className={`text-sm sm:text-base ${isCompleted ? "text-green-300 line-through" : "text-zinc-300"}`}>
                                            {objective}
                                        </span>
                                    </div>
                                );
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:gap-3">
                        <Button
                            ...
                            className="flex flex-1 items-center justify-center border-emerald-700 text-zinc-300 hover:bg-zinc-900/50">
                        ...
                        {isLevelCompleted && (
                            <Button
                                ...
                                size="sm"
                                className="flex flex-1 items-center justify-center bg-[#00cc66] text-white hover:bg-emerald-700">
                    ...
                    {/* Hints Section */}
                    {showHints && (
                        <div className="mt-1 rounded-lg border border-emerald-700/50 bg-zinc-900/30 p-3">
                            <h3 className="mb-2 flex items-center text-sm font-medium text-zinc-200">
                                <HelpCircleIcon className="mr-2 h-4 w-4" />
                                {t("level.hints")}
                            </h3>
            ...
            <div className="bg-[#0a0a0c] text-zinc-100">
                <div className="container mx-auto p-2 sm:p-4">
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:gap-4 lg:grid-cols-2">
                        ...
                        <Card className="order-1 flex flex-col overflow-hidden border-white/10 bg-zinc-900/10 lg:order-2 lg:h-[580px]">
                            <CardHeader className="shrink-0 p-3 sm:p-6 sm:pb-0">
                                <CardTitle className="flex items-center justify-between text-base text-white sm:text-lg">
                                    <div className="flex items-center">
                                        <Shield className="mr-2 h-4 w-4 text-[#00ff88] sm:h-5 sm:w-5" />
                                    ...
                                    <div className="group relative">
                                        <Button
                                            ...
                                            className={`h-8 w-8 rounded-full p-0 transition-all hover:bg-white/20 ${
                                                isAdvancedMode
                                                    ? "bg-[#00cc66]/30 text-zinc-300"
                                                    : "text-[#00ff88] hover:text-zinc-300"
                                            }`}>
                                            {isAdvancedMode ? (
